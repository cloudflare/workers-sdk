...
async function classify_sentiment(input: string|string[], config?: TextClassificationConfig): Promise<string> {
    let topk = 1
    if (config?.topk) {
        topk = config.topk
    }
...
async function textSimilarity(query: string, value: string|string[]): Promise<string> {
...
async function translate(input: string|string[], config: TranslationConfig): Promise<string> {
    const max_tokens = 20;
