	...
	stft(frames, window) {
		...
		for (let i = 0; i < frames.length; ++i) {
			...
			for (let j = 0; j < outBuffer.length; j += 2) {
				...
				buffer2[j] =
					outBuffer2[j] * outBuffer[j] - outBuffer2[j2] * outBuffer[j2];
				buffer2[j2] =
					outBuffer2[j] * outBuffer[j2] + outBuffer2[j2] * outBuffer[j];
			}
			// TODO: decide between Float32Array and Float64Array
	...
	fram_wave(waveform, center = true) {
		...
		for (let i = 0; i < waveformLength + 1; i += this.config.hop_length) {
			...
			if (center) {
				...
				let frameEnd =
					i < waveformLength - half_window ? i + half_window : waveformLength;
