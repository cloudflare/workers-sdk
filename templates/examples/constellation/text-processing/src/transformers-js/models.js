...
const { Tensor: ConsnTensor, InferenceSession } = require("@cloudflare/constellation")
const {
    Tensor,
    cat
...
async function constructSession(modelName) {
    const sess = await new InferenceSession(
        globalThis.binding,
        modelName,
    )
...
async function seq2seq_forward(self, model_inputs, {
    ...
    if (use_merged) {
        ...
        decoder_session = withPast ? self.decoder_with_past_session : self.decoder_session
    }
    ...
    getPastKeyValues(decoderResults, pastKeyValues) {
        ...
        if (pastKeyValues !== null) {
            for (const name in pastKeyValues) {
                if (name.startsWith('past_key_values') && name.includes('encoder') && !(name in pkvs)) {
...
class T5ForConditionalGeneration extends T5PreTrainedModel {
     ...
     */
    constructor(config, session, decoder_session, decoder_with_past_session, generation_config) {
