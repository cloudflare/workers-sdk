	...
	return PACKAGES.map((pkg) => {
		...
		try {
			...
			for (const entry of packed.files) {
				const path = object(entry, "Packed file").path;
				if (
					typeof path !== "string" ||
					path.startsWith("/") ||
					path.split("/").includes("..") ||
...
export function validate(input) {
	...
	return {
		...
		packages: report.packages.map((inputPackage, index) => {
			const comparison = object(inputPackage, "Package comparison");
...
function measurement(input, name) {
	const value = object(input, "Measurement");
...
export function bindReport(report, event, pullRequest, candidateCommit) {
	const workflowEvent = object(event, "Event");
	const repository = object(workflowEvent.repository, "Repository");
	const run = object(workflowEvent.workflow_run, "Workflow run");
	if (!Array.isArray(run.pull_requests) || run.pull_requests.length === 0) {
		throw new Error("Workflow run has no pull request");
	}
	const workflowPull = object(run.pull_requests[0], "Workflow PR");
	const pull = object(pullRequest, "Pull request");
	const head = object(pull.head, "PR head");
	const base = object(pull.base, "PR base");
	if (
		pull.number !== workflowPull.number ||
		head.sha !== run.head_sha ||
		head.ref !== run.head_branch ||
		object(head.repo, "Head repo").full_name !==
			object(run.head_repository, "Run head repo").full_name ||
		object(base.repo, "Base repo").full_name !== repository.full_name ||
		pull.merge_commit_sha !== report.candidateSha
	) {
		throw new Error("Report does not match the PR merge ref");
	}
	const commit = object(candidateCommit, "Commit");
	if (
		commit.sha !== report.candidateSha ||
		!Array.isArray(commit.parents) ||
		commit.parents.length !== 2 ||
		object(commit.parents[0], "First parent").sha !== report.baselineSha ||
		object(commit.parents[1], "Second parent").sha !== run.head_sha
	) {
