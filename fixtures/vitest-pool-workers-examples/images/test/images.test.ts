...
const TINY_PNG_BUFFER = TINY_PNG.slice().buffer;
...
describe("Images binding", () => {
	...
	it("can upload an image", async ({ expect }) => {
		const metadata = await env.IMAGES.hosted.upload(TINY_PNG_BUFFER, {
			id: "test-123",
		});
	...
	it("can upload a base64-encoded image", async ({ expect }) => {
		...
		const metadata = await env.IMAGES.hosted.upload(
			base64Bytes.slice().buffer,
			{
				id: "base64-test",
				encoding: "base64",
			}
	...
	it("can get image metadata", async ({ expect }) => {
		await env.IMAGES.hosted.upload(TINY_PNG_BUFFER, { id: "get-test" });
	...
	it("can get image data", async ({ expect }) => {
		await env.IMAGES.hosted.upload(TINY_PNG_BUFFER, { id: "blob-test" });
	...
	it("can update image metadata", async ({ expect }) => {
		await env.IMAGES.hosted.upload(TINY_PNG_BUFFER, { id: "update-test" });
	...
	it("can delete an image", async ({ expect }) => {
		await env.IMAGES.hosted.upload(TINY_PNG_BUFFER, { id: "delete-test" });
	...
	it("can list images", async ({ expect }) => {
		await env.IMAGES.hosted.upload(TINY_PNG_BUFFER);
	...
	it("can use list images with a cursor", async ({ expect }) => {
		...
		for (const id of imageIds) {
			await env.IMAGES.hosted.upload(TINY_PNG_BUFFER, { id });
		}
		...
		const page2 = await env.IMAGES.hosted.list({
			limit: 2,
			cursor: page1.cursor,
		});
		...
		const page3 = await env.IMAGES.hosted.list({
			limit: 2,
			cursor: page2.cursor,
		});
