...
describe("regressions", () => {
	...
	test("#5095 requests where body (of >= 2 bytes) is not drained sometimes fail", async () => {
		const run = await fakeStartUserWorker({
			script: `
				export default {
					async fetch(request) {
		...
		const COUNT = 30;
		const STEP_DELAY = 4; // setting to less than 4 results in errorCount === 0 indicating race condition or a request influencing a subsequent request
		let requestCount = 0;
		let errorCount = 0;

		await Promise.allSettled(
			Array.from({ length: COUNT }).map(async (_, i) => {
				await new Promise((r) => setTimeout(r, i * STEP_DELAY));

				return undici
					.fetch(run.proxyWorkerUrl, {
						method: "POST",
