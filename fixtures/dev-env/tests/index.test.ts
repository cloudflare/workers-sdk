...
} from "wrangler/src/api/startDevWorker/types";
...
async function fakeStartUserWorker(options: {
	...
	config?: OptionalKeys<StartDevWorkerInput, "name" | "entrypoint">;
}) {
...
function fakeConfigUpdate(config: StartDevWorkerInput) {
	devEnv.proxy.onConfigUpdate({
		type: "configUpdate",
		config: config as StartDevWorkerOptions,
	});

	return config; // convenience to allow calling and defining new config inline but also store the new object
}
function fakeReloadStart(config: StartDevWorkerInput) {
