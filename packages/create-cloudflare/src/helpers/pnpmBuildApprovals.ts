...
// Build-script packages C3 pre-approves because they are installed through
// Wrangler. Framework-introduced build scripts are out of scope.
const APPROVED_BUILDS = ["esbuild", "node-hid", "workerd"] as const;
...
const FRESH_HEADER = [
	...
	"# them: `workerd` downloads the platform binary, while `esbuild` and",
	"# `node-hid` download/build native bindings. Without these, pnpm 11+",
