...
const verifyBuildCfTypesScript = async (
	...
	const outputFileContent = readFile(join(projectPath, outputFile));
	const outputFileContentLines = outputFileContent.split("\n");

	expect(outputFileContentLines[0]).toContain(
	...
	expect(outputFileContentLines[1]).toContain(
		`"// Runtime types generated with workerd@`,
	);
	expect(outputFileContentLines[1]).toContain(compatFlags?.sort().join(", "));
	// the file still contains the env interface
	const hasEnvInterface = outputFileContentLines.some(
		(line) =>
