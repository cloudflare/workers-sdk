...
function getFrameworkTestConfig(pm: string): NamedFrameworkTestConfig[] {
	return [
		...
		},
		{
			...
			timeout: LONG_TIMEOUT,
			testCommitMessage: true,
			// create-vinext-app scaffolds a Workers-ready vinext project. Preview
			// maps to the generated `start` script (wrangler dev on the build
			// output), so we must build first and no OpenNext-style `--` separator
			// is needed.
			verifyPreview: {
				build: true,
				previewArgs: ["--inspector-port=0"],
...
function getExperimentalFrameworkTestConfig(
	...
	return [
		...
		},
		{
			...
			testCommitMessage: true,
			unsupportedOSs: ["win32"],
			// create-vinext-app no longer needs R2 at scaffold time (OpenNext did).
			// Keep the API-token gate only if deploy verification is enabled in CI.
			...
			nodeCompat: true,
			// vinext generates wrangler.jsonc; wrangler types should work, but the
			// scaffold does not ship a pre-baked cloudflare-env.d.ts the way the
			// old OpenNext template did.
			verifyTypes: false,
		},
		{
			name: "vike",
