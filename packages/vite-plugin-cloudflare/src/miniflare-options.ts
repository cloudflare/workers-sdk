	...
	const assetWorkers: WorkerOptions[] = [
		{
			config: {
				...
				env: {
					...
					ASSET_WORKER: {
						type: "worker",
						workerName: ASSET_WORKER_NAME,
					},
					...(entryWorkerConfig
						? {
								USER_WORKER: {
									type: "worker",
									workerName: entryWorkerConfig.name,
						...
						async ([environmentName, worker]) => {
							...
							for (const [name, binding] of Object.entries(config.env ?? {})) {
								if (binding.type === "hyperdrive") {
									assert(
										binding.localConnectionString !== undefined,
										`Hyperdrive binding "${name}" must define a localConnectionString for local development.`
									);
									env[name] = {
										...binding,
