...
// `@cloudflare/workers-utils` dependency, which bundles `xdg-app-paths` and so
// performs a top-level dynamic `require`) stays out of the plugin's module
// graph at config-load time — Vite evaluates the config in a pure-ESM context
// where that `require` throws.
