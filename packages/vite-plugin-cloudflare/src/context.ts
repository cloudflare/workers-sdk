...
export class PluginContext {
	...
	get settings(): ParsedSettingsConfig | undefined {
		if (this.resolvedPluginConfig.type === "preview") {
			return this.resolvedPluginConfig.workers[0]?.settings;
		}

		const settings = this.resolvedPluginConfig.parsedConfig.settings;
		return settings?.type === "settings" ? settings : undefined;
	}
