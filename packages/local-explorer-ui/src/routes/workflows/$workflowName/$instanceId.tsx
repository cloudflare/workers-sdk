...
const ACTION_ICONS = {
	...
	resume: PlayIcon,
	terminate: StopIcon,
} satisfies Record<Action, typeof PauseIcon>;
...
const ParamsOutput = memo<ParamsOutputProps>(function ParamsOutput({
	params,
	output,
}) {
	return (
...
const ErrorCard = memo<ErrorCardProps>(function ErrorCard({ error }) {
	return (
		<LayerCard>
...
const StepHistory = memo<StepHistoryProps>(function StepHistory({ steps }) {
	const stepList = steps ?? [];
	const [search, setSearch] = useState<string>("");
	const [typeFilter, setTypeFilter] =
		useState<(typeof STEP_TYPE_FILTERS)[number]["value"]>("all");
				...
				<div className="mb-3 flex items-center gap-2">
					...
					<DropdownMenu>
						<DropdownMenu.Trigger
							render={
								<Button icon={<ActiveIcon className="text-text-secondary" />}>
									{activeFilter.label}

									<CaretDownIcon className="text-text-secondary" />
								</Button>
							}
						/>
						...
						>
							{STEP_TYPE_FILTERS.map(({ value, label, icon: Icon }) => (
								<DropdownMenu.Item
									...
									icon={<Icon className="text-text-secondary" />}
									key={value}
...
function InstanceDetailView(): JSX.Element {
