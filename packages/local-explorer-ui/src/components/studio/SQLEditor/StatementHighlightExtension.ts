...
/**
 * Extracts the text content of a syntax node from the editor document.
 *
 * @param state - The current editor state containing the document.
 * @param node - The syntax node whose text content should be extracted.
 *
 * @returns The string content of the document between the node's start and end positions.
 */
 ...
 *
 * @returns An array of statement segments with their position (from, to), and optionally the text
 * 			content if `generateText` is true.
 */
...
/**
 * Resolves the nearest SQL statement to a given cursor position.
 *
 * If the cursor is within a statement, that statement is returned. If the cursor
 * is between statements, the function determines which statement is "nearest"
 * based on line proximity—preferring the next statement if equidistant.
 *
 * @param state - The current editor state containing the SQL syntax tree.
 * @param position - Optional cursor position to resolve from. Defaults to the
 *                   current selection's anchor position.
 *
 * @returns The nearest statement's range (from, to), or null if no statements exist.
 */
