		...
		if (stats.queryDurationMs !== null) {
			content.push(
				<div className="px-2 flex gap-1 items-center" key="query-duration">
					...
					<Tooltip
						message={
							"Time taken by the server to execute the SQL query (excludes network delay and request overhead)."
						}
					>
						<QuestionIcon />
					</Tooltip>
		...
		if (stats.requestDurationMs) {
			content.push(
				<div className="px-2 flex gap-1 items-center" key="request-duration">
					...
					<Tooltip
						message={
							"Total time from request sent to response received (includes query time, network latency, and server processing)."
						}
					>
						<QuestionIcon />
					</Tooltip>
