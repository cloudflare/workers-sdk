...
import { runStudioMultipleSQLStatements } from "../../../utils/studio/query";
...
export function StudioQueryTab({ query }: StudioQueryTabProps): JSX.Element {
	...
	const [cursorColumnNumber, setCursorColumnNumber] = useState<number>(1);
	const [cursorLineNumber, setCursorLineNumber] = useState<number>(1);
	...
	const runMultipleStatements = useCallback(
		async (statements: string[]): Promise<void> => {
			...
			try {
				const { logs, result } = await runStudioMultipleSQLStatements(
					driver,
					statements,
					setProgress
				);
				...
				const hasAlterSchema = logs.some((log) => {
	...
	const queryTabs = useMemo((): StudioWindowTabItem[] | null => {
		...
		const queryTabItems = new Array<StudioWindowTabItem>();
	...
	return (
		<SplitPane
			defaultSize={201}
			minSize={150}
			resizerClassName="!bg-border bg-border border-transparent"
			split="horizontal"
		>
			<div className="w-full flex flex-col bg-surface">
				...
				>
					<div className="text-xs grow text-muted">
						Ln {cursorLineNumber} Col {cursorColumnNumber}
					</div>
