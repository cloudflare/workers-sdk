...
export function TraceWaterfall({
	...
	// Row click behaviour:
	// - Parent spans: clicking toggles collapse/expand of their subtree and
	//   selects the span so its detail panel reflects what you clicked.
	...
	// Derive the visible window from the laid-out spans so it covers
	// sub-invocations that run past the root invocation. Falls back to the
	// provided trace duration.
...
function NormalSpanBar({
	...
	const label = span.running ? "running…" : formatDuration(spanDuration);
	...
	return (
		<div className="border-t border-kumo-fill bg-kumo-elevated p-4">
			<div className="mb-3 flex items-center gap-2">
				...
				>
					{span.running ? "running" : (span.outcome ?? "?")}
				</span>
				<span className="text-xs text-kumo-subtle">
					{span.running ? "running…" : formatDuration(span.duration_ms)}
				</span>
			</div>
			{span.error ? (
