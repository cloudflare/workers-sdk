import { Tooltip, useSidebar } from "@cloudflare/kumo";
...
export function WorkerSelector({
	...
	const selectRoot = (
		...
		>
			{sidebar.open ? (
				...
				<Select.Trigger
					aria-label={`Selected worker: ${selectedWorker}`}
					className="flex cursor-pointer items-center justify-center rounded-md p-2 text-kumo-subtle transition-colors hover:bg-kumo-fill hover:text-kumo-default data-popup-open:bg-kumo-fill data-popup-open:text-kumo-default"
				>
	...
	return (
		<div className="flex justify-center py-2">
			<Tooltip content={selectedWorker} side="right">
				{selectRoot}
			</Tooltip>
		</div>
	);
