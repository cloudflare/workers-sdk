...
describe("validateKey", () => {
	test("`MAX_KEY_BYTES` is 512", ({ expect }) => {
		expect(MAX_KEY_BYTES).toBe(512);
	});

	test("empty string returns error", ({ expect }) => {
		expect(validateKey("")).toBe("Key is required");
	});

	test("whitespace-only string returns error", ({ expect }) => {
		expect(validateKey("   ")).toBe("Key is required");
		expect(validateKey("\t")).toBe("Key is required");
	});

	test("valid key returns null", ({ expect }) => {
		expect(validateKey("my-key")).toBeNull();
	});

	test("key at exactly 512 bytes returns null", ({ expect }) => {
		const key = "a".repeat(512);
		expect(validateKey(key)).toBeNull();
	});

	test("key exceeding 512 bytes returns error", ({ expect }) => {
	...
	test("multi-byte characters are counted by byte length", ({ expect }) => {
