...
import { getIssueWithComments } from '../github/get-issue-with-comments';
 ...
 * as context to determine what to do with the issue.
 */
...
export async function getSimilarIssues(issue: Issue): Promise<Issue[]> {
	...
	const matchingIssues = matches
		.map((match) => match.metadata.issueNumber)
		// Ensure that the current issue is not included in the results
		.filter((issueNumber) => issueNumber !== issue.number)
		.map((issueNumber) => getIssueWithComments(issueNumber));
...
function buildPrompt(issue: Issue, similarIssues: Issue[]) {
	const prompt = dedent`
        ...
        **Issue Details:**
        ...
        Duplicate: [Please provide the issue URL here if it is a duplicate]
        Close issue: [Yes or No]
        Close reason: [string]
        Add to backlog: [Yes or No]
        Request reproduction: [Yes or No]
