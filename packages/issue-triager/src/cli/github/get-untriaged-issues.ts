...
export function isUntriaged(node: GitHubNodeWithContent) {
	return node.fieldValues.nodes.find((field) => field.field?.name === 'Status')?.name === 'Untriaged';
}

export function isNodeWithContent(node: GitHubNode): node is GitHubNodeWithContent {
	const { content } = node;
...
export type GitHubNodeWithContent = GitHubNode & {
	content: GitHubContent;
};
