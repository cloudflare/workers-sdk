...
export async function generateEnvTypes(
	...
	const configToDTS = buildConfigBindingsWithSecrets(config, configPath, env);
...
function buildConfigBindingsWithSecrets(
	config: Config,
	configPath: string,
	env?: string
) {
	const secrets = getVarsForDev({ configPath, vars: {} }, env, true) as Record<
		string,
		string
	>;
	...
	return configBindingsWithSecrets;
}
