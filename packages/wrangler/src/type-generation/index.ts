...
export const typesCommand = createCommand({
	...
	async handler(args) {
		...
		if (args.includeEnv) {
			...
			const { envHash, envTypes } = await generateEnvTypes(
				config,
				args,
				entrypoint,
				secondaryEntries
		...
		const commandHeader = `// Generated by Wrangler by running \`${wranglerCommandUsed}\` (hash: ${hash ?? "n/a"})`;
		header.push(commandHeader);
...
export async function generateEnvTypes(
	...
	log = true
): Promise<{ envHash: string; envTypes: string | undefined }> {
	const { path: outputPath } = args;
