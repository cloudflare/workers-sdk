...
export const typesCommand = createCommand({
	...
	args: {
		...
		"header-hash": {
			type: "boolean",
			default: true,
			describe: "Include a hash of the types in the file's header",
		},
	},
	validateArgs(args) {
...
export async function generateEnvTypes(
	...
	if (entrypointFormat === "modules" || typesHaveBeenFound) {
		...
		const envHeader = `// Generated by Wrangler by running \`${wranglerCommandUsed}\`${args.headerHash ? ` (hash: ${hash})` : ""}`;
