...
// diffs.
export function cleanForInstanceType(app: Record<string, any>): ContainerApp {
	if (!("configuration" in app)) {
		return app as ContainerApp;
	}

	const instance_type = inferInstanceType(
		app.configuration as UserDeploymentConfiguration
	);
	if (instance_type !== undefined) {
