...
export const secretBulkCommand = createCommand({
	metadata: {
		description: "Create, update, or delete multiple secrets for a Worker in a single request, with up to 25 total modifications per request.",
		status: "stable",
		owner: "Workers: Deploy and Config",
	},
	positionalArgs: ["file"],
	...
	args: {
		file: {
			describe: `The file of key-value pairs to create, update, or delete, as JSON in form {"key": "value", ...} or .env file in the form KEY=VALUE. Set a key to null in the JSON file to delete it. If omitted, Wrangler expects to receive input from stdin rather than a file.`,
			type: "string",
		},
		name: {
			describe:
