...
export const certNamespace = createNamespace({
	metadata: {
		description: "🪪  Manage client certificates and CA certificate chains used for mTLS connections",
		status: "open-beta",
        owner: "Product: SSL",
	},
});
...
export const certDeleteCommand = createCommand({
	...
	async handler({id, name}, { config }) {
        ...
        if (id) {
            ...
            certificate = await getMTlsCertificateByName(accountId, name as string, true);
        }
