...
See https://developers.cloudflare.com/workers/platform/compatibility-dates for more information.`);
	...
	if (props.noBundle && minify) {
		logger.warn(
			"`--minify` and `--no-bundle` are not supported together. If you want to minify your Worker and disable Wrangler's bundling, please minify as part of your own bundling process."
		);
	}

	if (props.noBundle && nodeCompat) {
		logger.warn(
			"`--node-compat` and `--no-bundle` are not supported together. If you want to polyfill Node.js built-ins and disable Wrangler's bundling, please polyfill as part of your own bundling process."
		);
	}
