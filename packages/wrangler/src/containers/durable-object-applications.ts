...
function getContainerImageClasses(
	...
	// User variables can legally use the image binding's name when no managed
	// Containers are configured. Only Wrangler's versioned marker gives that
	// binding Container semantics, including classes with empty image maps.
	if (!hasContainerImagesMetadata(version.resources.bindings)) {
		return undefined;
	}
