...
/** Returns whether command arguments select the local rather than remote store. */
...
/** Runs an operation against the persisted local Flagship store. */
export async function usingLocalFlagshipAPI<T>(
	...
	if (Object.hasOwn(getWorkerRegistry(getRegistryPath()), workerName)) {
		throw new UserError(
			`Cannot use local Flagship commands while a dev session for '${workerName}' is running. Stop the dev session and retry.`,
			{ telemetryMessage: "flagship local store dev session active" }
		);
	}
...
async function localOperation<T>(
	...
	try {
		...
		if (
			...
			!/^Flag(?: input| key| ')/.test(error.message)
		) {
			throw error;
		}
		throw new UserError(error.message, { telemetryMessage });
	}
}
...
/** Runs a command operation against its selected local or remote flag store. */
