...
import { getR2LocalUploads, setR2LocalUploads } from "./helpers/local-uploads";
...
export const r2BucketLocalUploadsGetCommand = createCommand({
	metadata: {
	...
	async handler(args, { config }) {
		...
		const localUploads = await getR2LocalUploads(
			config,
			accountId,
			bucket,
			jurisdiction
...
export const r2BucketLocalUploadsEnableCommand = createCommand({
	...
	async handler(args, { config }) {
		...
		await setR2LocalUploads(config, accountId, bucket, true, jurisdiction);
...
export const r2BucketLocalUploadsDisableCommand = createCommand({
	...
	async handler(args, { config }) {
		...
		await setR2LocalUploads(config, accountId, bucket, false, jurisdiction);
