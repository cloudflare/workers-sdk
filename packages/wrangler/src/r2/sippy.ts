...
export const r2BucketSippyEnableCommand = createCommand({
	...
	async handler(args, { config }) {
		...
		if (isInteractive) {
			...
			if (!args.provider) {
				throw new UserError(
					"Missing cloud storage provider. Use --provider to specify the source provider (AWS or GCS).",
					{
			...
			if (!SIPPY_PROVIDER_CHOICES.includes(args.provider)) {
				throw new UserError(
					`Invalid cloud storage provider: "${args.provider}". The --provider flag must be one of: AWS, GCS.`,
					{
			...
			if (!args.bucket) {
				throw new UserError(
					`Missing upstream bucket name. Use --bucket to specify the name of your ${args.provider} bucket.`,
					{
			...
			if (args.provider === "AWS") {
				...
				if (!args.region) {
					throw new UserError(
						"Missing AWS region. Use --region to specify the AWS region of your S3 bucket (e.g., us-west-2).",
						{
				...
				if (!args.accessKeyId) {
					throw new UserError(
						"Missing AWS access key. Use --access-key-id to specify the AWS access key ID with read and list access to your S3 bucket.",
						{
				...
				if (!args.secretAccessKey) {
					throw new UserError(
						"Missing AWS secret access key. Use --secret-access-key to specify the AWS secret access key for your S3 bucket.",
						{
				...
				if (
					...
					if (!args.serviceAccountKeyFile) {
						throw new UserError(
							"Missing GCS credentials. Use --service-account-key-file to specify the path to your Google Cloud service account key JSON file, or provide --client-email and --private-key directly.",
							{
								telemetryMessage:
			...
			if (!args.r2AccessKeyId) {
				throw new UserError(
					"Missing R2 access key. Use --r2-access-key-id to specify the R2 access key ID with read and write access to your R2 bucket. You can create API tokens at https://dash.cloudflare.com/?to=/:account/r2/api-tokens.",
					{
			...
			if (!args.r2SecretAccessKey) {
				throw new UserError(
					"Missing R2 secret access key. Use --r2-secret-access-key to specify the R2 secret access key for your R2 bucket.",
					{
		...
		if (args.provider === "AWS") {
			...
			if (!args.bucket) {
				throw new UserError(
					"Missing upstream bucket name. Use --bucket to specify the name of your AWS S3 bucket.",
					{
			...
			if (!args.bucket) {
				throw new UserError(
					"Missing upstream bucket name. Use --bucket to specify the name of your GCS bucket.",
					{
			...
			if (!args.clientEmail) {
				throw new UserError(
					"Missing GCS client email. Use --service-account-key-file to provide your Google Cloud service account key JSON file, or specify --client-email directly.",
					{
			...
			if (!args.privateKey) {
				throw new UserError(
					"Missing GCS private key. Use --service-account-key-file to provide your Google Cloud service account key JSON file, or specify --private-key directly.",
					{
