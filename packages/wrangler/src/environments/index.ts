...
export function applyServiceAndEnvironmentTags(config: Config, tags: string[]) {
	...
	if (shouldApplyTags && !serviceName) {
		...
		return tags.some((tag) => tag.startsWith(SERVICE_TAG_PREFIX))
			? tags.filter((tag) => !tag.startsWith(SERVICE_TAG_PREFIX)) // remove any accidental `cf:service=undefined` tags wrangler@4.37.0 may have set
			: tags;
	}

	const serviceTag = shouldApplyTags
		? `${SERVICE_TAG_PREFIX}${serviceName}`
		: null;
	const environmentTag = env ? `${ENVIRONMENT_TAG_PREFIX}${env}` : null;

	const hasMissingOrStaleServiceTag = serviceTag
		? !tags.includes(serviceTag)
		: tags.some((tag) => tag.startsWith(SERVICE_TAG_PREFIX));
	...
	if (hasMissingOrStaleServiceTag || hasMissingOrStaleEnvironmentTag) {
