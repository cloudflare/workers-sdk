...
export const versionsUploadCommand = createCommand({
	...
	handler: async function versionsUploadHandler(args, { config }) {
		...
		let previewAlias: string | undefined = args.previewAlias;
		const shouldGeneratePreviewAlias = getCIGeneratePreviewAlias();
		if (shouldGeneratePreviewAlias === "true") {
			previewAlias = generatePreviewAlias(name);
