...
describe("createFlatWorkerUploadForm — commonjs service-worker format", () => {
	it.for([
		{
			...
			expectedBinding: { name: "add_wasm", type: "wasm_module", part: "add_wasm" },
		...
		},
		{
			...
			expectedBinding: { name: "template_txt", type: "text_blob", part: "template_txt" },
		...
		},
		{
			...
			expectedBinding: { name: "data_bin", type: "data_blob", part: "data_bin" },
			expectedPartName: "data_bin",
			expectedMime: "application/octet-stream",
		},
	])(
		"should convert $label bindings in commonjs format",
		({ module, expectedBinding, expectedPartName, expectedMime }, { expect }) => {
...
describe("createFlatWorkerUploadForm — __STATIC_CONTENT_MANIFEST ESM subdirectories", () => {
