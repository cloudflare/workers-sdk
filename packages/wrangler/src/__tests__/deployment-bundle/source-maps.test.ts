...
describe("loadSourceMaps", () => {
	it("loads source maps from bundled metadata", ({ expect }) => {
		...
		const main = makeModule("src/index.ts", path.join(dir, "src/index.ts"), "const x = 1;\n");
	...
	it("handles multiple modules with source maps in scan mode", ({ expect }) => {
		...
		const main = makeModule("a.js", path.join(dir, "a.js"), `//# sourceMappingURL=a.js.map`);
		const extra = makeModule("b.js", path.join(dir, "b.js"), `//# sourceMappingURL=b.js.map`);
		const bundle: SourceMapBundle = {};
...
describe("tryAttachSourcemapToModule", () => {
	...
	it("throws when sourceMappingURL points to missing file", ({ expect }) => {
		...
		expect(() => tryAttachSourcemapToModule(module)).toThrow("Invalid source map path");
	});
});
