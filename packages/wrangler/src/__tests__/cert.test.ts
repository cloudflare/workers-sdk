		...
		describe("api", () => {
			...
			describe("uploadCaCertificateFromFs", () => {
				it("should fail to read ca cert when file is missing", async () => {
					await expect(
						uploadCaCertificateFromFs(accountId, {
				...
				it("should read ca cert from disk and call mtls_certificates upload endpoint", async () => {
					...
					const cert = await uploadCaCertificateFromFs(accountId, {
						certificates: "caCert.pem",
						ca: true,
						name: "my_cert",
					});
			...
			describe("help", () => {
				it("should show the correct help text", async () => {
					...
					expect(std.out).toMatchInlineSnapshot(`
						...
						COMMANDS
						  wrangler cert upload  upload a new cert [mtls-certificate|certificate-authority]
...
Expires on ${oneYearLater.toLocaleDateString()}`
				...
				test("uploads ca certificate chain from file", async () => {
					...
					await runWrangler(
						"cert upload certificate-authority --ca-cert cert.pem"
					);
...
Expires on ${oneYearLater.toLocaleDateString()}`
				...
				test("uploads ca certificate chain from file with name", async () => {
					...
					await runWrangler(
						"cert upload certificate-authority --ca-cert cert.pem --name my-caCert"
					);
