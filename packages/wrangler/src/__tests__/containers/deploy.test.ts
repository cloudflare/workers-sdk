...
describe("wrangler deploy with containers", () => {
	...
	it("should preserve deployed managed containers when local configuration is empty and rollout is disabled", async ({
		expect,
	}) => {
		const deployedImage =
			"registry.cloudflare.com/some-account-id/tools@sha256:cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc";
						...
						createFetchResult([
							{
								...
								type: "json",
								json: JSON.stringify({
									ExampleDurableObject: { app: deployedImage },
								}),
							},
						])
					)
