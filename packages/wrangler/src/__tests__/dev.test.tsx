...
describe("wrangler dev", () => {
	...
	describe("usage-model", () => {
		it("should read wrangler.toml's usage_model", async () => {
			...
			fs.writeFileSync("index.js", `export default {};`);
			await runWrangler("dev");
			expect((Dev as vi.Mock).mock.calls[0][0].usageModel).toEqual("unbound");
		});
	...
	describe("entry-points", () => {
		...
		it("should use `main` from the top-level environment", async () => {
			...
			fs.writeFileSync("index.js", `export default {};`);
			await runWrangler("dev");
			expect((Dev as vi.Mock).mock.calls[0][0].entry.file).toMatch(
				/index\.js$/
			);
			expect(std.out).toMatchInlineSnapshot(`""`);
			expect(std.warn).toMatchInlineSnapshot(`""`);
	...
	describe("routes", () => {
		it("should pass routes to <Dev/>", async () => {
			...
			await runWrangler("dev");
			expect((Dev as vi.Mock).mock.calls[0][0]).toEqual(
	...
	describe("host", () => {
		...
		it("should read wrangler.toml's dev.host", async () => {
			...
			mockGetZones("some-host.com", [{ id: "some-zone-id" }]);
			await runWrangler("dev");
			expect((Dev as vi.Mock).mock.calls[0][0].host).toEqual("some-host.com");
		});
		...
		it("should not try to resolve a zone when starting in local mode", async () => {
			...
			expect((Dev as vi.Mock).mock.calls[0][0].zone).toEqual(undefined);
		});
	});
	...
	describe("local upstream", () => {
		it("should use dev.host from toml by default", async () => {
			...
			fs.writeFileSync("index.js", `export default {};`);
			await runWrangler("dev --local");
			expect((Dev as vi.Mock).mock.calls[0][0].localUpstream).toEqual(
				"2.some-host.com"
			);
		});
		...
		it("should use route from toml by default", async () => {
			...
			fs.writeFileSync("index.js", `export default {};`);
			await runWrangler("dev --local");
			expect((Dev as vi.Mock).mock.calls[0][0].host).toEqual("4.some-host.com");
		});
	...
	describe("custom builds", () => {
		it("should run a custom build before starting `dev`", async () => {
			...
			expect((Dev as vi.Mock).mock.calls[0][0].build).toEqual({
				command:
	...
	describe("upstream-protocol", () => {
		it("should default upstream-protocol to `https`", async () => {
			...
			fs.writeFileSync("index.js", `export default {};`);
			await runWrangler("dev");
			expect((Dev as vi.Mock).mock.calls[0][0].upstreamProtocol).toEqual(
				"https"
			);
			expect(std.out).toMatchInlineSnapshot(`""`);
			expect(std.warn).toMatchInlineSnapshot(`""`);
	...
	describe("local-protocol", () => {
		it("should default local-protocol to `http`", async () => {
			...
			fs.writeFileSync("index.js", `export default {};`);
			await runWrangler("dev");
			expect((Dev as vi.Mock).mock.calls[0][0].localProtocol).toEqual("http");
			expect(std.out).toMatchInlineSnapshot(`""`);
			expect(std.warn).toMatchInlineSnapshot(`""`);
			expect(std.err).toMatchInlineSnapshot(`""`);
		});
		...
		it("should use `local_protocol` from `wrangler.toml`, if available", async () => {
			...
			fs.writeFileSync("index.js", `export default {};`);
			await runWrangler("dev");
			expect((Dev as vi.Mock).mock.calls[0][0].localProtocol).toEqual("https");
			expect(std.out).toMatchInlineSnapshot(`""`);
			expect(std.warn).toMatchInlineSnapshot(`""`);
			expect(std.err).toMatchInlineSnapshot(`""`);
		});
	...
	describe("ip", () => {
		it("should default ip to 0.0.0.0", async () => {
			...
			fs.writeFileSync("index.js", `export default {};`);
			await runWrangler("dev");
			expect((Dev as vi.Mock).mock.calls[0][0].initialIp).toEqual("0.0.0.0");
			expect(std.out).toMatchInlineSnapshot(`""`);
			expect(std.warn).toMatchInlineSnapshot(`""`);
			expect(std.err).toMatchInlineSnapshot(`""`);
		});
		...
		it("should use to `ip` from `wrangler.toml`, if available", async () => {
			...
			fs.writeFileSync("index.js", `export default {};`);
			await runWrangler("dev");
			expect((Dev as vi.Mock).mock.calls[0][0].initialIp).toEqual("1.2.3.4");
			expect(std.out).toMatchInlineSnapshot(`""`);
			expect(std.warn).toMatchInlineSnapshot(`""`);
			expect(std.err).toMatchInlineSnapshot(`""`);
		});
		...
		it("should use --ip command line arg, if provided", async () => {
			...
			expect((Dev as vi.Mock).mock.calls[0][0].initialIp).toEqual("5.6.7.8");
			expect(std.out).toMatchInlineSnapshot(`""`);
			expect(std.warn).toMatchInlineSnapshot(`""`);
			expect(std.err).toMatchInlineSnapshot(`""`);
		});
	...
	describe("inspector port", () => {
		it("should connect WebSocket server with --experimental-local", async () => {
			...
			expect((Dev as vi.Mock).mock.calls[0][0].inspectorPort).toEqual(9229);
		...
		it("should read --inspector-port", async () => {
			...
			expect((Dev as vi.Mock).mock.calls[0][0].inspectorPort).toEqual(9999);
			expect(std).toMatchInlineSnapshot(`
			Object {
			  "debug": "",
			  "err": "",
	...
	describe("port", () => {
		it("should default port to 8787 if it is not in use", async () => {
			...
			fs.writeFileSync("index.js", `export default {};`);
			await runWrangler("dev");
			expect((Dev as vi.Mock).mock.calls[0][0].initialPort).toEqual(8787);
			expect(std.out).toMatchInlineSnapshot(`""`);
			expect(std.warn).toMatchInlineSnapshot(`""`);
			expect(std.err).toMatchInlineSnapshot(`""`);
		});
		...
		it("should use `port` from `wrangler.toml`, if available", async () => {
			...
			(getPort as vi.Mock).mockResolvedValue(98765);

			await runWrangler("dev");
			expect((Dev as vi.Mock).mock.calls[0][0].initialPort).toEqual(8888);
			expect(std.out).toMatchInlineSnapshot(`""`);
			expect(std.warn).toMatchInlineSnapshot(`""`);
			expect(std.err).toMatchInlineSnapshot(`""`);
		});
		...
		it("should use --port command line arg, if provided", async () => {
			...
			expect((Dev as vi.Mock).mock.calls[0][0].initialPort).toEqual(9999);
			expect(std.out).toMatchInlineSnapshot(`""`);
			expect(std.warn).toMatchInlineSnapshot(`""`);
			expect(std.err).toMatchInlineSnapshot(`""`);
		});
		...
		it("should use a different port to the default if it is in use", async () => {
			...
			await runWrangler("dev");
			expect((Dev as vi.Mock).mock.calls[0][0].initialPort).toEqual(98765);
			expect(std.out).toMatchInlineSnapshot(`""`);
			expect(std.warn).toMatchInlineSnapshot(`""`);
			expect(std.err).toMatchInlineSnapshot(`""`);
		});
	...
	describe(".dev.vars", () => {
      ...
      `;
			...
			await runWrangler("dev");
			const varBindings: Record<string, unknown> = (Dev as vi.Mock).mock
	...
	describe("serve static assets", () => {
		...
		it("should indicate whether Sites is being used", async () => {
			...
			await runWrangler("dev");
			expect((Dev as vi.Mock).mock.calls[0][0].isWorkersSite).toEqual(false);

			await runWrangler("dev --site abc");
			expect((Dev as vi.Mock).mock.calls[1][0].isWorkersSite).toEqual(true);

			await runWrangler("dev --assets abc");
			expect((Dev as vi.Mock).mock.calls[2][0].isWorkersSite).toEqual(false);
		});
	...
	describe("--inspect", () => {
		...
		it("should default to true, without a warning", async () => {
			fs.writeFileSync("index.js", `export default {};`);
			await runWrangler("dev index.js");
			expect((Dev as vi.Mock).mock.calls[0][0].inspect).toEqual(true);
			expect(std).toMatchInlineSnapshot(`
			Object {
			  "debug": "",
			  "err": "",
		...
		it("should pass false, without a warning", async () => {
			...
			expect((Dev as vi.Mock).mock.calls[0][0].inspect).toEqual(false);
			expect(std).toMatchInlineSnapshot(`
			Object {
			  "debug": "",
			  "err": "",
