...
import { getRegistryPath, UserError } from "@cloudflare/workers-utils";
import {
	readWranglerConfig,
	runInTempDir,
	writeWranglerConfig,
...
describe("flagship", () => {
	...
	describe("flags pull", () => {
		...
		it("rejects local access while the project's dev session is running", async ({
			expect,
		}) => {
			...
			await fs.writeFile(nodePath.join(registryPath, workerName), "{}");

			await expect(
				usingLocalFlagshipAPI(undefined, config, "app-1", async () => undefined)
			).rejects.toThrow(
				`Cannot use local Flagship commands while a dev session for '${workerName}' is running. Stop the dev session and retry.`
			);
		});
