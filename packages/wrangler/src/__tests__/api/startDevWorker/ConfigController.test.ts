...
describe("ConfigController", () => {
	...
	describe("dev.zone derivation (used for the outbound CF-Worker header)", () => {
		it("uses dev.host when set", async ({ expect }) => {
			await seed({
			...
			await expect(event).resolves.toMatchObject({
				type: "configUpdate",
				config: { dev: { zone: "explicit.example.com" } },
			});
		});

		it("derives from the first route's `zone_name` when dev.host is not set", async ({
			expect,
		}) => {
			await seed({
				"src/index.ts": "export default {};",
			...
			// the CF-Worker header is the zone name, not the route hostname.
			await expect(event).resolves.toMatchObject({
				type: "configUpdate",
				config: { dev: { zone: "example.com" } },
			});
		});

		it("falls back to the route pattern's hostname when `zone_name` is absent", async ({
			expect,
		}) => {
			await seed({
				"src/index.ts": "export default {};",
			...
			await expect(event).resolves.toMatchObject({
				type: "configUpdate",
				config: { dev: { zone: "foo.example.com" } },
			});
		});
	});
