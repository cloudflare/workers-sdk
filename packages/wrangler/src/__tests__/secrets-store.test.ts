...
describe("secrets-store store commands", () => {
	...
	describe("secrets-store store create", () => {
		it("creates a store", async () => {
			...
			await runWrangler(
				"secrets-store store create --name test-store --remote"
			);
...
└─────────────┴──────────────────────────────────┴──────────────────────────────────┴─────────────────┴─────────┴─────────┴──────────────────────┴──────────────────────┘"
	...
	describe("secrets-store secret list", () => {
		it("lists secrets", async () => {
			mockSecretList();
			await runWrangler(
				"secrets-store secret list --store-id 850e0805c1084551bb46d150b5dfe414 --remote"
			);
...
❌ List request returned no secrets."
	...
	describe("secrets-store secret get", () => {
		it("gets a secret", async () => {
			mockSecretGet();
			await runWrangler(
				"secrets-store secret get --store-id 850e0805c1084551bb46d150b5dfe414 --secret-id df3f6eb1159a4f10ac5fe836e2b8169c --remote"
			);
...
└────────┴──────────────────────────────────┴──────────────────────────────────┴─────────────────────────────┴─────────┴────────┴───────────────────────┴───────────────────────┘"`);
		...
		it("errors in getting a secret when no secret-id passed", async () => {
			...
			try {
				await runWrangler(
					"secrets-store secret get --store-id 850e0805c1084551bb46d150b5dfe414 --remote"
				);
			} catch (e) {
				err = e as Error;
			}
	...
	describe("secrets-store secret delete", () => {
		it("deletes a secret", async () => {
			mockSecretDelete();
			await runWrangler(
				"secrets-store secret delete --store-id 850e0805c1084551bb46d150b5dfe414 --secret-id df3f6eb1159a4f10ac5fe836e2b8169c --remote"
			);
...
✅ Secrets Store: Deleted secret! (ID: df3f6eb1159a4f10ac5fe836e2b8169c)"
      `);
		...
		it("errors in deleting a secret when no secret-id passed", async () => {
			...
			try {
				await runWrangler(
					"secrets-store secret delete --store-id 850e0805c1084551bb46d150b5dfe414 --remote"
				);
			} catch (e) {
				err = e as Error;
			}
