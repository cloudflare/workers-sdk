...
describe("secrets-store store commands", () => {
	...
	describe("secrets-store store create", () => {
		it("creates a store", async () => {
			...
			await runWrangler("secrets-store store create --name test-store");
      ...
      `);
			...
			expect(std.out).toMatchInlineSnapshot(`
        "🔐 Secrets Store: Creating store... (Name: test-store)
✅ Secrets Store: Created store! (Name: test-store, ID: 8b9199cad1954bc39add51c948767679)"
      `);
		});
		...
		it("errors in creating a store when no name passed", async () => {
			...
			try {
				await runWrangler("secrets-store store create");
			} catch (e) {
				err = e as Error;
			}
	...
	describe("secrets-store store list", () => {
		it("lists stores", async () => {
			mockStoreList();
			await runWrangler("secrets-store store list");

			expect(std.out).toMatchInlineSnapshot(`
        "🔐 Secrets Store: Listing stores...
┌─────────────┬──────────────────────────────────┬──────────────────────────────────┬──────────────────────┬──────────────────────┐
...
describe("secrets-store secret commands", () => {
	...
	describe("secrets-store secret create", () => {
      ...
      `);
			...
			expect(std.out).toMatchInlineSnapshot(`
        "🔐 Secrets Store: Creating secret... (Name: TEST_SECRET, Value: REDACTED, Scopes: workers, Comment: wrangler secret)
✅ Secrets Store: Created secret! (ID: 36dabbe4d01c49de82847b9a22673cbd)
┌─────────────┬──────────────────────────────────┬──────────────────────────────────┬─────────────────┬─────────┬─────────┬──────────────────────┬──────────────────────┐
...
└─────────────┴──────────────────────────────────┴──────────────────────────────────┴─────────────────┴─────────┴─────────┴──────────────────────┴──────────────────────┘"
	...
	describe("secrets-store secret list", () => {
		it("lists secrets", async () => {
			mockSecretList();
			await runWrangler(
				"secrets-store secret list --store-id 850e0805c1084551bb46d150b5dfe414"
			);

			expect(std.out).toMatchInlineSnapshot(`
        "🔐 Secrets Store: Listing secrets... (store-id: 850e0805c1084551bb46d150b5dfe414, page: 1, per-page: 10)
┌────────────┬──────────────────────────────────┬─────────────────────────────────┬─────────┬────────┬───────────────────────┬───────────────────────┐
...
└────────────┴──────────────────────────────────┴─────────────────────────────────┴─────────┴────────┴───────────────────────┴───────────────────────┘"`);
		...
		it("handles empty response of secrets", async () => {
			...
			expect(std.out)
				.toMatchInlineSnapshot(`"🔐 Secrets Store: Listing secrets... (store-id: 850e0805c1084551bb46d150b5dfe414, page: 1, per-page: 10)
❌ List request returned no secrets."
      `);
		...
		it("errors in listing secrets when no store-id passed", async () => {
			let err: undefined | Error;
			try {
				await runWrangler("secrets-store secret list");
			} catch (e) {
				err = e as Error;
			}
			expect(err?.message).toMatchInlineSnapshot(`
	...
	describe("secrets-store secret get", () => {
		it("gets a secret", async () => {
			mockSecretGet();
			await runWrangler(
				"secrets-store secret get --store-id 850e0805c1084551bb46d150b5dfe414 --secret-id df3f6eb1159a4f10ac5fe836e2b8169c"
			);

			expect(std.out).toMatchInlineSnapshot(`
        "🔐 Secrets Store: Getting secret... (ID: df3f6eb1159a4f10ac5fe836e2b8169c)
┌────────┬──────────────────────────────────┬──────────────────────────────────┬─────────────────────────────┬─────────┬────────┬───────────────────────┬───────────────────────┐
│ Name   │ ID                               │ StoreID                          │ Comment                     │ Scopes  │ Status │ Created               │ Modified              │
├────────┼──────────────────────────────────┼──────────────────────────────────┼─────────────────────────────┼─────────┼────────┼───────────────────────┼───────────────────────┤
...
└────────┴──────────────────────────────────┴──────────────────────────────────┴─────────────────────────────┴─────────┴────────┴───────────────────────┴───────────────────────┘"`);
		...
		it("errors in getting a secret when no store-id passed", async () => {
			...
			try {
				await runWrangler(
					"secrets-store secret get --secret-id df3f6eb1159a4f10ac5fe836e2b8169c"
				);
			} catch (e) {
				err = e as Error;
			}
		...
		it("errors in getting a secret when no secret-id passed", async () => {
			...
			try {
				await runWrangler(
					"secrets-store secret get --store-id 850e0805c1084551bb46d150b5dfe414"
				);
			} catch (e) {
				err = e as Error;
			}
	...
	describe("secrets-store secret delete", () => {
		it("deletes a secret", async () => {
			mockSecretDelete();
			await runWrangler(
				"secrets-store secret delete --store-id 850e0805c1084551bb46d150b5dfe414 --secret-id df3f6eb1159a4f10ac5fe836e2b8169c"
			);

			expect(std.out).toMatchInlineSnapshot(`
        "🔐 Secrets Store: Deleting secret... (ID: df3f6eb1159a4f10ac5fe836e2b8169c)
✅ Secrets Store: Deleted secret! (ID: df3f6eb1159a4f10ac5fe836e2b8169c)"
      `);
		});
		...
		it("errors in deleting a secret when no store-id passed", async () => {
			...
			try {
				await runWrangler(
					"secrets-store secret delete --secret-id df3f6eb1159a4f10ac5fe836e2b8169c"
				);
			} catch (e) {
				err = e as Error;
			}
		...
		it("errors in deleting a secret when no secret-id passed", async () => {
			...
			try {
				await runWrangler(
					"secrets-store secret delete --store-id 850e0805c1084551bb46d150b5dfe414"
				);
			} catch (e) {
				err = e as Error;
			}
	...
	describe("secrets-store secret update", () => {
      ...
      `);
			...
			expect(std.out).toMatchInlineSnapshot(`
        "🔐 Secrets Store: Updating secret... (ID: df3f6eb1159a4f10ac5fe836e2b8169c)
✅ Secrets Store: Updated secret! (ID: 36dabbe4d01c49de82847b9a22673cbd)
┌────────┬──────────────────────────────────┬──────────────────────────────────┬────────────────────────┬─────────┬─────────┬──────────────────────┬──────────────────────┐
│ Name   │ ID                               │ StoreID                          │ Comment                │ Scopes  │ Status  │ Created              │ Modified             │
├────────┼──────────────────────────────────┼──────────────────────────────────┼────────────────────────┼─────────┼─────────┼──────────────────────┼──────────────────────┤
...
└────────┴──────────────────────────────────┴──────────────────────────────────┴────────────────────────┴─────────┴─────────┴──────────────────────┴──────────────────────┘"`);
		...
		it("errors in updating a secret when no params to update are passed", async () => {
			...
			expect(std.out).toMatchInlineSnapshot(
				`"❌ No new values passed to update on secret."`
			);
		});
	});
	...
	describe("secrets-store secret duplicate", () => {
      ...
      `);
			...
			expect(std.out).toMatchInlineSnapshot(`
        "🔐 Secrets Store: Duplicating secret... (ID: df3f6eb1159a4f10ac5fe836e2b8169c)
✅ Secrets Store: Duplicated secret! (ID: 36dabbe4d01c49de82847b9a22673cbd)
┌────────┬──────────────────────────────────┬──────────────────────────────────┬────────────────────────┬─────────┬─────────┬──────────────────────┬──────────────────────┐
│ Name   │ ID                               │ StoreID                          │ Comment                │ Scopes  │ Status  │ Created              │ Modified             │
├────────┼──────────────────────────────────┼──────────────────────────────────┼────────────────────────┼─────────┼─────────┼──────────────────────┼──────────────────────┤
│ DB_KEY │ 36dabbe4d01c49de82847b9a22673cbd │ 850e0805c1084551bb46d150b5dfe414 │ wrangler secret update │ workers │ pending │ 3/5/2025, 9:56:40 PM │ 3/5/2025, 9:56:40 PM │
