...
export async function runCustomBuild(
  ...
  if (build?.command) {
    ...
    if (fileExists(expectedEntry) === false) {
      const relativePath = path.relative(process.cwd(), expectedEntry) || ".";
      let message = `The expected output file at "${relativePath}" was not found after running custom build: ${build.command}.`;
      if (existsSync(expectedEntry) && statSync(expectedEntry).isDirectory()) {
        ...
        for (const filePath of [
          ...
          if (fileExists(path.resolve(expectedEntry, filePath))) {
            message +=
              `\nDid you mean to set the main field to:\n` +
              "```\n" +
              `main = "./${path.join(relativePath, filePath)}"\n` +
              "```";
          }
        }
      }
