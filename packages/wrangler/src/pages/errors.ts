...
Routes must have at least 1 include rule, but no include rules were detected.`;
		case RoutesValidationError.TOO_MANY_RULES:
			return `Invalid _routes.json file found at: ${routesPath}.
Rules in _routes.json are over the ${MAX_FUNCTIONS_ROUTES_RULES} rule limit. Please make sure you have a total of ${MAX_FUNCTIONS_ROUTES_RULES} include and exclude rules combined.`;
		case RoutesValidationError.RULE_TOO_LONG:
			return `Invalid _routes.json file found at: ${routesPath}.
Rules in _routes.json that are over the ${MAX_FUNCTIONS_ROUTES_RULE_LENGTH} character limit. Please make sure that each include and exclude routing rule is at most ${MAX_FUNCTIONS_ROUTES_RULE_LENGTH} characters long.`;
