		...
		if (!isInteractive()) {
			if (!deploymentId) {
				throw new FatalError(
					...
					{ code: 1, telemetryMessage: "pages deployment tail missing deployment" }
				);
			}
			...
			if (!projectName) {
				throw new FatalError(
					...
					{ code: 1, telemetryMessage: "pages deployment tail missing project name" }
				);
			}
		}
		...
		if (isUrl(deployment)) {
			...
			if (envDeployments.length === 0) {
				throw new FatalError(
					...
					{ code: 1, telemetryMessage: "pages deployment tail no deployments" }
				);
			}
