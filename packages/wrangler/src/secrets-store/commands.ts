...
export const secretsStoreStoreCreateCommand = createCommand({
	...
	async handler(args, _ctx) {
		...
		const store = await createStore(config, { name: args.name });
		logger.log(
...
export const secretsStoreStoreListCommand = createCommand({
	...
	async handler(args, _ctx) {
		...
		const stores = await listStores(config, urlParams);
...
export const secretsStoreSecretListCommand = createCommand({
	...
	async handler(args, _ctx) {
		...
		const secrets = await listSecrets(config, args.storeId, urlParams);
...
export const secretsStoreSecretGetCommand = createCommand({
	...
	async handler(args, _ctx) {
		...
		const secret = await getSecret(config, args.storeId, args.secretId);
...
export const secretsStoreSecretCreateCommand = createCommand({
	...
	async handler(args, _ctx) {
		...
		const secrets = await createSecret(config, args.storeId, {
			name: args.name,
			value: args.value,
...
export const secretsStoreSecretUpdateCommand = createCommand({
	...
	async handler(args, _ctx) {
		...
		const secret = await updateSecret(config, args.storeId, args.secretId, {
...
export const secretsStoreSecretDuplicateCommand = createCommand({
	...
	async handler(args, _ctx) {
		...
		const duplicatedSecret = await duplicateSecret(
			config,
			args.storeId,
			args.secretId,
			{
