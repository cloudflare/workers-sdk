...
export const ApplyHandler = withConfig<
	StrictYargsOptionsToInterface<typeof ApplyOptions>
...
Your database may not be available to serve requests during the migration, continue?`
	...
	if (!local) {
		assert(databaseInfo, "In non-local mode `databaseInfo` should be defined.");
	...
	for (const migration of unappliedMigrations) {
		let query = fs.readFileSync(`${migrationsPath}/${migration.Name}`, "utf8");
		query += `
		...
		render(
			<Box flexDirection="column">
				<Table data={unappliedMigrations} columns={["Name", "Status"]}></Table>
				{errorNotes.length > 0 && (
					<Box flexDirection="column">
						<Text>&nbsp;</Text>
						<Text>
