...
function loadSourceMap(
	...
	const map = JSON.parse(
		fs.readFileSync(sourceMapPath).toString()
	) as SourceMapV3;
	...
	const sourceRootPath = path.dirname(path.join(entryDirectory, sourceMapPath));
	map.sourceRoot = stripPrefix(sourceRootPath, map.sourceRoot);
	map.sources = map.sources.map((source) => {
		...
		return stripPrefix(withTrailingSep(entryDirectory), originalPath);
	});
	return [
		{
			name: name + ".map",
...
function scanSourceMaps(modules: CfModule[]): CfWorkerSourceMap[] {
	...
	for (const module of modules) {
		...
		if (commentPath.startsWith("data:")) {
			throw new Error(
				`Invalid source map path in ${module.filePath}: expected file path but found data URL.`
			);
		}
		...
		const map = JSON.parse(
			fs.readFileSync(wranglerPath).toString()
		) as SourceMapV3;
...
function cleanPathPrefix(filePath: string): string {
	return stripPrefix("..", stripPrefix("." + path.sep, filePath));
}
