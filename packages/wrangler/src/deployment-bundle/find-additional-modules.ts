...
function getPythonVendorModulesSize(modules: CfModule[]): number {
	const vendorModules = modules.filter((m) => m.name.startsWith("python_modules/"));
...
export async function findAdditionalModules(
	...
	if (isPythonEntrypoint) {
		...
		const pythonModulesDirInModuleRoot = path.resolve(entry.moduleRoot, "python_modules");

		// Check for conflict between a `python_modules` directory in the module root and the project root.
		const pythonModulesExistsInModuleRoot = existsSync(pythonModulesDirInModuleRoot);
		if (pythonModulesExistsInModuleRoot) {
			throw new UserError(
				"The 'python_modules' directory is used during bundling and so cannot exist in your module root. Delete it to continue."
			);
		}
