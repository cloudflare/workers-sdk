...
/**
 * Mask vars that were overridden in .dev.vars for the flat bindings format.
 * This prevents logging potential secrets into the terminal.
 */
export function maskVars(
	...
	const maskedBindings: NonNullable<StartDevWorkerInput["bindings"]> = {};
	...
	for (const [key, binding] of Object.entries(bindings)) {
		if (binding.type === "plain_text") {
			...
			// Non-var bindings pass through unchanged
			maskedBindings[key] = binding;
		}
	}
...
/**
 * Get the bindings for a Cloudflare Worker in the StartDevWorkerInput format.
 *
 * This converts config bindings to the flat `Record<string, Binding>` format
 * with dev-specific behavior: using preview IDs, validating preview configs,
 * loading `.dev.vars`, and handling Hyperdrive env vars.
 *
 * @param configParam The configuration for the worker.
 ...
 * @param inputBindings Additional bindings to merge on top of config bindings (overrides by name).
 * @returns The bindings for the Cloudflare Worker as a Record<string, Binding>.
 */
export function getBindings(
	...
	// Dev-specific validations for preview configs
	validatePreviewBindings(configParam, local);

	// Apply Hyperdrive env vars to config (mutates config)
	applyHyperdriveEnvVars(configParam, local);

	// Core conversion with preview IDs
...
/**
 * Validate preview bindings configuration for dev mode.
...
function validatePreviewBindings(config: Config, local: boolean): void {
	// KV: throw if no preview_id in remote mode
	for (const kv of config.kv_namespaces ?? []) {
		if (!kv.preview_id && !local) {
			throw new UserError(
				`In development, you should use a separate kv namespace than the one you'd use in production. Please create a new kv namespace with "wrangler kv namespace create <name> --preview" and add its id as preview_id to the kv_namespace "${kv.binding}" in your ${configFileName(config.configPath)} file`,
				{
	...
	// R2: throw if no preview_bucket_name in remote mode
	for (const r2 of config.r2_buckets ?? []) {
		if (!r2.preview_bucket_name && !local) {
			throw new UserError(
				`In development, you should use a separate r2 bucket than the one you'd use in production. Please create a new r2 bucket with "wrangler r2 bucket create <name>" and add its name as preview_bucket_name to the r2_buckets "${r2.binding}" in your ${configFileName(config.configPath)} file`,
				{
	...
	// D1: warn if no preview_database_id
	for (const d1 of config.d1_databases ?? []) {
		if (!local && !d1.preview_database_id && !process.env.NO_D1_WARNING) {
			logger.log(
				`--------------------\n💡 Recommendation: for development, use a preview D1 database rather than the one you'd use in production.\n💡 Create a new D1 database with "wrangler d1 create <name>" and add its id as preview_database_id to the d1_database "${d1.binding}" in your ${configFileName(config.configPath)} file\n--------------------\n`
			);
		}
	}
}
