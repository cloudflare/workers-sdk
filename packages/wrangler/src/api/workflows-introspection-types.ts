/**
 * Vendored copy of the Workflow introspection types from
 * `@cloudflare/workflows-shared/src/types`.
 *
 * workflows-shared ships no built `.d.ts` for its `/src/*` subpaths, so pulling
 * these types in via the source import forces rolldown-plugin-dts to recompile
 * workflows-shared source inside wrangler's declaration build — which emits an
 * empty module and breaks the bundled `.d.ts`. Vendoring the (dependency-free)
 * type surface keeps wrangler's public test-harness types self-contained.
 *
 * `workflows-introspection-types.compat.ts` guards against drift from the
