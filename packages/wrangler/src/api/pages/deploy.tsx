...
export async function deploy({
	...
	if (configPath) {
		...
		// if config does not specify `pages_build_output_dir`, then ignore it
	...
	if (config !== undefined && config.configPath !== undefined) {
		const configHash = createHash("sha256")
			.update(await readFile(config.configPath))
			.digest("hex");

		formData.append("wrangler-config-hash", configHash);
		formData.append("pages_build_output_dir", config.pages_build_output_dir);
	}
