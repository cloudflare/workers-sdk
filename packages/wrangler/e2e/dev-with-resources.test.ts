...
const RUNTIMES = [
	// { runtime: "local" },
	// { runtime: "local", experimentalDevenvRuntime: true },
	// { runtime: "remote" },
	{ runtime: "remote", experimentalDevenvRuntime: true },
];

function convertRuntimeOptionsToFlags({
	runtime,
	...
	return [
		...
		experimentalDevenvRuntime
			? "--experimental-devenv-runtime"
			: "--no-experimental-devenv-runtime",
	]
		.filter(Boolean)
		.join(" ")
		.trim();
...
const OPTIONS = RUNTIMES.map(({ runtime, experimentalDevenvRuntime }) => {
	const isLocal = runtime === "local";
	const runtimeFlags = convertRuntimeOptionsToFlags({
		runtime,
		experimentalDevenvRuntime,
	});
...
describe.each(OPTIONS)("Core: $runtimeFlags", ({ isLocal, runtimeFlags }) => {
	e2eTest(
...
describe.each(OPTIONS)(
	"Bindings: $runtimeFlags",
	({ isLocal, runtimeFlags, resourceFlags }) => {
		e2eTest(
...
describe.each(OPTIONS)("Multi-Worker Bindings: $runtimeFlags", (_) => {
