...
import { ENV_HEADER_COMMENT_PREFIX } from "../src/type-generation/helpers";
...
describe("types", () => {
	...
	it("should be able to generate an Env type only", async () => {
		...
		expect(file).toMatchInlineSnapshot(`
			"/* eslint-disable */
			${ENV_HEADER_COMMENT_PREFIX} \`wrangler types --include-runtime=false\` (hash: 5c82572f95137bbfb775d02fdf441070)
	...
	it("should include header with version information in the generated types", async () => {
		...
		expect(lines[1]).toMatchInlineSnapshot(
			`"${ENV_HEADER_COMMENT_PREFIX} \`wrangler types ./types.d.ts\` (hash: 5c82572f95137bbfb775d02fdf441070)"`
		);
	...
	it("should include header with wrangler command that generated it", async () => {
		...
		expect(lines[1]).toMatchInlineSnapshot(
			`"${ENV_HEADER_COMMENT_PREFIX} \`wrangler types -c wranglerA.toml --env-interface MyCloudflareEnv ./cflare-env.d.ts\` (hash: e981fccb455c04c58ced00f3442b83ac)"`
		);
		expect(lines[2]).match(
			/\/\/ Runtime types generated with workerd@1\.\d{8}\.\d \d{4}-\d{2}-\d{2} ([a-z_]+,?)*/
		);
	...
	it("should read .env files for secret env vars", async () => {
		...
		expect(file).toMatchInlineSnapshot(`
			"/* eslint-disable */
			${ENV_HEADER_COMMENT_PREFIX} \`wrangler types --include-runtime=false\` (hash: 1457dc49fa39d3fb92583c9c66b2dbe5)
			declare namespace Cloudflare {
				interface GlobalProps {
					mainModule: typeof import("./src/index");
				}
