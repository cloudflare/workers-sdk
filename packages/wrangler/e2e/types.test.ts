...
describe("types", () => {
	...
	it("should allow users to exclude the hard in the header with --header-hash=false", async () => {
		const helper = new WranglerE2ETestHelper();
		await helper.seed(seed);
		await helper.run('wrangler types "./types.d.ts" --header-hash=false');
		...
		expect(lines[1]).toMatchInlineSnapshot(
			`"// Generated by Wrangler by running \`wrangler types ./types.d.ts --header-hash=false\`"`
		);
		expect(lines[2]).match(
			/\/\/ Runtime types generated with workerd@1\.\d{8}\.\d \d{4}-\d{2}-\d{2} ([a-z_]+,?)*/
		);
