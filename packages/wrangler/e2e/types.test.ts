...
describe("types", () => {
	...
	it("should generate runtime types and env types in one file at the default path", async ({ expect }) => {
		const helper = new WranglerE2ETestHelper();
		await helper.seed(seed);
		const output = await helper.run(`wrangler types`);
	...
	it("should include header with version information in the generated types", async ({ expect }) => {
		const helper = new WranglerE2ETestHelper();
		await helper.seed(seed);
	...
	it("should include header with wrangler command that generated it", async ({ expect }) => {
		const helper = new WranglerE2ETestHelper();
		await helper.seed({
			...seed,
	...
	it("should not regenerate runtime types if the header matches, but should regenerate env types", async ({ expect }) => {
		const helper = new WranglerE2ETestHelper();
		await helper.seed(seed);
	...
	describe("--check", () => {
		...
		it("should error when runtime types are out of date", async ({ expect }) => {
			await helper.run(`wrangler types`);
		...
		it("should not error on `--check` if types generated with `--include-env=false`", async ({ expect }) => {
		...
		it("should not error on `--check` if types generated with `--include-runtime=false`", async ({ expect }) => {
		...
		it("should error if types file was not generated by wrangler", async ({ expect }) => {
			await writeFile(
				path.join(helper.tmpPath, "worker-configuration.d.ts"),
				"// Some other content\ninterface Foo {}"
			);
