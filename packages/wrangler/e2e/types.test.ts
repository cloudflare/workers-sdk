...
describe("types", () => {
	...
	it("should be able to generate an Env type only", async () => {
		...
		expect(file).toMatchInlineSnapshot(`
			"/* eslint-disable */
			// Generated by Wrangler by running \`wrangler types --include-runtime=false\` (hash: 40f25ef8f3807c3af318c6ca32de7eee)
			declare namespace CF {
				interface Env {
					MY_VAR: "my-var-value";
				}
			}
			interface Env extends CF.Env {}
			"
		`);
	});
	...
	it("should include header with version information in the generated types", async () => {
		...
		expect(lines[1]).toMatchInlineSnapshot(
			`"// Generated by Wrangler by running \`wrangler types ./types.d.ts\` (hash: 40f25ef8f3807c3af318c6ca32de7eee)"`
		);
	...
	it("should include header with wrangler command that generated it", async () => {
		...
		expect(lines[1]).toMatchInlineSnapshot(
			`"// Generated by Wrangler by running \`wrangler types -c wranglerA.toml --env-interface MyCloudflareEnv ./cflare-env.d.ts\` (hash: b67007acbe0400bc43ddc2fa86d2ce38)"`
		);
		expect(lines[2]).match(
			/\/\/ Runtime types generated with workerd@1\.\d{8}\.\d \d{4}-\d{2}-\d{2} ([a-z_]+,?)*/
		);
