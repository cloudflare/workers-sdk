...
describe("types", () => {
	...
	it("should read .env files for secret env vars", async () => {
		...
		expect(file).toContain("/* eslint-disable */");
		expect(file).toMatch(
			/\/\/ Generated by Wrangler by running .* \(hash: [a-f0-9]{32}\)/
		);
		expect(file).toContain("interface Env {");
		expect(file).toContain('MY_VAR: "my-var-value"');
		expect(file).toContain("MY_SECRET_VAR?: string");
		expect(file).toContain("interface Env extends Cloudflare.Env {}");
	});
	...
	it("should include declared secrets from config as optional Env properties", async () => {
		...
		expect(file).toContain("WEBHOOK_SECRET?: string");
		expect(file).toContain("GITHUB_PATS?: string");
		expect(file).toContain('MY_VAR: "my-var-value"');
	});
