...
describe.sequential("wrangler pages dev", () => {
	it("should warn if no [--compatibility_date] command line arg was specified", async ({ expect }) => {
	...
	it("should warn that [--experimental-local] is no longer required, if specified", async ({ expect }) => {
		const helper = new WranglerE2ETestHelper();
		const worker = helper.runLongLived(
	...
	it("should show [--service] related warnings if specified as arg in the command line", async ({ expect }) => {
		const helper = new WranglerE2ETestHelper();
		const worker = helper.runLongLived(
	...
	it("should warn if bindings specified as args in the command line are invalid", async ({ expect }) => {
		const helper = new WranglerE2ETestHelper();
		const worker = helper.runLongLived(
	...
	it("should use bindings specified as args in the command line", async ({ expect }) => {
		const helper = new WranglerE2ETestHelper();
		await helper.seed({
			"_worker.js": dedent`
					export default {
	...
	it("should recover from syntax error during dev session (_worker)", async ({ expect }) => {
		const helper = new WranglerE2ETestHelper();
		...
		await worker.readUntil(/Failed to build/, 5_000);
	...
	it("should recover from syntax error during dev session (Functions)", async ({ expect }) => {
		const helper = new WranglerE2ETestHelper();
		const worker = helper.runLongLived(
			`${cmd} --port ${port} --inspector-port ${inspectorPort} .`
		);
		...
		await worker.readUntil(/Failed to build Functions/, 5_000);
	...
	it("should validate _routes.json during dev session, and fallback to default value", async ({ expect }) => {
		const helper = new WranglerE2ETestHelper();
		await helper.seed({
	...
	it("should use top-level configuration specified in `wrangler.toml`", async ({ expect }) => {
		const helper = new WranglerE2ETestHelper();
		const worker = helper.runLongLived(
			`${cmd} --port ${port} --inspector-port ${inspectorPort}`
		);
	...
	it("should merge (with override) `wrangler.toml` configuration with configuration provided via the command line, with command line args taking precedence", async ({ expect }) => {
		const helper = new WranglerE2ETestHelper();
	...
	it("should pick up wrangler.toml configuration even in cases when `pages_build_output_dir` was not specified, but the <directory> command argument was", async ({ expect }) => {
		const helper = new WranglerE2ETestHelper();
	...
	it("should allow user to override CF_PAGES... environment variables", async ({ expect }) => {
		const helper = new WranglerE2ETestHelper();
		await helper.seed({
			"_worker.js": dedent`
				export default {
	...
	describe("watch mode", () => {
		it("should modify worker during dev session (Functions)", async ({ expect }) => {
			const helper = new WranglerE2ETestHelper();
		...
		it("should support modifying dependencies during dev session (Functions)", async ({ expect }) => {
			const helper = new WranglerE2ETestHelper();
		...
		it("should support modifying external modules during dev session (Functions)", async ({ expect }) => {
			const helper = new WranglerE2ETestHelper();
		...
		it("should modify worker during dev session (_worker)", async ({ expect }) => {
			const helper = new WranglerE2ETestHelper();
		...
		it("should support modifying dependencies during dev session (_worker)", async ({ expect }) => {
			const helper = new WranglerE2ETestHelper();
		...
		it("should support modifying external modules during dev session (_worker)", async ({ expect }) => {
			const helper = new WranglerE2ETestHelper();
		...
		it("should support modifying _routes.json during dev session", async ({ expect }) => {
			const helper = new WranglerE2ETestHelper();
