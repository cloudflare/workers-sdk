...
describe("cert", () => {
	...
	let mtlsCertName = `mtls_cert_${randomUUID()}`;
	let caCertName = `ca_cert_${randomUUID()}`;
	...
	it("upload mtls-certificate", async () => {
		...
		const output = await helper.run(`wrangler cert upload mtls-certificate --name ${mtlsCertName} --cert mtls_client_cert_file.pem --key mtls_client_private_key_file.pem`);
    ...
    it("upload certificate-authority", async () => {
		...
		const output = await helper.run(`wrangler cert upload certificate-authority --name ${caCertName} --ca-cert ca_chain_cert.pem`);
		expect(normalize(output.stdout)).toMatchInlineSnapshot(`
    ...
    it("list cert", async () => {
		...
		expect(result).toContain(`Name: mtls_cert_00000000-0000-0000-0000-000000000000`);
		expect(result).toContain(`Name: ca_cert_00000000-0000-0000-0000-000000000000`);
	});

    it("delete mtls cert", async () => {
		const delete_mtls_cert_output = await helper.run(`wrangler cert delete --name ${mtlsCertName}`);
	...
	it("delete ca chain cert", async() => {
		const delete_ca_cert_output = await helper.run(`wrangler cert delete --name ${caCertName}`);
