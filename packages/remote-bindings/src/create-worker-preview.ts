...
/** Identifies remote-bindings preview requests to the Cloudflare API. */
...
/**
 * A logger that may optionally sanitise secrets from debug output. Wrangler's
 * logger provides `debugWithSanitization`; the standalone package logger
 * (`console` / an OAuth-flow logger) does not, so callers without it fall back
 * to plain `debug`.
 */
type PreviewLogger = Logger & {
	debugWithSanitization?: (label: string, ...args: unknown[]) => void;
};

/**
 * Cross-cutting options shared by the preview API calls. Kept as a trailing
 * bag so wrangler's shim can inject its logger + interactivity predicate
 * without changing the historical positional call sites.
 */
export interface PreviewRequestOptions {
	/** Logger for debug output. Defaults to `console`. */
	logger?: PreviewLogger;
	/**
	 * Predicate consulted when the preview domain is behind Cloudflare Access
	 * and no service token is configured: `true` errors with an actionable
	 * message, `false` falls back to interactive `cloudflared` login. Defaults
	 * to `() => true` (non-interactive), which is correct for the standalone
	 * package; wrangler injects its real interactivity predicate so
	 * `wrangler dev` against an Access-protected preview domain can still log in
	 * interactively.
	 */
	isNonInteractiveOrCI?: () => boolean;
}

function debugWithSanitization(
	logger: PreviewLogger,
	label: string,
	...args: unknown[]
): void {
	if (logger.debugWithSanitization) {
		logger.debugWithSanitization(label, ...args);
	} else {
 ...
 * or error parsing.
 */
async function fetchResult<ResponseType>(
...
async function tryExpandToken(
	...
	try {
		...
		debugWithSanitization(logger, "RESPONSE:", bodyText);
...
export async function createPreviewSession(
	...
	const isNonInteractiveOrCI = options.isNonInteractiveOrCI ?? (() => true);
