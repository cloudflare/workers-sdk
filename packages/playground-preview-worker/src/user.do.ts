...
import { RealishPreviewConfig, UploadResult, doUpload, setupTokens } from './realish';
...
export class UserSession {
	...
	constructor(private state: DurableObjectState, private env: Env) {
		this.state.blockConcurrencyWhile(async () => {
			this.config = await this.state.storage.get<RealishPreviewConfig>('config');
	...
	async uploadWorker(name: string, worker: FormData) {
		worker.set('wrangler-session-config', JSON.stringify({ workers_dev: true }));
		...
		try {
			uploadResult = await doUpload(this.env.ACCOUNT_ID, this.env.API_TOKEN, this.config, name, worker);
		} catch {
		...
		const inspector = new URL(this.config.uploadConfigToken.inspector_websocket);
		inspector.searchParams.append('cf_workers_preview_token', this.config.uploadConfigToken.token);
	...
	async fetch(request: Request) {
		...
		if (request.headers.has('cf-run-user-worker')) {
			...
			return fetch(
				switchRemote(new URL(request.url), `https://${this.workerName}.s.workers.dev`),
				new Request(request, {
					headers: {
		...
		const metadata = {
			...
			compatibility_date: uploadedMetadata?.compatibility_date ?? `${year}-${month}-${date}`,
			compatibility_flags: uploadedMetadata?.compatibility_flags ?? ['nodejs_compat'],
		};

		const { entrypoint, additionalModules } = constructMiddleware(uploadedMetadata.main_module);
		...
		worker.set('metadata', new File([JSON.stringify(metadata)], 'metadata', { type: 'application/json' }));
