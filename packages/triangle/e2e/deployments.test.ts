...
describe("deployments", () => {
	...
	beforeAll(async () => {
		...
		const email = matchWhoamiEmail(
			(await runInRoot`$ ${TRIANGLE} whoami`).stdout
		);
		normalize = (str) =>
			normalizeOutput(str, {
	...
	it("list 2 deployments", async () => {
		const { stdout, stderr } =
			await runInWorker`$ ${TRIANGLE} deployments list`;
		expect(normalize(stdout)).toMatchInlineSnapshot(`
			"🚧\`triangle deployments\` is a beta command. Please report any issues to https://github.com/khulnasoft/workers-sdk/issues/new/choose
			Deployment ID: 00000000-0000-0000-0000-000000000000
			Created on:    TIMESTAMP
	...
	it("rollback", async () => {
		const { stdout, stderr } =
			await runInWorker`$ ${TRIANGLE} rollback --message "A test message"`;
		expect(normalize(stdout)).toMatchInlineSnapshot(`
