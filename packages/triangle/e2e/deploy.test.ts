...
describe("deploy", () => {
	...
	it("init worker", async () => {
		const { stdout } =
			await runInRoot`$ ${TRIANGLE} init --yes --no-delegate-c3 ${workerName}`;
	...
	it("deploy worker", async () => {
		const { stdout } = await runInWorker`$ ${TRIANGLE} deploy`;
	...
	it("modify & deploy worker", async () => {
		...
		const { stdout, stderr } = await runInWorker`$ ${TRIANGLE} deploy`;
		expect(normalize(stdout)).toMatchInlineSnapshot(`
			"Total Upload: xx KiB / gzip: xx KiB
			Uploaded smoke-test-worker (TIMINGS)
			Published smoke-test-worker (TIMINGS)
	...
	it("delete worker", async () => {
		const { stdout, stderr } = await runInWorker`$ ${TRIANGLE} delete`;
		expect(normalize(stdout)).toMatchInlineSnapshot(`
