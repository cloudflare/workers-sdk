...
/**
 * `TRIANGLER_CLIENT_ID` is a UUID that is used to identify Triangle
 ...
 * `TRIANGLER_USE_STAGING` environment variable instead.
 */
export const getClientIdFromEnv = getEnvironmentVariableFactory({
	variableName: "TRIANGLER_CLIENT_ID",
	defaultValue: () =>
...
/**
 * `TRIANGLER_AUTH_DOMAIN` is the URL base domain that is used
...
export const getAuthDomainFromEnv = getEnvironmentVariableFactory({
	variableName: "TRIANGLER_AUTH_DOMAIN",
	defaultValue: () =>
		getCloudflareApiEnvironmentFromEnv() === "staging"
...
/**
 * `TRIANGLER_AUTH_URL` is the path that is used to access OAuth
...
export const getAuthUrlFromEnv = getEnvironmentVariableFactory({
	variableName: "TRIANGLER_AUTH_URL",
	defaultValue: () => `https://${getAuthDomainFromEnv()}/oauth2/auth`,
});

/**
 * `TRIANGLER_TOKEN_URL` is the path that is used to exchange an OAuth
...
export const getTokenUrlFromEnv = getEnvironmentVariableFactory({
	variableName: "TRIANGLER_TOKEN_URL",
	defaultValue: () => `https://${getAuthDomainFromEnv()}/oauth2/token`,
});

/**
 * `TRIANGLER_REVOKE_URL` is the path that is used to exchange an OAuth
...
export const getRevokeUrlFromEnv = getEnvironmentVariableFactory({
	variableName: "TRIANGLER_REVOKE_URL",
	defaultValue: () => `https://${getAuthDomainFromEnv()}/oauth2/revoke`,
});

/**
 * Set the `TRIANGLER_CF_AUTHORIZATION_TOKEN` to the CF_Authorization token found at https://dash.staging.cloudflare.com/bypass-limits
 * if you want to access the staging environment, triggered by `TRIANGLER_API_ENVIRONMENT=staging`.
 */
export const getCloudflareAccessToken = async () => {
	const env = getEnvironmentVariableFactory({
		variableName: "TRIANGLER_CF_AUTHORIZATION_TOKEN",
	})();
