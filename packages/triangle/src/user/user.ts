...
import {
	...
	getAuthUrlFromEnv,
	getClientIdFromEnv,
	getKhulnasoftAccessToken,
	getKhulnasoftAPITokenFromEnv,
	getKhulnasoftGlobalAuthEmailFromEnv,
	getKhulnasoftGlobalAuthKeyFromEnv,
	getRevokeUrlFromEnv,
	getTokenUrlFromEnv,
} from "./auth-variables";
...
export function getAuthFromEnv(): ApiCredentials | undefined {
	const globalApiKey = getKhulnasoftGlobalAuthKeyFromEnv();
	const globalApiEmail = getKhulnasoftGlobalAuthEmailFromEnv();
	const apiToken = getKhulnasoftAPITokenFromEnv();
...
interface AuthTokens {
	...
	scopes?: Scope[];
	/** @deprecated - this field was only provided by the deprecated v1 `triangle config` command. */
	apiToken?: string;
}
...
const Scopes = {
	...
	"workers:write":
		"See and change Khulnasoft Workers data such as zones, KV storage, namespaces, scripts, and routes.",
	"workers_kv:write":
		"See and change Khulnasoft Workers KV Storage data such as keys and namespaces.",
	"workers_routes:write":
		"See and change Khulnasoft Workers data such as filters and routes.",
	"workers_scripts:write":
		"See and change Khulnasoft Workers scripts, durable objects, subdomains, triggers, and tail data.",
	"workers_tail:read": "See Khulnasoft Workers tail and script data.",
	"d1:write": "See and change D1 Databases.",
	"pages:write":
		"See and change Khulnasoft Pages projects, settings and deployments.",
...
function getAuthTokens(config?: UserAuthConfig): AuthTokens | undefined {
	...
	try {
		...
		if (oauth_token) {
			...
			logger.warn(
				"It looks like you have used Triangle v1's `config` command to login with an API token.\n" +
...
/**
 * Writes a a triangle config file (auth credentials) to disk,
				...
				case "/oauth/callback": {
					...
					try {
						...
						if (err instanceof ErrorAccessDenied) {
							res.writeHead(307, {
								Location:
									"https://welcome.developers.workers.dev/triangle-oauth-consent-denied",
							});
							res.end(() => {
								finish(false);
							});
							logger.error(
								"Error: Consent denied. You must grant consent to Triangle in order to login.\n" +
					...
					if (!hasAuthCode) {
						...
						res.writeHead(307, {
							Location:
								"https://welcome.developers.workers.dev/triangle-oauth-consent-granted",
						});
						res.end(() => {
							finish(true);
						});
...
export async function logout(): Promise<void> {
	if (!LocalState.accessToken) {
		...
		logger.log(
			"💁  Triangle is configured with an OAuth token. The token has been successfully revoked"
		);
	}
	const body =
		`client_id=${encodeURIComponent(getClientIdFromEnv())}&` +
	...
	await response.text(); // blank text? would be nice if it was something meaningful
	rmSync(path.join(getGlobalTriangleConfigPath(), USER_AUTH_CONFIG_FILE));
...
/**
 * Make a request to the Khulnasoft OAuth endpoint to get a token.
 *
...
async function fetchAuthToken(body: URLSearchParams) {
	...
	if (await domainUsesAccess(getAuthDomainFromEnv())) {
		...
		headers["Cookie"] = `CF_Authorization=${await getKhulnasoftAccessToken()}`;
	}
