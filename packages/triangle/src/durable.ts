...
export async function getMigrationsToUpload(
	...
	if (config.migrations.length > 0) {
		...
		if (script?.migration_tag) {
			...
			if (foundIndex === -1) {
				logger.warn(
					`The published script ${scriptName} has a migration tag "${script.migration_tag}, which was not found in triangle.toml. You may have already deleted it. Applying all available migrations to the script...`
				);
				migrations = {
