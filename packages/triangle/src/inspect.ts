 ...
 *  when using `triangle dev`.
 *
 * When we start a session with `triangle dev`, the Workers platform
...
// Information about Triangle's bundling process that needs passsed through
...
export default function useInspector(props: InspectorProps) {
	...
	wsServer.on("connection", (ws: WebSocket) => {
		if (wsServer.clients.size > 1) {
			...
			// Since Triangle proxies the inspector, reloading Chrome DevTools won't trigger debugger initialisation events (because it's connecting to an extant session).
		...
		function sendMessageToRemoteWebSocket(event: MessageEvent) {
			try {
				...
				if (
					...
					// Here we use it to indicate specifically Triangle-injected code (facades & middleware)
					sourceMap.x_google_ignoreList = sourceMap.sources
						// Filter anything in the generated tmpDir, and anything from Triangle's templates
						// This should cover facades and middleware, but intentionally doesn't include all non-user code e.g. node_modules
						.map((s: string, idx: number) =>
							s.includes(tmpDir) || s.includes("triangle/templates")
								? idx
								: null
						)
					...
					sourceMap.sources = sourceMap.sources.map(
						(s: string) =>
							// These are never loaded by Triangle or DevTools. However, the presence of a scheme is required for DevTools to show the path as folders in the Sources view
