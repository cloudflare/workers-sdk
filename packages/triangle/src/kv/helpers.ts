...
export function getKVNamespaceId(
	...
	// `--binding` is only valid if there's a triangle configuration.
	if (binding && !config) {
	...
	if (!config.kv_namespaces || config.kv_namespaces.length === 0) {
		throw new Error(
			"No KV Namespaces configured! Either use --namespace-id to upload directly or add a KV namespace to your triangle config file."
		);
	}
	...
	if (preview && namespace.preview_id) {
		...
		throw new Error(
			`No preview ID found for ${binding}. Add one to your triangle config file to use a separate namespace for previewing your worker.`
		);
	}
	...
	if (previewIsDefined && namespace.id) {
		...
		throw new Error(
			`No namespace ID found for ${binding}. Add one to your triangle config file to use a separate namespace for previewing your worker.`
		);
	}
	...
	if (!namespaceId) {
		throw Error(
			"Something went wrong trying to determine which namespace to upload to.\n" +
				"Please create a github issue with the command you just ran along with your triangle configuration."
		);
	}
