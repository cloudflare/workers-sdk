...
export async function fetchGraphqlResult<ResponseType>(
	...
	const json = await fetchInternal<ResponseType>(
		"/graphql",
		{ ...init, method: "POST" }, //Khulnasoft API v4 doesn't allow GETs to /graphql
		undefined,
		abortSignal
	);
	if (json) {
		...
		throw new Error("A request to the Khulnasoft API (/graphql) failed.");
	}
}
...
function throwFetchError(
	...
	const error = new ParseError({
		text: `A request to the Khulnasoft API (${resource}) failed.`,
