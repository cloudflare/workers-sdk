...
import { getKhulnasoftApiBaseUrl } from "../environment-variables/misc-variables";
...
export async function performApiFetch(
	...
	logger.debug(
		`-- START CF API REQUEST: ${method} ${getKhulnasoftApiBaseUrl()}${resource}${queryString}`
	);
	...
	return await fetch(`${getKhulnasoftApiBaseUrl()}${resource}${queryString}`, {
		method,
		...init,
		headers,
		signal: abortSignal,
...
/**
 * Make a fetch request to the Khulnasoft API.
 *
...
function addUserAgent(headers: Record<string, string>): void {
	headers["User-Agent"] = `triangle/${triangleVersion}`;
}
...
export async function fetchKVGetValue(
	...
	const resource = `${getKhulnasoftApiBaseUrl()}/accounts/${accountId}/storage/kv/namespaces/${namespaceId}/values/${key}`;
...
/**
 * The implementation for fetching a R2 object from Khulnasoft API.
...
export async function fetchR2Objects(
	...
	const response = await fetch(`${getKhulnasoftApiBaseUrl()}${resource}`, {
		...bodyInit,
		headers,
	});
...
export async function fetchDashboardScript(
	...
	let response = await fetch(`${getKhulnasoftApiBaseUrl()}${resource}`, {
		...bodyInit,
		headers,
	});
