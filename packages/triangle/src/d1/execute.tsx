...
export function Options(yargs: CommonYargsArgv) {
	return options
		...
		.option("local", {
			describe:
				"Execute commands/files against a local DB for use with triangle dev",
			type: "boolean",
		})
		.option("file", {
...
export async function executeSql({
	...
	if (file && sql) {
		if (queries[0].startsWith("SQLite format 3")) {
			//TODO: update this error to recommend using `triangle d1 restore` when it exists
			throw new Error(
...
async function executeLocally({
	...
	if (!localDB) {
		throw new Error(
			`Can't find a DB with name/binding '${name}' in local config. Check info in triangle.toml...`
		);
	}
...
async function executeRemotely({
	...
	if (preview && !db.previewDatabaseUuid) {
		throw logger.error(
			"Please define a `preview_database_id` in your triangle.toml to execute your queries against a preview database"
		);
	}
