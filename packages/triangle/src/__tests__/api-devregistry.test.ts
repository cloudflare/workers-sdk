...
describe("multi-worker testing", () => {
	...
	beforeAll(async () => {
		childWorker = await unstable_dev(
			...
			{
				config: "src/__tests__/helpers/worker-scripts/child-triangle.toml",
				experimental: {
		...
		parentWorker = await unstable_dev(
			...
			{
				config: "src/__tests__/helpers/worker-scripts/parent-triangle.toml",
				experimental: {
					disableExperimentalWarning: true,
				},
			}
	...
	it("should be able to stop and start the server with no warning logs", async () => {
		...
		async function startWorker() {
			return await unstable_dev(
				"src/__tests__/helpers/worker-scripts/hello-world-worker.js",
				{
					// We need the triangle.toml config to specify a Worker name
