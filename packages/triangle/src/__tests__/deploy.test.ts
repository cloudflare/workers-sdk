...
describe("deploy", () => {
	...
	describe("output additional script information", () => {
		it("for first party workers, it should print worker information at log level", async () => {
			...
			await runTriangle("deploy ./index");
	...
	describe("authentication", () => {
		...
		it("drops a user into the login flow if they're unauthenticated", async () => {
			...
			await expect(runTriangle("deploy index.js")).resolves.toBeUndefined();
		...
		it("warns a user when they're authenticated with an API token in triangle config file", async () => {
			writeTriangleToml();
			writeWorkerSource();
			mockSubDomainRequest();
			mockUploadWorkerRequest();
		...
		`);
			expect(std.warn).toMatchInlineSnapshot(`
			"[33m▲ [43;33m[[43;30mWARNING[43;33m][0m [1mIt looks like you have used Triangle v1's \`config\` command to login with an API token.[0m

			  This is no longer supported in the current version of Triangle.
		...
		describe("non-TTY", () => {
			...
			it("should not throw an error in non-TTY if 'CLOUDFLARE_API_TOKEN' & 'account_id' are in scope", async () => {
				...
				await runTriangle("deploy index.js");
		...
		`);
			...
			it("should throw an error in non-TTY & there is more than one account associated with API token", async () => {
				...
				await expect(runTriangle("deploy index.js")).rejects
					.toMatchInlineSnapshot(`
			[Error: More than one account available but unable to select one in non-interactive mode.
			Please set the appropriate \`account_id\` in your \`triangle.toml\` file.
		...
		`);
			...
			it("should throw error in non-TTY if 'CLOUDFLARE_API_TOKEN' is missing", async () => {
				...
				await expect(runTriangle("deploy index.js")).rejects.toThrowError();

				expect(std.err).toMatchInlineSnapshot(`
			          "[31mX [41;31m[[41;97mERROR[41;31m][0m [1mIn a non-interactive environment, it's necessary to set a CLOUDFLARE_API_TOKEN environment variable for triangle to work. Please go to https://developers.cloudflare.com/fundamentals/api/get-started/create-token/ for instructions on how to create an api token, and assign its value to CLOUDFLARE_API_TOKEN.[0m
			...
			it("should throw error with no account ID provided and no members retrieved", async () => {
				...
				expect(std.err).toMatchInlineSnapshot(`
			          "[31mX [41;31m[[41;97mERROR[41;31m][0m [1mFailed to automatically retrieve account IDs for the logged in user.[0m
			            ...
			            its value to CLOUDFLARE_ACCOUNT_ID, or as \`account_id\` in your \`triangle.toml\` file.
	...
	describe("environments", () => {
		it("should use legacy environments by default", async () => {
			writeTriangleToml({ env: { "some-env": {} } });
			writeWorkerSource();
			mockSubDomainRequest();
			mockUploadWorkerRequest({
				env: "some-env",
			...
			await runTriangle("deploy index.js --env some-env");
			expect(std.out).toMatchInlineSnapshot(`
			"Total Upload: xx KiB / gzip: xx KiB
		...
		describe("legacy", () => {
			it("uses the script name when no environment is specified", async () => {
				...
				await runTriangle("deploy index.js --legacy-env true");
				expect(std.out).toMatchInlineSnapshot(`
			"Total Upload: xx KiB / gzip: xx KiB
			Uploaded test-name (TIMINGS)
			Published test-name (TIMINGS)
		...
		`);
			...
			it("appends the environment name when provided, and there is associated config", async () => {
				...
				await runTriangle("deploy index.js --env some-env --legacy-env true");
				expect(std.out).toMatchInlineSnapshot(`
			"Total Upload: xx KiB / gzip: xx KiB
			Uploaded test-name-some-env (TIMINGS)
			Published test-name-some-env (TIMINGS)
		...
		`);
			...
			it("should throw an error when an environment name when provided, which doesn't match those in the config", async () => {
				writeTriangleToml({ env: { "other-env": {} } });
				writeWorkerSource();
				mockSubDomainRequest();
				await expect(
					runTriangle("deploy index.js --env some-env --legacy-env true")
			...
			it("should throw an error w/ helpful message when using --env --name", async () => {
				...
				await runTriangle(
					...
					expect(err).toMatchInlineSnapshot(`
				            [Error: In legacy environment mode you cannot use --name and --env together. If you want to specify a Worker name for a specific environment you can add the following to your triangle.toml config:
				                [env.some-env]
				                name = "voyager"
				                ]
			          `)
		...
		describe("services", () => {
			it("uses the script name when no environment is specified", async () => {
				...
				await runTriangle("deploy index.js --legacy-env false");
				expect(std.out).toMatchInlineSnapshot(`
			"Total Upload: xx KiB / gzip: xx KiB
			Uploaded test-name (TIMINGS)
			Published test-name (TIMINGS)
		...
		`);
			...
			it("publishes as an environment when provided", async () => {
				...
				await runTriangle("deploy index.js --env some-env --legacy-env false");
				expect(std.out).toMatchInlineSnapshot(`
			"Total Upload: xx KiB / gzip: xx KiB
			Uploaded test-name (some-env) (TIMINGS)
			Published test-name (some-env) (TIMINGS)
	...
	it("should resolve triangle.toml relative to the entrypoint", async () => {
		fs.mkdirSync("./some-path/worker", { recursive: true });
		fs.writeFileSync(
			"./some-path/triangle.toml",
			TOML.stringify({
				name: "test-name",
				compatibility_date: "2022-01-12",
				vars: { xyz: 123 },
		...
		mockSubDomainRequest();
		await runTriangle("deploy ./some-path/worker/index.js");
		expect(std.out).toMatchInlineSnapshot(`
		"Total Upload: xx KiB / gzip: xx KiB
	...
	describe("routes", () => {
		...
		it("should deploy to a route with a pattern/{zone_id|zone_name} combo (service environments)", async () => {
			...
			await runTriangle("deploy ./index --legacy-env false --env staging");
			expect(std).toMatchInlineSnapshot(`
			Object {
			  "debug": "",
			  "err": "",
		...
		it("should deploy to legacy environment specific routes", async () => {
			...
			await runTriangle("deploy ./index --env dev --legacy-env true");
		});
		...
		it("services: should deploy to service environment specific routes", async () => {
			...
			await runTriangle("deploy ./index --env dev --legacy-env false");
		});

		it("should fallback to the Triangle v1 zone-based API if the bulk-routes API fails", async () => {
			writeTriangleToml({
				routes: ["example.com/some-route/*"],
			});
			writeWorkerSource();
		...
		it("should error if the bulk-routes API fails and trying to push to a non-production environment", async () => {
			...
			await expect(runTriangle("deploy ./index --env=staging")).rejects
...
Update them to point to this script instead?`,
	...
	describe("entry-points", () => {
		...
		it("should be able to use the `main` config as the entry-point for ESM sources", async () => {
			writeTriangleToml({ main: "./index.js" });
			writeWorkerSource();
			mockUploadWorkerRequest();
			mockSubDomainRequest();

			await runTriangle("deploy");
		...
		it("should use `main` relative to the triangle.toml not cwd", async () => {
			writeTriangleToml({
				main: "./foo/index.js",
			});
		...
		it('should use `build.upload.main` as an entry point, where `build.upload.dir` defaults to "./dist", and log a deprecation warning', async () => {
			writeTriangleToml({ build: { upload: { main: "./index.js" } } });
		...
		it("should error when both `main` and `build.upload.main` are used", async () => {
			...
			await expect(runTriangle("deploy")).rejects
				.toThrowErrorMatchingInlineSnapshot(`
			              "Processing triangle.toml configuration:
		...
		it("should be able to transpile TypeScript (esm)", async () => {
			...
			mockUploadWorkerRequest({ expectedEntry: "var foo = 100;" });
			mockSubDomainRequest();
			await runTriangle("deploy index.ts");
...
`
		...
		it("should be able to transpile entry-points in sub-directories (esm)", async () => {
			...
			await runTriangle("deploy ./src/index.js");
...
export default {};`
			...
			await runTriangle("deploy index.js --dry-run --outdir out");
...
addEventListener('fetch', event => {});`
		...
		it('should error if a site definition doesn\'t have a "bucket" field', async () => {
			...
			expect(std.out).toMatchInlineSnapshot(`
		      ...
		      `);
			expect(std.err).toMatchInlineSnapshot(`
			        "[31mX [41;31m[[41;97mERROR[41;31m][0m [1mProcessing triangle.toml configuration:[0m
		...
		it("should resolve site.entry-point relative to triangle.toml", async () => {
			const assets = [
				{ filePath: "file-1.txt", content: "Content of file-1" },
				{ filePath: "file-2.txt", content: "Content of file-2" },
			];
			...
			mockKeyListRequest(kvNamespace.id, []);
			mockUploadAssetsToKVRequest(kvNamespace.id, assets);
			await runTriangle("deploy --config ./my-site/triangle.toml");
		...
		`);
			expect(std.err).toMatchInlineSnapshot(`""`);
			expect(normalizeSlashes(std.warn)).toMatchInlineSnapshot(`
			"[33m▲ [43;33m[[43;30mWARNING[43;33m][0m [1mProcessing my-site/triangle.toml configuration:[0m
		...
		it("should error if there is no entry-point specified", async () => {
			...
			await expect(
				...
				`"Missing entry-point: The entry-point should be specified via the command line (e.g. \`triangle deploy path/to/script\`) or the \`main\` config field."`
			);
			...
			expect(std.out).toMatchInlineSnapshot(`
		      ...
		      `);
			expect(std.err).toMatchInlineSnapshot(`
			        "[31mX [41;31m[[41;97mERROR[41;31m][0m [1mMissing entry-point: The entry-point should be specified via the command line (e.g. \`triangle deploy path/to/script\`) or the \`main\` config field.[0m
		...
		it("should not require an explicit entry point when using --assets", async () => {
			...
			await runTriangle("deploy --assets assets --latest --name test-name");
			...
			[33m▲ [43;33m[[43;30mWARNING[43;33m][0m [1mUsing the latest version of the Workers runtime. To silence this warning, please choose a specific version of the runtime with --compatibility-date, or add a compatibility_date to your triangle.toml.[0m
	...
	describe("asset upload", () => {
		...
		it("should upload all the files in the directory specified by `--assets`", async () => {
			...
			mockKeyListRequest(kvNamespace.id, []);
			mockUploadAssetsToKVRequest(kvNamespace.id, assets);
			await runTriangle("deploy --assets assets");
		...
		it("should error if --assets and --site are used together", async () => {
			...
			writeWorkerSource();
			await expect(
				runTriangle("deploy --assets abc --site xyz")
			).rejects.toThrowErrorMatchingInlineSnapshot(
		...
		it("should make environment specific kv namespace for assets, even for service envs", async () => {
			...
			mockKeyListRequest(kvNamespace.id, []);
			mockUploadAssetsToKVRequest(kvNamespace.id, assets);
			await runTriangle("deploy --env some-env --legacy-env false");
		...
		it("should make environment specific kv namespace for assets, even for legacy envs", async () => {
			...
			mockKeyListRequest(kvNamespace.id, []);
			mockUploadAssetsToKVRequest(kvNamespace.id, assets);
			await runTriangle("deploy --env some-env --legacy-env true");
		...
		it("should only upload files that match the `site-include` arg", async () => {
			...
			await runTriangle("deploy --site-include file-1.txt");
		...
		it("should not upload files that match the `site-exclude` arg", async () => {
			...
			await runTriangle("deploy --site-exclude file-2.txt");
		...
		it("should use the relative path from current working directory to Worker directory when using `--assets`", async () => {
			...
			mockUploadAssetsToKVRequest(kvNamespace.id, assets);
			process.chdir("./my-assets");
			await runTriangle("deploy --assets .");
		...
		it("should abort other bucket uploads if one bucket upload fails", async () => {
			...
			await expect(
				...
				`"A request to the Khulnasoft API (/accounts/some-account-id/storage/kv/namespaces/__test-name-workers_sites_assets-id/bulk) failed."`
			);
	...
	describe("workers_dev setting", () => {
		...
		it("should error if a compatibility_date is not available in triangle.toml or cli args", async () => {
			writeWorkerSource();
			...
			expect(err?.message.replaceAll(/\d/g, "X")).toMatchInlineSnapshot(`
			        "A compatibility_date is required when publishing. Add the following to your triangle.toml file:.
			            \`\`\`
		...
		it("should enable the workers.dev domain if workers_dev is true and subdomain is not already available", async () => {
			writeTriangleToml({ workers_dev: true });
			writeWorkerSource();
			mockUploadWorkerRequest({ available_on_subdomain: false });
			mockSubDomainRequest();
			mockUpdateWorkerRequest({ enabled: true });
		...
		it("should offer to create a new workers.dev subdomain when publishing to workers_dev without one", async () => {
			...
			await expect(runTriangle("deploy ./index")).rejects
				.toThrowErrorMatchingInlineSnapshot(`
		...
		it("should not deploy to workers.dev if there are any routes defined (environments)", async () => {
			...
			await runTriangle("deploy index.js --env production");
	...
	describe("[define]", () => {
      ...
      `
			...
			mockSubDomainRequest();
			mockUploadWorkerRequest();
			await runTriangle("build --env staging");
		...
		it("can be overridden with cli args", async () => {
			...
			mockSubDomainRequest();
			mockUploadWorkerRequest();
			await runTriangle("deploy --dry-run --outdir dist --define abc:789");
	...
	describe("custom builds", () => {
        ...
        `
			...
			mockSubDomainRequest();
			await runTriangle("deploy index.js --minify");
			expect(std.out).toMatchInlineSnapshot(`
			"Total Upload: xx KiB / gzip: xx KiB
			Uploaded test-name (TIMINGS)
			Published test-name (TIMINGS)
        ...
        `
			...
			mockSubDomainRequest();
			await runTriangle("deploy -e testEnv index.js");
			expect(std.out).toMatchInlineSnapshot(`
			"Total Upload: xx KiB / gzip: xx KiB
	...
	describe("durable object migrations", () => {
		...
		`);
			...
			"[33m▲ [43;33m[[43;30mWARNING[43;33m][0m [1mProcessing triangle.toml configuration:[0m
			    ...
			    - In triangle.toml, you have configured [durable_objects] exported by this Worker (SomeClass),
			  but no [migrations] for them. This may not work as expected until you add a [migrations] section
			  to your triangle.toml. Add this configuration to your triangle.toml:
		...
		`);
			...
			it("should deploy all migrations on first deploy (--env)", async () => {
				...
				await runTriangle("deploy index.js --legacy-env false --env xyz");
				expect(std.out).toMatchInlineSnapshot(`
			"Total Upload: xx KiB / gzip: xx KiB
			Your worker has access to the following bindings:
			- Durable Objects:
	...
	describe("tail consumers", () => {
		it("should allow specifying workers as tail consumers", async () => {
			...
			await runTriangle("publish index.js");
			expect(std.out).toMatchInlineSnapshot(`
			"Total Upload: xx KiB / gzip: xx KiB
			Uploaded test-name (TIMINGS)
			Published test-name (TIMINGS)
	...
	describe("bindings", () => {
		...
		it("should error when bindings of different types have the same name", async () => {
			...
			await expect(runTriangle("deploy index.js")).rejects
				.toMatchInlineSnapshot(`
						              [Error: Processing triangle.toml configuration:
		...
		`);
			...
			it("should resolve wasm modules relative to the triangle.toml file", async () => {
				fs.mkdirSync("./path/to/and/the/path/to/", { recursive: true });
				fs.writeFileSync(
					"./path/to/triangle.toml",
					TOML.stringify({
						compatibility_date: "2022-01-12",
						name: "test-name",
						wasm_modules: {
				...
				mockSubDomainRequest();
				await runTriangle("deploy index.js --config ./path/to/triangle.toml");
				expect(std.out).toMatchInlineSnapshot(`
			"Total Upload: xx KiB / gzip: xx KiB
			Your worker has access to the following bindings:
			- Wasm Modules:
		...
		`);
			...
			it("should error when defining text blobs for modules format workers", async () => {
		        ...
		        `);
				expect(std.err).toMatchInlineSnapshot(`
			          "[31mX [41;31m[[41;97mERROR[41;31m][0m [1mYou cannot configure [text_blobs] with an ES module worker. Instead, import the file directly in your code, and optionally configure \`[rules]\` in your triangle.toml[0m
			...
			it("should resolve text blobs relative to the triangle.toml file", async () => {
				fs.mkdirSync("./path/to/and/the/path/to/", { recursive: true });
				fs.writeFileSync(
					"./path/to/triangle.toml",
					TOML.stringify({
		...
		`);
			...
			it("should error when defining data blobs for modules format workers", async () => {
		        ...
		        `);
				expect(std.err).toMatchInlineSnapshot(`
			          "[31mX [41;31m[[41;97mERROR[41;31m][0m [1mYou cannot configure [data_blobs] with an ES module worker. Instead, import the file directly in your code, and optionally configure \`[rules]\` in your triangle.toml[0m
			...
			it("should resolve data blobs relative to the triangle.toml file", async () => {
				fs.mkdirSync("./path/to/and/the/path/to/", { recursive: true });
				fs.writeFileSync(
					"./path/to/triangle.toml",
					TOML.stringify({
		...
		`);
			...
			it("should read vars passed as cli arguments", async () => {
				...
				mockSubDomainRequest();
				mockUploadWorkerRequest();
				await runTriangle("deploy index.js --var TEXT:sometext --var COUNT:1");
				expect(std).toMatchInlineSnapshot(`
			Object {
			  "debug": "",
			  "err": "",
		...
		`);
			...
			it("should support durable objects and D1", async () => {
				...
				await runTriangle("deploy index.js --outdir tmp --dry-run");
				expect(std.out).toMatchInlineSnapshot(`
			"Total Upload: xx KiB / gzip: xx KiB
			Your worker has access to the following bindings:
			- Durable Objects:
		...
		`);
			...
			"[33m▲ [43;33m[[43;30mWARNING[43;33m][0m [1mProcessing triangle.toml configuration:[0m
			    ...
			    - D1 Bindings are currently in alpha to allow the API to evolve before general availability.
			      Please report any issues to [4mhttps://github.com/khulnasoft/workers-sdk/issues/new/choose[0m
			...
			it("should error when detecting a service-worker worker implementing durable objects", async () => {
				...
				await expect(runTriangle("deploy index.js")).rejects
					.toThrowErrorMatchingInlineSnapshot(`
			                              ...
			                              You can use Durable Objects defined in other Workers by specifying a \`script_name\` in your triangle.toml, where \`script_name\` is the name of the Worker that implements that Durable Object. For example:
		...
		`);
				expect(std.err).toMatchInlineSnapshot(`""`);
				expect(std.warn).toMatchInlineSnapshot(`
			"[33m▲ [43;33m[[43;30mWARNING[43;33m][0m [1m\`triangle publish\` is deprecated and will be removed in the next major version.[0m

			  Please use \`triangle deploy\` instead, which accepts exactly the same arguments.
		...
		describe("[unsafe]", () => {
			describe("[unsafe.bindings]", () => {
				...
				it("should warn if using unsafe bindings already handled by triangle", async () => {
					writeTriangleToml({
						unsafe: {
							bindings: [
								{
					...
					expect(std.warn).toMatchInlineSnapshot(`
							"[33m▲ [43;33m[[43;30mWARNING[43;33m][0m [1mProcessing triangle.toml configuration:[0m
							    ...
							    - \\"unsafe.bindings[0]\\": {\\"name\\":\\"my-binding\\",\\"type\\":\\"plain_text\\",\\"text\\":\\"text\\"}
							      - The binding type \\"plain_text\\" is directly supported by triangle.
							        Consider migrating this unsafe binding to a format for 'plain_text' bindings that is
							  supported by triangle for optimal support.
							        For more details, see [4mhttps://developers.cloudflare.com/workers/cli-triangle/configuration[0m
	...
	describe("legacy module specifiers", () => {
		...
		it("should not match regular module specifiers when there aren't any possible legacy module matches", async () => {
			// see https://github.com/khulnasoft/workers-sdk/issues/655 for bug details
	...
	describe("tsconfig", () => {
		...
		it("should output to target es2022 even if tsconfig says otherwise", async () => {
			...
			await runTriangle("deploy index.js"); // this would throw if we tried to compile with es5
			expect(std).toMatchInlineSnapshot(`
			Object {
			  "debug": "",
			  "err": "",
	...
	describe("--outdir", () => {
		it("should generate built assets at --outdir if specified", async () => {
			...
			mockSubDomainRequest();
			mockUploadWorkerRequest();
			await runTriangle("deploy index.js --outdir some-dir");
		...
		it("should preserve the entry point file name, even when using a facade", async () => {
			...
			mockKeyListRequest(kvNamespace.id, []);
			mockUploadAssetsToKVRequest(kvNamespace.id, assets);
			await runTriangle("deploy index.js --outdir some-dir --assets assets");
			expect(fs.existsSync("some-dir/index.js")).toBe(true);
			expect(fs.existsSync("some-dir/index.js.map")).toBe(true);
			expect(std).toMatchInlineSnapshot(`
			Object {
...
`
	...
	describe("--dry-run", () => {
		it("should not deploy the worker if --dry-run is specified", async () => {
			...
			await runTriangle("deploy index.js --dry-run");
			expect(std).toMatchInlineSnapshot(`
			Object {
			  "debug": "",
			  "err": "",
	...
	describe("--node-compat", () => {
		it("should warn when using node compatibility mode", async () => {
			writeTriangleToml();
			writeWorkerSource();
			await runTriangle("deploy index.js --node-compat --dry-run");
			expect(std).toMatchInlineSnapshot(`
			Object {
			  "debug": "",
			  "err": "",
      ...
      `
			...
			try {
				await runTriangle("deploy index.js --dry-run"); // expecting this to throw, as node compatibility isn't enabled
			} catch (e) {
			...
			expect(
				...
				/The package "path" wasn't found on the file system but is built into node\.\s+Add "node_compat = true" to your triangle\.toml file to enable Node.js compatibility\./
			);
		});
      ...
      `
			...
			await runTriangle("deploy index.js --node-compat --dry-run"); // this would throw if node compatibility didn't exist
			expect(std).toMatchInlineSnapshot(`
			Object {
			  "debug": "",
			  "err": "",
      ...
      `
        ...
        export default {
          async fetch(request) {
			...
			await expect(runTriangle("deploy")).rejects.toMatchInlineSnapshot(
				`[ParseError: A request to the Khulnasoft API (/accounts/some-account-id/workers/scripts/test-name) failed.]`
			);
			expect(std).toMatchInlineSnapshot(`
			Object {
			  "debug": "",
			...
			[31mX [41;31m[[41;97mERROR[41;31m][0m [1mA request to the Khulnasoft API (/accounts/some-account-id/workers/scripts/test-name) failed.[0m
		...
		describe("unit tests", () => {
			...
			// big files that would take a while to deal with in a full triangle test
    ...
    `;
			fs.writeFileSync("index.js", scriptContent);
			await runTriangle("deploy index.js --no-bundle --dry-run --outdir dist");
		...
		`;
			fs.writeFileSync("index.js", scriptContent);
			await runTriangle("deploy index.js --dry-run --outdir dist");
			expect(fs.readFileSync("dist/index.js", "utf-8")).toMatch(scriptContent);
		});
	});
	...
	describe("--no-bundle --minify", () => {
		...
		`;
			...
			expect(std.warn).toMatchInlineSnapshot(`
			"[33m▲ [43;33m[[43;30mWARNING[43;33m][0m [1m\`--minify\` and \`--no-bundle\` can't be used together. If you want to minify your Worker and disable Triangle's bundling, please minify as part of your own bundling process.[0m
	...
	describe("--no-bundle --node-compat", () => {
		...
		`;
			...
			[33m▲ [43;33m[[43;30mWARNING[43;33m][0m [1m\`--node-compat\` and \`--no-bundle\` can't be used together. If you want to polyfill Node.js built-ins and disable Triangle's bundling, please polyfill as part of your own bundling process.[0m
	...
	it("should not deploy if there's any other kind of error when checking deployment source", async () => {
		...
		expect(std.err).toContain(
			`A request to the Khulnasoft API (/accounts/some-account-id/workers/services/test-name) failed`
		);
	});
	...
	describe("queues", () => {
		...
		it("consumer should error when a queue doesn't exist", async () => {
			...
			await expect(
				...
				`[Error: Queue "queue1" does not exist. To create it, run: triangle queues create queue1]`
			);
		});
	...
	describe("--keep-vars", () => {
		it("should send keepVars when keep-vars is passed in", async () => {
			...
			await runTriangle("deploy index.js --keep-vars");
