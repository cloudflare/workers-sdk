...
describe("generateTypes()", () => {
	...
	it("should create a DTS file at the location that the command is executed from", async () => {
		...
		await expect(runTriangle("types")).rejects.toMatchInlineSnapshot(
			`[Error: A non-triangle worker-configuration.d.ts already exists, please rename and try again.]`
		);
		expect(fs.existsSync("./worker-configuration.d.ts")).toBe(true);
	});
