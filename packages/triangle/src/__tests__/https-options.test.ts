...
describe("getHttpsOptions()", () => {
	...
	it("should use cached values if they have not expired", async () => {
		fs.mkdirSync(path.resolve(getGlobalTriangleConfigPath(), "local-cert"), {
			recursive: true,
		});
		fs.writeFileSync(
			path.resolve(getGlobalTriangleConfigPath(), "local-cert/key.pem"),
			"PRIVATE KEY"
		);
		fs.writeFileSync(
			path.resolve(getGlobalTriangleConfigPath(), "local-cert/cert.pem"),
			"PUBLIC KEY"
		);
	...
	it("should warn if not able to write to the cache (legacy config path)", async () => {
		fs.mkdirSync(path.join(os.homedir(), ".triangle"));
		...
		expect(std.warn).toMatchInlineSnapshot(`
		      "[33m▲ [43;33m[[43;30mWARNING[43;33m][0m [1mUnable to cache generated self-signed certificate in home/.triangle/local-cert.[0m
	    ...
	    `);
		expect(std.err).toMatchInlineSnapshot(`""`);
		fs.rmSync(path.join(os.homedir(), ".triangle"), { recursive: true });
	});
