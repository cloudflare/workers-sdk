...
describe("workers change behaviour with middleware with triangle dev", () => {
	runInTempDir();
...
describe("multiple middleware", () => {
	...
	it("should respond correctly with D1 databases, scheduled testing, and formatted dev errors", async () => {
		...
		const originalFormatErrors = process.env.FORMAT_TRIANGLER_ERRORS;
		...
		try {
			...
			process.env.FORMAT_TRIANGLER_ERRORS = originalFormatErrors;
			await worker.stop();
		}
	});
});
