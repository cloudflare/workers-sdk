...
describe("deployment create", () => {
	...
	it("should be aliased with 'triangle pages deploy'", async () => {
		await runTriangle("pages deploy --help");
		await endEventLoop();
...
>>>>>>> da9ba3c855317c6071eb892def4965706f2fb97f:packages/wrangler/src/__tests__/pages/deploy.test.ts
	...
	it("should upload a directory of files", async () => {
		...
		await runTriangle("pages deploy . --project-name=foo");
	...
	it("should resolve child directories correctly", async () => {
		...
		await runTriangle(`pages publish public --project-name=foo`);
	...
	it("should resolve the current directory correctly", async () => {
		...
		chdir("public");
		await runTriangle(`pages publish . --project-name=foo`);
		// We have 3 buckets, so expect 3 uploads
		expect(requests.length).toBe(3);
		// One bucket should end up with 2 files
		expect(bodies.map((b) => b.length).sort()).toEqual([1, 1, 2]);
	...
	it("should throw an error if user attempts to use config with pages", async () => {
		await expect(
			runTriangle("pages dev --config foo.toml")
		).rejects.toThrowErrorMatchingInlineSnapshot(
			`"Pages does not support triangle.toml"`
		);
		await expect(
			runTriangle("pages deploy --config foo.toml")
		).rejects.toThrowErrorMatchingInlineSnapshot(
			`"Pages does not support triangle.toml"`
		);
	});
    ...
    `
			...
			rest.post(
				...
				async (req, res, ctx) => {
					...
					expect(maybeRoutesJSONSpec).toMatchObject({
						...
						description: `Generated by triangle@${version}`,
						include: ["/hello"],
						exclude: [],
					});
		...
		await runTriangle("pages deploy public --project-name=foo");
	...
	it("should not deploy Functions projects that provide an invalid custom _routes.json file", async () => {
      ...
      `
		...
		await expect(runTriangle("pages deploy public --project-name=foo")).rejects
  ...
  return new Response(wasmGreeting + text);
	...
	describe("_worker.js bundling", () => {
		...
		it("should not bundle the _worker.js when `--no-bundle` is set", async () => {
			...
			await runTriangle("pages deploy public --project-name=foo --no-bundle");
			expect(std.out).toContain("✨ Uploading Worker bundle");
		});
		...
		it("should bundle the _worker.js when the `--bundle` is set to true", async () => {
			...
			await runTriangle("pages deploy public --bundle=true --project-name=foo");
			expect(std.out).toContain("✨ Uploading Worker bundle");
		});
	});
});
