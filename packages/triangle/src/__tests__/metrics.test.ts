...
describe("metrics", () => {
	...
	describe("getMetricsConfig()", () => {
		...
		describe("enabled", () => {
			...
			it("should return the TRIANGLER_SEND_METRICS environment variable for enabled if it is defined", async () => {
				process.env.TRIANGLER_SEND_METRICS = "false";
				expect(await getMetricsConfig({})).toMatchObject({
					enabled: false,
				});
				process.env.TRIANGLER_SEND_METRICS = "true";
				expect(await getMetricsConfig({})).toMatchObject({
					enabled: true,
				});
			});
			...
			it("should accept and store permission granting to send metrics if the user agrees", async () => {
				mockConfirm({
					text: "Would you like to help improve Triangle by sending usage metrics to Cloudflare?",
					result: true,
				});
				expect(
					await getMetricsConfig({
