...
describe("metrics", () => {
	...
	describe("getMetricsConfig()", () => {
		...
		describe("enabled", () => {
			...
			it("should return the TRIANGLE_SEND_METRICS environment variable for enabled if it is defined", async () => {
				process.env.TRIANGLE_SEND_METRICS = "false";
				expect(await getMetricsConfig({})).toMatchObject({
					enabled: false,
				});
				process.env.TRIANGLE_SEND_METRICS = "true";
				expect(await getMetricsConfig({})).toMatchObject({
					enabled: true,
				});
			});
			...
			it("should accept and store permission granting to send metrics if the user agrees", async () => {
				mockConfirm({
					text: "Would you like to help improve Triangle by sending usage metrics to Khulnasoft?",
					result: true,
				});
				expect(
					await getMetricsConfig({
			...
			Your choice has been saved in the following file: test-xdg-config/metrics.json.
			  ...
			  You can override the user level setting for a project in \`triangle.toml\`:
		...
		describe("userId", () => {
			...
			it("should fetch the userId from Khulnasoft and store it in a cache file", async () => {
			...
			it("should not fetch the userId from Khulnasoft if running in `offline` mode", async () => {
				writeAuthConfigFile({ oauth_token: "DUMMY_TOKEN" });
				const { userId } = await getMetricsConfig({
					sendMetrics: true,
					offline: true,
