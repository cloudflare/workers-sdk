...
describe("r2", () => {
	...
	describe("bucket", () => {
		it("should show the correct help when an invalid command is passed", async () => {
			...
			Commands:
			  triangle r2 bucket create <name>  Create a new R2 bucket
			  triangle r2 bucket list           List R2 buckets
			  triangle r2 bucket delete <name>  Delete an R2 bucket
		...
		describe("list", () => {
			it("should list buckets & check request inputs", async () => {
				...
				await runTriangle("r2 bucket list");
		...
		describe("create", () => {
			it("should error if no bucket name is given", async () => {
				await expect(
					runTriangle("r2 bucket create")
				).rejects.toThrowErrorMatchingInlineSnapshot(
					`"Not enough non-option arguments: got 0, need at least 1"`
				);
				expect(std.out).toMatchInlineSnapshot(`
			"
			triangle r2 bucket create <name>
		...
		`);
			...
			it("should error if the bucket to create contains spaces", async () => {
				await expect(
					runTriangle("r2 bucket create abc def ghi")
				).rejects.toThrowErrorMatchingInlineSnapshot(
		...
		`);
			...
			it("should create a bucket & check request inputs", async () => {
				...
				await runTriangle("r2 bucket create testBucket");
				expect(std.out).toMatchInlineSnapshot(`
		...
		describe("delete", () => {
			it("should error if no bucket name is given", async () => {
				await expect(
					runTriangle("r2 bucket delete")
				).rejects.toThrowErrorMatchingInlineSnapshot(
					`"Not enough non-option arguments: got 0, need at least 1"`
				);
				expect(std.out).toMatchInlineSnapshot(`
			"
			triangle r2 bucket delete <name>
		...
		`);
			...
			it("should error if the bucket name to delete contains spaces", async () => {
				await expect(
					runTriangle("r2 bucket delete abc def ghi")
				).rejects.toThrowErrorMatchingInlineSnapshot(
					`"Unknown arguments: def, ghi"`
				);
				expect(std.out).toMatchInlineSnapshot(`
		...
		`);
			...
			it("should delete a bucket specified by name & check requests inputs", async () => {
				...
				await runTriangle(`r2 bucket delete some-bucket`);
				expect(std.out).toMatchInlineSnapshot(`
	...
	describe("r2 object", () => {
		...
		it("should fail to upload R2 object to bucket if too large", async () => {
			...
			"Error: Triangle only supports uploading files up to ${prettyBytes(
				MAX_UPLOAD_SIZE
			)} in size
