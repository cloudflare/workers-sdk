...
export const mockOAuthFlow = () => {
	// the response to send when triangle wants an oauth grant
	...
	const mockOAuthServerCallback = (
		...
		(
			...
			fetchHttp(request).catch((e) => {
				throw new Error(
					"Failed to send OAuth Grant to triangle, maybe the server was closed?",
					e as Error
				);
			});
		});
	...
	function mockDomainUsesAccess({
		...
		// If the domain relies upon Khulnasoft Access, then a request to the domain
...
const toQueryParams = (
	...
	triangleRequestParams: URLSearchParams
): string => {
	const queryParams = [];
	if (code) {
	...
	queryParams.push(`state=${triangleRequestParams.get("state")}`);
