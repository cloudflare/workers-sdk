...
describe("triangle", () => {
	...
	describe("queues", () => {
		it("should show the correct help text", async () => {
			...
			Commands:
			  triangle queues list           List Queues
			  triangle queues create <name>  Create a Queue
			  triangle queues delete <name>  Delete a Queue
			  triangle queues consumer       Configure Queue Consumers
		...
		describe("list", () => {
			...
			it("should show the correct help text", async () => {
				await runTriangle("queues list --help");
				expect(std.err).toMatchInlineSnapshot(`""`);
				expect(std.out).toMatchInlineSnapshot(`
			"triangle queues list
		...
		`);
			...
			it("should list queues using --page=2", async () => {
				...
				mockListRequest(expectedQueues, expectedPage);
				await runTriangle("queues list --page=2");
		...
		describe("create", () => {
			...
			it("should show the correct help text", async () => {
				await runTriangle("queues create --help");
				expect(std.err).toMatchInlineSnapshot(`""`);
				expect(std.out).toMatchInlineSnapshot(`
			"triangle queues create <name>
		...
		`);
			...
			it("should create a queue", async () => {
				...
				await runTriangle("queues create testQueue");
				expect(std.out).toMatchInlineSnapshot(`
			...
			it("should show link to dash when not enabled", async () => {
				...
				await expect(
					runTriangle(`queues create ${queueName}`)
			...
			[31mX [41;31m[[41;97mERROR[41;31m][0m [1mA request to the Khulnasoft API (/accounts/some-account-id/workers/queues) failed.[0m

			  workers.api.error.unauthorized [code: 10023]

			  If you think this is a bug, please open an issue at:
			  [4mhttps://github.com/khulnasoft/workers-sdk/issues/new/choose[0m
		...
		describe("delete", () => {
			...
			it("should show the correct help text", async () => {
				await runTriangle("queues delete --help");
				expect(std.err).toMatchInlineSnapshot(`""`);
				expect(std.out).toMatchInlineSnapshot(`
			"triangle queues delete <name>
		...
		`);
			...
			it("should delete a queue", async () => {
				...
				await runTriangle("queues delete testQueue");
				expect(std.out).toMatchInlineSnapshot(`
		...
		describe("consumers", () => {
			it("should show the correct help text", async () => {
				await runTriangle("queues consumer --help");

				expect(std.err).toMatchInlineSnapshot(`""`);
				expect(std.out).toMatchInlineSnapshot(`
			"triangle queues consumer

			Configure Queue Consumers

			Commands:
			  triangle queues consumer add <queue-name> <script-name>     Add a Queue Consumer
			  triangle queues consumer remove <queue-name> <script-name>  Remove a Queue Consumer
		...
		`);
			...
			describe("add", () => {
				...
				it("should show the correct help text", async () => {
					await runTriangle("queues consumer add --help");
					expect(std.err).toMatchInlineSnapshot(`""`);
					expect(std.out).toMatchInlineSnapshot(`
				"triangle queues consumer add <queue-name> <script-name>
			...
			`);
				...
				it("should add a consumer using defaults", async () => {
					...
					await runTriangle("queues consumer add testQueue testScript");
					expect(std.out).toMatchInlineSnapshot(`
				...
				it("should show link to dash when not enabled", async () => {
					...
					await expect(
						runTriangle(`queues consumer add ${queueName} testScript`)
					).rejects.toThrowError();
					expect(std.out).toMatchInlineSnapshot(`
				"Adding consumer to queue testQueue.
				Queues is not currently enabled on this account. Go to https://dash.cloudflare.com/some-account-id/workers/queues to enable it.

				[31mX [41;31m[[41;97mERROR[41;31m][0m [1mA request to the Khulnasoft API (/accounts/some-account-id/workers/queues/testQueue/consumers) failed.[0m
			...
			describe("delete", () => {
				...
				it("should show the correct help text", async () => {
					await runTriangle("queues consumer remove --help");
					expect(std.err).toMatchInlineSnapshot(`""`);
					expect(std.out).toMatchInlineSnapshot(`
				"triangle queues consumer remove <queue-name> <script-name>
			...
			`);
				...
				it("should delete a consumer with no --env", async () => {
					...
					await runTriangle("queues consumer remove testQueue testScript");
