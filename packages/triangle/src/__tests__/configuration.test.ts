	...
	describe("top-level non-environment configuration", () => {
		...
		it("should warn on unexpected fields on `triggers`", async () => {
			...
			expect(normalizePath(diagnostics.renderWarnings()))
				.toMatchInlineSnapshot(`
			        "Processing project/triangle.toml configuration:
		...
		it("should resolve tsconfig relative to triangle.toml", async () => {
	...
	describe("top-level environment configuration", () => {
		it("should override config defaults with provided values", () => {
			...
			expect(diagnostics.renderWarnings()).toMatchInlineSnapshot(`
			        "Processing triangle configuration:
			          ...
			          - In triangle.toml, you have configured [durable_objects] exported by this Worker (CLASS1), but no [migrations] for them. This may not work as expected until you add a [migrations] section to your triangle.toml. Add this configuration to your triangle.toml:
		...
		describe("[build]", () => {
			it("should override build.upload config defaults with provided values and warn about deprecations", () => {
				...
				const { config, diagnostics } = normalizeAndValidateConfig(
					expectedConfig,
					path.resolve("project/triangle.toml"),
					{ env: undefined }
				);
			...
			it("should resolve custom build watch directories relative to triangle.toml", async () => {
				const expectedConfig: RawEnvironment = {
					build: {
						command: "execute some --build",
						watch_dir: "some/path",
...
function normalizePath(text: string): string {
	return text
		.replace("project\\triangle.toml", "project/triangle.toml")
