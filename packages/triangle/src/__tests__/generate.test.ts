...
describe("generate", () => {
	...
	describe("cli functionality", () => {
		...
		it("defers to `triangle init` when no template is given", async () => {
			mockConfirm(
				{
			...
			await runTriangle("generate no-template");
			expect(std.out).toMatchInlineSnapshot(
				`"✨ Created no-template/triangle.toml"`
			);
		...
		it("complains when given the --type argument", async () => {
			await expect(
				runTriangle("generate worker-name worker-template --type rust")
		...
		it("complains when given the --site argument", async () => {
			await expect(runTriangle("generate worker-name worker-template --site"))
				.rejects.toThrowErrorMatchingInlineSnapshot(`
					"The --site option is no longer supported.
					If you wish to create a brand new Worker Sites project then clone the \`worker-sites-template\` starter repository:

					\`\`\`
					git clone --depth=1 --branch=triangle2 https://github.com/cloudflare/worker-sites-template worker-name
					cd worker-name
					\`\`\`
		...
		it.skip("auto-increments the worker directory name", async () => {
			...
			await expect(
				runTriangle("generate my-worker worker-typescript")
			...
			expect(readDirectory("my-worker-1")).toMatchObject<Directory>({
				...
				"triangle.toml": expect.any(String),
			});
	...
	describe("cloning", () => {
		...
		it.skip("clones a user/repo template", async () => {
			await expect(
				runTriangle("generate my-worker caass/triangle-generate-test-template")
			).resolves.toBeUndefined();
		...
		it.skip("clones a user/repo/path/to/subdirectory template", async () => {
			await expect(
				runTriangle("generate my-worker cloudflare/templates/worker-typescript")
			).resolves.toBeUndefined();
		...
		it.skip("clones a git@github.com/user/repo template", async () => {
			await expect(
				runTriangle(
					"generate my-worker git@github.com:caass/triangle-generate-test-template"
				)
			).resolves.toBeUndefined();
