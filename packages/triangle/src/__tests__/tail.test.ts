...
describe("tail", () => {
	...
	describe("API interaction", () => {
		...
		it("should throw an error if name isn't provided", async () => {
			await expect(
				...
				`"Required Worker name missing. Please specify the Worker name in triangle.toml, or pass it as an argument with \`triangle tail <worker-name>\`"`
			);
		});
		it("warns about durable object restarts for tty", async () => {
			...
			await runTriangle("tail durable-object--websocket--response");
		...
		it("creates and then delete tails", async () => {
			...
			await runTriangle("tail test-worker");
		...
		it("should connect to the worker assigned to a given route", async () => {
			...
			await runTriangle("tail example.com/*");
		...
		it("should error if a given route is not assigned to the user's zone", async () => {
			...
			await expect(runTriangle("tail example.com/*")).rejects.toThrow();
		});
		...
		it("creates and then delete tails: legacy envs", async () => {
			...
			await runTriangle("tail test-worker --env some-env --legacy-env true");
		...
		it("creates and then delete tails: service envs", async () => {
			...
			await runTriangle("tail test-worker --env some-env --legacy-env false");
		...
		it("errors when the websocket closes unexpectedly", async () => {
			...
			await expect(runTriangle("tail test-worker")).rejects.toThrow();
		});

		it("activates debug mode when the cli arg is passed in", async () => {
			const api = mockWebsocketAPIs();
			await runTriangle("tail test-worker --debug");
	...
	describe("filtering", () => {
		it("sends sampling rate filters", async () => {
			const api = mockWebsocketAPIs();
			const tooHigh = runTriangle("tail test-worker --sampling-rate 10");
			await expect(tooHigh).rejects.toThrow();

			const tooLow = runTriangle("tail test-worker --sampling-rate -5");
			await expect(tooLow).rejects.toThrow();

			await runTriangle("tail test-worker --sampling-rate 0.25");
		...
		it("sends single status filters", async () => {
			const api = mockWebsocketAPIs();
			await runTriangle("tail test-worker --status error");
			expect(api.requests.creation[0]).toEqual({
				filters: [
					{
		...
		it("sends multiple status filters", async () => {
			const api = mockWebsocketAPIs();
			await runTriangle("tail test-worker --status error --status canceled");
			expect(api.requests.creation[0]).toEqual({
				filters: [
					{
						outcome: [
		...
		it("sends single HTTP method filters", async () => {
			const api = mockWebsocketAPIs();
			await runTriangle("tail test-worker --method POST");
			expect(api.requests.creation[0]).toEqual({
				filters: [{ method: ["POST"] }],
			});
		});
		...
		it("sends multiple HTTP method filters", async () => {
			const api = mockWebsocketAPIs();
			await runTriangle("tail test-worker --method POST --method GET");
			expect(api.requests.creation[0]).toEqual({
		...
		it("sends header filters without a query", async () => {
			const api = mockWebsocketAPIs();
			await runTriangle("tail test-worker --header X-CUSTOM-HEADER");
			expect(api.requests.creation[0]).toEqual({
		...
		it("sends header filters with a query", async () => {
			const api = mockWebsocketAPIs();
			await runTriangle("tail test-worker --header X-CUSTOM-HEADER:some-value");
			expect(api.requests.creation[0]).toEqual({
		...
		it("sends single IP filters", async () => {
			...
			await runTriangle(`tail test-worker --ip ${fakeIp}`);
			expect(api.requests.creation[0]).toEqual({
		...
		it("sends multiple IP filters", async () => {
			...
			await runTriangle(`tail test-worker --ip ${fakeIp} --ip self`);
			expect(api.requests.creation[0]).toEqual({
		...
		it("sends search filters", async () => {
			...
			await runTriangle(`tail test-worker --search ${search}`);
			expect(api.requests.creation[0]).toEqual({
		...
		it("sends everything but the kitchen sink", async () => {
			...
			await runTriangle(`tail test-worker ${cliFilters}`);
	...
	describe("printing", () => {
		...
		it("logs request messages in JSON format", async () => {
			const api = mockWebsocketAPIs();
			await runTriangle("tail test-worker --format json");
		...
		it("logs request messages in pretty format", async () => {
			const api = mockWebsocketAPIs();
			await runTriangle("tail test-worker --format pretty");
 ...
 * @returns the same type we expect when deserializing in triangle
 */
function serialize(message: TailEventMessage): WebSocket.RawData {
	if (!isRequest(message.event)) {
		...
		// works just fine and triangle never _sends_ any event messages,
 ...
 * re-stringifies with some spacing, the same way triangle tail does.
 *
...
/**
 * A mock for all the different API resources triangle accesses
 * when running `triangle tail`
 */
type MockAPI = {
	requests: {
 ...
 * Useful as a helper in our testing to check if triangle is making
 ...
 *
 * @param websocketURL a fake URL for triangle to connect a websocket to
 ...
 *
 * @param websocketURL a fake websocket URL for triangle to connect to
 ...
 * @returns a `TailEventMessage` that triangle can process and display
 */
