...
describe("triangle", () => {
	...
	describe("kv:namespace", () => {
		describe("create", () => {
			...
			it("should error if no namespace is given", async () => {
				await expect(
					runTriangle("kv:namespace create")
				).rejects.toThrowErrorMatchingInlineSnapshot(
					`"Not enough non-option arguments: got 0, need at least 1"`
				);
				expect(std.out).toMatchInlineSnapshot(`
			"
			triangle kv:namespace create <namespace>
		...
		`);
			...
			it("should error if the namespace to create contains spaces", async () => {
				await expect(
					runTriangle("kv:namespace create abc def ghi")
				).rejects.toThrowErrorMatchingInlineSnapshot(
		...
		`);
			...
			it("should error if the namespace to create is not valid", async () => {
				await expect(
					runTriangle("kv:namespace create abc-def")
				).rejects.toThrowErrorMatchingInlineSnapshot(
		...
		`);
			...
			it("should create a namespace", async () => {
				...
				await runTriangle("kv:namespace create UnitTestNamespace");
				expect(std.out).toMatchInlineSnapshot(`
			...
			it("should create a preview namespace if configured to do so", async () => {
				...
				await runTriangle("kv:namespace create UnitTestNamespace --preview");
				expect(std.out).toMatchInlineSnapshot(`
			...
			it("should create a namespace using configured worker name", async () => {
				writeFileSync("./triangle.toml", 'name = "other-worker"', "utf-8");
		...
		describe("list", () => {
			...
			it("should list namespaces", async () => {
				...
				await runTriangle("kv:namespace list");
		...
		describe("delete", () => {
			...
			it("should error if a given binding name is not in the configured kv namespaces", async () => {
				writeTriangleConfig();
				await expect(runTriangle("kv:namespace delete --binding otherBinding"))
					.rejects.toThrowErrorMatchingInlineSnapshot(`
	...
	describe("kv:key", () => {
		describe("put", () => {
			...
			it("should encode the key in the api request to put a value", async () => {
				...
				await runTriangle("kv:key put /my-key my-value --namespace-id DS9");
		...
		`);
			...
			it("should error if no binding nor namespace is provided", async () => {
				await expect(
					runTriangle("kv:key put foo bar")
				).rejects.toThrowErrorMatchingInlineSnapshot(
		...
		`);
			...
			it("should error if both binding and namespace is provided", async () => {
				await expect(
					runTriangle("kv:key put foo bar --binding x --namespace-id y")
				).rejects.toThrowErrorMatchingInlineSnapshot(
		...
		`);
			...
			it("should error if no value nor path is provided", async () => {
				await expect(
					runTriangle("kv:key put key --namespace-id 12345")
				).rejects.toThrowErrorMatchingInlineSnapshot(
		...
		`);
			...
			it("should error if both value and path is provided", async () => {
				await expect(
					runTriangle("kv:key put key value --path xyz --namespace-id 12345")
				).rejects.toThrowErrorMatchingInlineSnapshot(
		...
		`);
			...
			it("should error if a given binding name is not in the configured kv namespaces", async () => {
				writeTriangleConfig();
				await expect(
					runTriangle("kv:key put key value --binding otherBinding")
				).rejects.toThrowErrorMatchingInlineSnapshot(
					`"A namespace with binding name \\"otherBinding\\" was not found in the configured \\"kv_namespaces\\"."`
				);
			...
			it("should error if a given binding has both preview and non-preview and --preview is not specified", async () => {
				...
				await expect(
					runTriangle("kv:key put my-key my-value --binding someBinding")
				).rejects.toThrowErrorMatchingInlineSnapshot(
		...
		describe("list", () => {
			it("should list the keys of a namespace specified by namespace-id", async () => {
				...
				await runTriangle("kv:key list --namespace-id some-namespace-id");
				expect(std.err).toMatchInlineSnapshot(`""`);
				expect(std.out).toMatchInlineSnapshot(`
			          "[
			            {
			...
			it("should list the keys of a namespace specified by binding", async () => {
				...
				await runTriangle("kv:key list --binding someBinding");
				expect(std.err).toMatchInlineSnapshot(`""`);
				expect(std.out).toMatchInlineSnapshot(`
			          "[
			            {
			...
			it("should list the keys of a preview namespace specified by binding", async () => {
				...
				await runTriangle("kv:key list --binding someBinding --preview");
				expect(std.err).toMatchInlineSnapshot(`""`);
				expect(std.out).toMatchInlineSnapshot(`
			          "[
			            {
			...
			it("should error if a given binding name is not in the configured kv namespaces", async () => {
				writeTriangleConfig();
				await expect(
					runTriangle("kv:key list --binding otherBinding")
				).rejects.toThrowErrorMatchingInlineSnapshot(
					`"A namespace with binding name \\"otherBinding\\" was not found in the configured \\"kv_namespaces\\"."`
				);
				expect(std.err).toMatchInlineSnapshot(`
		...
		describe("get", () => {
			it("should get a key in a given namespace specified by namespace-id", async () => {
				...
				await runTriangle("kv:key get my-key --namespace-id some-namespace-id");
		...
		`);
			...
			it("should get a key in a given preview namespace specified by binding", async () => {
				...
				await runTriangle("kv:key get my-key --binding someBinding --preview");
				expect(proc.write).toEqual(Buffer.from("my-value"));
				expect(std.err).toMatchInlineSnapshot(`""`);
			});
			...
			it("should error if no key is provided", async () => {
				await expect(
					runTriangle("kv:key get")
				).rejects.toThrowErrorMatchingInlineSnapshot(
					`"Not enough non-option arguments: got 0, need at least 1"`
				);
				expect(std.out).toMatchInlineSnapshot(`
			"
			triangle kv:key get <key>
		...
		`);
			...
			it("should error if both binding and namespace is provided", async () => {
				await expect(
					runTriangle("kv:key get foo --binding x --namespace-id y")
				).rejects.toThrowErrorMatchingInlineSnapshot(
					`"Arguments binding and namespace-id are mutually exclusive"`
				);
		...
		`);
			...
			it("should error if a given binding name is not in the configured kv namespaces", async () => {
				writeTriangleConfig();
				await expect(
					runTriangle("kv:key get key --binding otherBinding")
				).rejects.toThrowErrorMatchingInlineSnapshot(
					`"A namespace with binding name \\"otherBinding\\" was not found in the configured \\"kv_namespaces\\"."`
				);
				expect(std.out).toMatchInlineSnapshot(`
			...
			describe("non-interactive", () => {
				...
				it("should error if there are multiple accounts available but not interactive on stdin", async () => {
					...
					await expect(runTriangle("kv:key get key --namespace-id=xxxx"))
						.rejects.toThrowErrorMatchingInlineSnapshot(`
				...
				it("should recommend using a configuration if unable to fetch memberships", async () => {
					...
					await expect(runTriangle("kv:key get key --namespace-id=xxxx"))
						.rejects.toThrowErrorMatchingInlineSnapshot(`
							...
							You may have incorrect permissions on your API token. You can skip this account check by adding an \`account_id\` in your \`triangle.toml\`, or by setting the value of CLOUDFLARE_ACCOUNT_ID\\""
						`);
				});
		...
		describe("delete", () => {
			...
			it("should encode the key in the api request to delete a value", async () => {
				...
				await runTriangle(`kv:key delete --namespace-id voyager /NCC-74656`);
			...
			it("should error if a given binding name is not in the configured kv namespaces", async () => {
				writeTriangleConfig();
				await expect(
					runTriangle(`kv:key delete --binding otherBinding someKey`)
				).rejects.toThrowErrorMatchingInlineSnapshot(
					`"A namespace with binding name \\"otherBinding\\" was not found in the configured \\"kv_namespaces\\"."`
				);
	...
	describe("kv:bulk", () => {
		describe("put", () => {
			...
			it("should error if the file is not a JSON array", async () => {
				...
				writeFileSync("./keys.json", JSON.stringify(keyValues));
				await expect(
					runTriangle(`kv:bulk put --namespace-id some-namespace-id keys.json`)
				).rejects.toThrowErrorMatchingInlineSnapshot(`
	...
	describe("local", () => {
		...
		it("should list local kv storage", async () => {
			await runTriangle(`kv:key list --namespace-id some-namespace-id --local`);
			expect(std.out).toMatchInlineSnapshot(`
			"{
			  \\"keys\\": [],
			  \\"list_complete\\": true
		...
		it("should put local bulk kv storage", async () => {
			await runTriangle(`kv:key list --namespace-id bulk-namespace-id --local`);
			expect(std.out).toMatchInlineSnapshot(`
			"{
			  \\"keys\\": [],
			  \\"list_complete\\": true
