...
describe("deployments", () => {
	...
	it("should log a help message for deployments command", async () => {
		await runTriangle("deployments --help");
		expect(std.out).toMatchInlineSnapshot(`
		"triangle deployments

		🚢 List and view details for deployments

		Commands:
		  triangle deployments list                  🚢 Displays the 10 most recent deployments for a worker
		  triangle deployments view [deployment-id]  🔍 View a deployment
	...
	describe("deployments subcommands", () => {
		describe("deployments list", () => {
			it("should log deployments", async () => {
				...
				await runTriangle("deployments list");
				expect(std.out).toMatchInlineSnapshot(`
			"🚧\`triangle deployments\` is a beta command. Please report any issues to https://github.com/khulnasoft/workers-sdk/issues/new/choose
		...
		`);
			...
			it("should log deployments for script with passed in name option", async () => {
				await runTriangle("deployments list --name something-else");
				expect(std.out).toMatchInlineSnapshot(`
			"🚧\`triangle deployments\` is a beta command. Please report any issues to https://github.com/khulnasoft/workers-sdk/issues/new/choose
		...
		`);
			...
			it("should error on missing script name", async () => {
				await expect(
					runTriangle("deployments list")
				).rejects.toMatchInlineSnapshot(
					`[Error: Required Worker name missing. Please specify the Worker name in triangle.toml, or pass it as an argument with \`--name\`]`
				);
			});
		});
		describe("deployment view", () => {
			it("should log deployment details", async () => {
				...
				await runTriangle("deployments view 1701-E");
		...
		`);
			...
			it("should log deployment details with bindings", async () => {
				...
				await runTriangle("deployments view bindings-tag");
		...
		`);
			...
			it("should automatically log latest deployment details", async () => {
				...
				await runTriangle("deployments view");
		...
		describe("rollback", () => {
			...
			it("should successfully rollback and output a success message", async () => {
				...
				await runTriangle("rollback 3mEgaU1T-Intrepid-someThing");
				expect(std.out).toMatchInlineSnapshot(`
		...
		`);
			...
			it("should early exit from rollback if user denies continuing", async () => {
				...
				await runTriangle("rollback 3mEgaU1T-Intrpid-someThing");
				expect(std.out).toMatchInlineSnapshot(`
			"🚧\`triangle rollback\` is a beta command. Please report any issues to https://github.com/khulnasoft/workers-sdk/issues/new/choose
			"
		`);
