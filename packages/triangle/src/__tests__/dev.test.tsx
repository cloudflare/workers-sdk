...
describe("triangle dev", () => {
	...
	describe("authorization", () => {
		...
		it("should kick you to the login flow when running triangle dev in remote mode without authorization", async () => {
			fs.writeFileSync("index.js", `export default {};`);
			await expect(
				runTriangle("dev --remote index.js")
			).rejects.toThrowErrorMatchingInlineSnapshot(
				`"You must be logged in to use triangle dev in remote mode. Try logging in, or run triangle dev --local."`
			);
		});
	...
	describe("compatibility-date", () => {
		it("should not warn if there is no triangle.toml and no compatibility-date specified", async () => {
			fs.writeFileSync("index.js", `export default {};`);
			await runTriangle("dev index.js");
			expect(std.out).toMatchInlineSnapshot(`""`);
			expect(std.warn).toMatchInlineSnapshot(`""`);
			expect(std.err).toMatchInlineSnapshot(`""`);
		});

		it("should warn if there is a triangle.toml but no compatibility-date", async () => {
			writeTriangleToml({
				main: "index.js",
				compatibility_date: undefined,
			});
			fs.writeFileSync("index.js", `export default {};`);
			await runTriangle("dev");
			...
			expect(std.warn.replaceAll(currentDate, "<current-date>"))
				.toMatchInlineSnapshot(`
			        "[33m▲ [43;33m[[43;30mWARNING[43;33m][0m [1mNo compatibility_date was specified. Using today's date: <current-date>.[0m
			          ...
			          Add one to your triangle.toml file:
			          \`\`\`
		...
		it("should not warn if there is a triangle.toml but compatibility-date is specified at the command line", async () => {
			writeTriangleToml({
				main: "index.js",
				compatibility_date: undefined,
			});
			fs.writeFileSync("index.js", `export default {};`);
			await runTriangle("dev --compatibility-date=2020-01-01");
			expect(std.out).toMatchInlineSnapshot(`""`);
			expect(std.warn).toMatchInlineSnapshot(`""`);
			expect(std.err).toMatchInlineSnapshot(`""`);
		});
	...
	describe("usage-model", () => {
		it("should read triangle.toml's usage_model", async () => {
			writeTriangleToml({
				main: "index.js",
				usage_model: "unbound",
			});
		...
		it("should read triangle.toml's usage_model in local mode", async () => {
			writeTriangleToml({
				main: "index.js",
				usage_model: "unbound",
			});
	...
	describe("entry-points", () => {
		it("should error if there is no entry-point specified", async () => {
			...
			await expect(
				...
				`"Missing entry-point: The entry-point should be specified via the command line (e.g. \`triangle dev path/to/script\`) or the \`main\` config field."`
			);
			...
			expect(std.out).toMatchInlineSnapshot(`
		      ...
		      `);
			expect(std.err).toMatchInlineSnapshot(`
			        "[31mX [41;31m[[41;97mERROR[41;31m][0m [1mMissing entry-point: The entry-point should be specified via the command line (e.g. \`triangle dev path/to/script\`) or the \`main\` config field.[0m
		...
		it("should use `main` from a named environment", async () => {
			...
			fs.writeFileSync("index.js", `export default {};`);
			await runTriangle("dev --env=ENV1");
			expect((Dev as jest.Mock).mock.calls[0][0].entry.file).toMatch(
				/index\.js$/
			);
			expect(std.out).toMatchInlineSnapshot(`""`);
	...
	describe("host", () => {
		it("should resolve a host to its zone", async () => {
			...
			await runTriangle("dev --remote --host some-host.com");
			expect((Dev as jest.Mock).mock.calls[0][0]).toEqual(
				expect.objectContaining({
					host: "some-host.com",
					zone: "some-zone-id",
		...
		it("should read triangle.toml's dev.host", async () => {
			writeTriangleToml({
				main: "index.js",
				dev: {
					host: "some-host.com",
		...
		it("should read --route", async () => {
			...
			fs.writeFileSync("index.js", `export default {};`);
			mockGetZones("some-host.com", [{ id: "some-zone-id" }]);
			await runTriangle("dev --route http://some-host.com/some/path/*");
			expect((Dev as jest.Mock).mock.calls[0][0].host).toEqual("some-host.com");
		});

		it("should read triangle.toml's routes", async () => {
			writeTriangleToml({
				main: "index.js",
				routes: [
		...
		it("should read triangle.toml's environment specific routes", async () => {
			writeTriangleToml({
				main: "index.js",
				routes: [
			...
			fs.writeFileSync("index.js", `export default {};`);
			mockGetZones("some-host.com", [{ id: "some-zone-id" }]);
			await runTriangle("dev --env staging");
			expect((Dev as jest.Mock).mock.calls[0][0].host).toEqual("some-host.com");
		});
		...
		it("should fail for non-existing zones", async () => {
			...
			await fs.promises.writeFile("index.js", `export default {};`);
			await expect(runTriangle("dev --remote")).rejects.toEqual(
		...
		it("fails when given the pattern */* and no zone_name", async () => {
			...
			await expect(runTriangle("dev")).rejects.toEqual(err);
		});
		...
		it("given a long host, it should use the longest subdomain that resolves to a zone", async () => {
			...
			await runTriangle("dev --remote --host 111.222.333.some-host.com");
		...
		it("should error if a host can't resolve to a zone", async () => {
			...
			await expect(
				runTriangle("dev --remote --host some-host.com")
			).rejects.toThrowErrorMatchingInlineSnapshot(
		...
		it("should not try to resolve a zone when starting in local mode", async () => {
			...
			fs.writeFileSync("index.js", `export default {};`);
			await runTriangle("dev --host some-host.com");
	...
	describe("local upstream", () => {
		...
		it("should respect the option when provided", async () => {
			...
			fs.writeFileSync("index.js", `export default {};`);
			await runTriangle("dev --local-upstream some-host.com");
			expect((Dev as jest.Mock).mock.calls[0][0].localUpstream).toEqual(
				"some-host.com"
			);
		});
	...
	describe("custom builds", () => {
		...
		it("should throw an error if the entry doesn't exist after the build finishes", async () => {
			...
			await expect(runTriangle("dev")).rejects
				.toThrowErrorMatchingInlineSnapshot(`
			"The expected output file at \\"index.js\\" was not found after running custom build: node -e \\"4+4;\\".
			The \`main\` property in triangle.toml should point to the file generated by the custom build."
		`);
			expect(std.out).toMatchInlineSnapshot(`
		...
		describe(".env", () => {
			...
			it("should prefer to load environment variables from `.env.<environment>` if `--env <environment>` is set", async () => {
				await runTriangle("dev --env custom");
				const output = fs.readFileSync("var.txt", "utf8");
	...
	describe("upstream-protocol", () => {
		...
		it("should warn if `--upstream-protocol=http` is used", async () => {
			...
			fs.writeFileSync("index.js", `export default {};`);
			await runTriangle("dev --upstream-protocol=http");
			expect((Dev as jest.Mock).mock.calls[0][0].upstreamProtocol).toEqual(
				"http"
			);
			expect(std.out).toMatchInlineSnapshot(`""`);
			expect(std.warn).toMatchInlineSnapshot(`
			        "[33m▲ [43;33m[[43;30mWARNING[43;33m][0m [1mSetting upstream-protocol to http is not currently implemented.[0m
			          ...
			          [4mhttps://github.com/khulnasoft/workers-sdk/issues/583[0m.
	...
	describe("local-protocol", () => {
		...
		it("should use `local_protocol` from `triangle.toml`, if available", async () => {
			writeTriangleToml({
				main: "index.js",
				dev: {
		...
		it("should use --local-protocol command line arg, if provided", async () => {
			// Here we show that the command line overrides the triangle.toml by
			...
			fs.writeFileSync("index.js", `export default {};`);
			await runTriangle("dev --local-protocol=http");
			expect((Dev as jest.Mock).mock.calls[0][0].localProtocol).toEqual("http");
			expect(std.out).toMatchInlineSnapshot(`""`);
			expect(std.warn).toMatchInlineSnapshot(`""`);
			expect(std.err).toMatchInlineSnapshot(`""`);
	...
	describe("ip", () => {
		...
		it("should use to `ip` from `triangle.toml`, if available", async () => {
			writeTriangleToml({
				main: "index.js",
				dev: {
					ip: "1.2.3.4",
		...
		it("should use --ip command line arg, if provided", async () => {
			...
			fs.writeFileSync("index.js", `export default {};`);
			await runTriangle("dev --ip=5.6.7.8");
	...
	describe("inspector port", () => {
		...
		it("should read --inspector-port", async () => {
			...
			fs.writeFileSync("index.js", `export default {};`);
			await runTriangle("dev --inspector-port=9999");
		...
		it("should read dev.inspector_port from triangle.toml", async () => {
			writeTriangleToml({
				main: "index.js",
				dev: {
					inspector_port: 9999,
		...
		it("should error if a bad dev.inspector_port config is provided", async () => {
			...
			await expect(runTriangle("dev")).rejects
				.toThrowErrorMatchingInlineSnapshot(`
						"Processing triangle.toml configuration:
	...
	describe("port", () => {
		...
		it("should use `port` from `triangle.toml`, if available", async () => {
			writeTriangleToml({
				main: "index.js",
				dev: {
					port: 8888,
		...
		it("should use --port command line arg, if provided", async () => {
			...
			await runTriangle("dev --port=9999");
	...
	describe("durable_objects", () => {
		it("should warn if there are remote Durable Objects, or missing migrations for local Durable Objects", async () => {
			...
			expect(std.out).toMatchInlineSnapshot(`
		      ...
		      `);
			expect(std.warn).toMatchInlineSnapshot(`
			        "[33m▲ [43;33m[[43;30mWARNING[43;33m][0m [1mProcessing triangle.toml configuration:[0m

			            - In triangle.toml, you have configured [durable_objects] exported by this Worker (CLASS_1,
			          CLASS_3), but no [migrations] for them. This may not work as expected until you add a [migrations]
			          section to your triangle.toml. Add this configuration to your triangle.toml:
			              ...
			              Refer to
			          [4mhttps://developers.cloudflare.com/workers/learning/using-durable-objects/#durable-object-migrations-in-triangletoml[0m
			          for more details.
	...
	describe(".dev.vars", () => {
		it("should override `vars` bindings from `triangle.toml` with values in `.dev.vars`", async () => {
			fs.writeFileSync("index.js", `export default {};`);
      ...
      `;
		...
		it("should prefer `.dev.vars.<environment>` if `--env <environment> set`", async () => {
			...
			writeTriangleToml({ main: "index.js", env: { custom: {} } });
			await runTriangle("dev --env custom");
			const varBindings: Record<string, unknown> = (Dev as jest.Mock).mock
				.calls[0][0].bindings.vars;
	...
	describe("serve static assets", () => {
		it("should error if --site is used with no value", async () => {
			...
			Options:
			  ...
			  -r, --remote                                     Run on the global Khulnasoft network with access to production resources  [boolean] [default: false]
			      --minify                                     Minify the script  [boolean]
			      --node-compat                                Enable Node.js compatibility  [boolean]
			      --persist-to                                 Specify directory to use for local persistence (defaults to .triangle/state)  [string]
		...
		it("should error if --assets and --site are used together", async () => {
			...
			fs.writeFileSync("index.js", `export default {};`);
			await expect(
				runTriangle("dev --assets abc --site xyz")
			).rejects.toThrowErrorMatchingInlineSnapshot(
		...
		it("should warn if --assets is used", async () => {
			...
			await runTriangle('dev --assets "./assets"');
			expect(std).toMatchInlineSnapshot(`
			Object {
			  "debug": "",
			  "err": "",
		...
		it("should warn if config.assets is used", async () => {
			...
			Object {
			  ...
			  "info": "",
			  "out": "",
			  "warn": "[33m▲ [43;33m[[43;30mWARNING[43;33m][0m [1mProcessing triangle.toml configuration:[0m
	...
	describe("--inspect", () => {
		it("should warn if --inspect is used", async () => {
			fs.writeFileSync("index.js", `export default {};`);
			await runTriangle("dev index.js --inspect");
			expect(std).toMatchInlineSnapshot(`
			Object {
			  "debug": "",
			  "err": "",
		...
		it("should pass false, without a warning", async () => {
			fs.writeFileSync("index.js", `export default {};`);
			await runTriangle("dev index.js --inspect false");
	...
	describe("--log-level", () => {
		it("should not output warnings with log-level 'none'", async () => {
			fs.writeFileSync("index.js", `export default {};`);
			await runTriangle("dev index.js --inspect --log-level none");
			expect(std).toMatchInlineSnapshot(`
			Object {
			  "debug": "",
			  "err": "",
		...
		it("should output warnings with log-level 'warn'", async () => {
			fs.writeFileSync("index.js", `export default {};`);
			await runTriangle("dev index.js --inspect --log-level warn");
			expect(std).toMatchInlineSnapshot(`
			Object {
			  "debug": "",
			  "err": "",
		...
		it("should not output Errors with log-level error", async () => {
			fs.writeFileSync("index.js", `export default {};`);
			await runTriangle("dev index.js --inspect --log-level debug");
