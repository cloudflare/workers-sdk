...
describe("dispatch-namespace", () => {
	...
	it("should display a list of available subcommands, for dispatch-namespace with no subcommand", async () => {
		await runTriangle("dispatch-namespace");
		...
		Object {
		  ...
		  "err": "",
		  "info": "",
		  "out": "triangle dispatch-namespace

		📦 Interact with a dispatch namespace

		Commands:
		  triangle dispatch-namespace list                          List all dispatch namespaces
		  triangle dispatch-namespace get <name>                    Get information about a dispatch namespace
		  triangle dispatch-namespace create <name>                 Create a dispatch namespace
		  triangle dispatch-namespace delete <name>                 Delete a dispatch namespace
		  triangle dispatch-namespace rename <old-name> <new-name>  Rename a dispatch namespace
	...
	describe("create namespace", () => {
		...
		it("should display help for create", async () => {
			await expect(
				runTriangle("dispatch-namespace create")
			...
			"
			triangle dispatch-namespace create <name>
		...
		it("should attempt to create the given namespace", async () => {
			await runTriangle(`dispatch-namespace create ${namespaceName}`);
	...
	describe("delete namespace", () => {
		...
		it("should try to delete the given namespace", async () => {
			await runTriangle(`dispatch-namespace delete ${namespaceName}`);
	...
	describe("get namespace", () => {
		...
		it("should display help for get", async () => {
			await expect(
				runTriangle("dispatch-namespace get")
			).rejects.toThrowErrorMatchingInlineSnapshot(
				`"Not enough non-option arguments: got 0, need at least 1"`
			);
			...
			expect(std.out).toMatchInlineSnapshot(`
			"
			triangle dispatch-namespace get <name>
		...
		it("should attempt to get info for the given namespace", async () => {
			await runTriangle(`dispatch-namespace get ${namespaceName}`);
	...
	describe("list namespaces", () => {
		...
		it("should list all namespaces", async () => {
			await runTriangle("dispatch-namespace list");
			expect(std.out).toMatchInlineSnapshot(`
			"[
			  {
			    namespace_id: 'some-namespace-id',
	...
	describe("rename namespace", () => {
		...
		it("should display help for rename", async () => {
			await expect(
				runTriangle("dispatch-namespace rename")
			).rejects.toThrowErrorMatchingInlineSnapshot(
			...
			expect(std.out).toMatchInlineSnapshot(`
			"
			triangle dispatch-namespace rename <old-name> <new-name>
