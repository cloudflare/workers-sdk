...
function writeDTSFile({
	...
	const triangleOverrideDTSPath = findUpSync("worker-configuration.d.ts");
	try {
		if (
			triangleOverrideDTSPath !== undefined &&
			!fs
				.readFileSync(triangleOverrideDTSPath, "utf8")
				.includes("Generated by Triangle")
		) {
			throw new Error(
				"A non-triangle worker-configuration.d.ts already exists, please rename and try again."
			);
		}
	} catch (error) {
	...
	if (envTypeStructure.length || modulesTypeStructure.length) {
		fs.writeFileSync(
			...
			`// Generated by Triangle on ${new Date()}` + "\n" + combinedTypeStrings
		);
