 ...
 * The algorithm used here matches that of Triangle v1.
 */
function hashAsset(
	hasher: XXHashAPI,
	filePath: string,
...
export async function syncAssets(
	...
	// don't want to OOM: https://github.com/khulnasoft/workers-sdk/issues/2223)
	...
	function logDiff(line: string) {
		...
		if (LOGGER_LEVELS[level] >= LOGGER_LEVELS.debug) {
			...
			const msg =
				"   (truncating changed assets log, set `TRIANGLER_LOG=debug` environment variable to see full diff)";
...
export interface AssetPaths {
	 ...
	 *
	 * This is the directory containing triangle.toml or cwd if no config.
	 */
...
export function getAssetPaths(
	...
	const baseDirectory = assetDirectory
		? process.cwd()
		: path.resolve(path.dirname(config.configPath ?? "triangle.toml"));
