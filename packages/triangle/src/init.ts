...
import { CommandLineArgsError, printTriangleBanner } from "./index";
...
export function initOptions(yargs: CommonYargsArgv) {
	return yargs
		...
		.option("from-dash", {
			describe:
				"The name of the Worker you wish to download from the Khulnasoft dashboard for local development.",
			type: "string",
			requiresArg: true,
		})
		.option("delegate-c3", {
			describe: "Delegate to Create Khulnasoft CLI (C3)",
			type: "boolean",
			hidden: true,
			default: true,
		});
...
export type ServiceMetadataRes = {
	...
	default_environment: {
		...
		script: {
			...
			last_deployed_from?: "triangle" | "dash" | "api";
...
export async function initHandler(args: InitArgs) {
	...
	const triangleTomlDestination = path.join(
		creationDirectory,
		"./triangle.toml"
	);
	let justCreatedTriangleToml = false;
	...
	/*
    Even though the init command is now deprecated and replaced by Create Khulnasoft CLI (C3),
  ...
  */
	if (fromDashScriptName) {
		...
		try {
			...
			if ((err as { code?: number }).code === 10090) {
				throw new Error(
					"triangle couldn't find a Worker script with that name in your account.\nRun `triangle whoami` to confirm you're logged into the correct account."
				);
			}
			throw err;
		}
		...
		if (yesFlag) {
			c3Arguments.push("--triangle-defaults");
		}
		...
		// C3 will run triangle with the --do-not-delegate flag to communicate with the API
		if (args.delegateC3) {
	...
	if (fs.existsSync(triangleTomlDestination)) {
		let shouldContinue = false;
		logger.warn(
			`${path.relative(process.cwd(), triangleTomlDestination)} already exists!`
		);
		if (!fromDashScriptName) {
		...
		//    if a triangle.toml file does not exist (C3 expects to scaffold *new* projects)
		//    and if --from-dash is not set (C3 will run triangle to communicate with the API)
		if (!fromDashScriptName) {
		...
		try {
			...
			logger.log(
				`✨ Created ${path.relative(process.cwd(), triangleTomlDestination)}`
			);
			justCreatedTriangleToml = true;
		} catch (err) {
			throw new Error(
	...
	if (!pathToPackageJson) {
		...
		// If package.json exists and triangle isn't installed,
		// then ask to add triangle to devDependencies
		...
		if (
			!(
				packageJson.devDependencies?.triangle ||
				packageJson.dependencies?.triangle
			)
		) {
			const shouldInstall =
				yesFlag ||
				(await confirm(
					`Would you like to install triangle into ${path.relative(
						process.cwd(),
						pathToPackageJson
					)}?`
				));
			if (shouldInstall) {
				devDepsToInstall.push(`triangle@${triangleVersion}`);
			}
		}
	}
	...
	async function writePackageJsonScriptsAndUpdateTriangleToml({
		isWritingScripts,
		isAddingTests,
		testRunner,
		isCreatingTriangleToml,
		packagePath,
		scriptPath,
		extraToml,
	}: {
		...
		isCreatingTriangleToml: boolean;
		packagePath: string;
		scriptPath: string;
		...
		if (isCreatingTriangleToml) {
			// rewrite triangle.toml with main = "path/to/script" and any additional config specified in `extraToml`
			const parsedTriangleToml = parseTOML(
				readFileSync(triangleTomlDestination)
			);
			const newToml = {
				name: parsedTriangleToml.name,
				main: scriptPath,
				compatibility_date: parsedTriangleToml.compatibility_date,
				...extraToml,
			};
			fs.writeFileSync(triangleTomlDestination, TOML.stringify(newToml));
		}
		const isNamedWorker =
			isCreatingTriangleToml && path.dirname(packagePath) !== process.cwd();
		...
		if (isWritingScripts) {
				...
				JSON.stringify(
					{
						...
						scripts: {
							...
							start: isCreatingTriangleToml
								? `triangle dev`
								: `triangle dev ${scriptPath}`,
							deploy: isCreatingTriangleToml
								? `triangle deploy`
								: `triangle deploy ${scriptPath}`,
			...
			instructions.push(
				`\nTo start developing your Worker, run \`npx triangle dev\`${
					isCreatingTriangleToml ? "" : ` ${scriptPath}`
				}`
			);
			instructions.push(
	...
	if (isTypescriptProject) {
		if (!fs.existsSync(path.join(creationDirectory, "./src/index.ts"))) {
			...
			if (fromDashScriptName) {
				...
				await writePackageJsonScriptsAndUpdateTriangleToml({
					isWritingScripts: shouldWritePackageJsonScripts,
					isCreatingTriangleToml: justCreatedTriangleToml,
		...
		if (!fs.existsSync(path.join(creationDirectory, "./src/index.js"))) {
			...
			if (fromDashScriptName) {
				...
				await writePackageJsonScriptsAndUpdateTriangleToml({
					...
					//? Should we have Environment argument for `triangle init --from-dash` - Jacob
					extraToml: (await getWorkerConfig(accountId, fromDashScriptName, {
						defaultEnvironment,
						environments: serviceMetadata?.environments,
					})) as TOML.JsonMap,
	...
	try {
		...
		instructions.push(
			"\n🚨 triangle was unable to fetch your npm packages, but your project is ready to go"
		);
	}
...
async function installPackages(
	...
	if (depsToInstall.length > 0) {
		...
		const versionlessPackages = depsToInstall.map((dep) =>
			dep === `triangle@${triangleVersion}` ? "triangle" : dep
		);
	...
	return (
		bindings
			...
			.reduce((configObj, binding) => {
				// Some types have different names in triangle.toml
...
function assertNoSiteArg(args: InitArgs, creationDirectory: string) {
	if (args.site) {
		...
		const message =
			...
			"```\n" +
			`git clone --depth=1 --branch=triangle2 https://github.com/cloudflare/worker-sites-template ${gitDirectory}\n` +
			`cd ${gitDirectory}\n` +
			"```\n\n" +
			"Find out more about how to create and maintain Sites projects at https://developers.cloudflare.com/workers/platform/sites.\n" +
			"Have you considered using Khulnasoft Pages instead? See https://pages.cloudflare.com/.";
		throw new CommandLineArgsError(message);
	}
}
