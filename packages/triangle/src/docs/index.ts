import { printTriangleBanner } from "..";
...
export function docsOptions(yargs: CommonYargsArgv) {
	return yargs
		.positional("command", {
			describe: "Enter the triangle command you want to know more about",
			type: "string",
			array: true,
		})
		.option("yes", {
...
export async function docsHandler(
	...
	let urlToOpen =
		args.yes || !args.command || args.command.length === 0
			? "https://developers.cloudflare.com/workers/triangle/commands/"
			: "";
