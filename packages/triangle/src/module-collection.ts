...
export async function matchFiles(
	...
	// Deduplicate modules. This is usually a poorly specified `triangle.toml` configuration, but duplicate modules will cause a crash at runtime
...
export default function createModuleCollector(props: {
	...
	triangle1xlegacyModuleReferences: {
	...
	return {
		modules,
		plugin: {
			name: "triangle-module-collector",
			setup(build) {
				build.onStart(() => {
				...
				// and modifies them so they "work" in triangle v2, but with a warning
				...
				if (props.triangle1xlegacyModuleReferences.fileNames.size > 0) {
					build.onResolve(
						{
							filter: new RegExp(
								"^(" +
									[...props.triangle1xlegacyModuleReferences.fileNames]
						...
						async (args: esbuild.OnResolveArgs) => {
							...
							const filePath = path.join(
								props.triangle1xlegacyModuleReferences.rootDirectory,
								args.path
							);
							...
							if (rule) {
								...
								return {
									...
									namespace: `triangle-module-${rule.type}`, // just a tag, this isn't strictly necessary
