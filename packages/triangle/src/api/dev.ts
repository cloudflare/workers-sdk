...
export interface UnstableDevOptions {
	...
	persist?: boolean; // Enable persistence for local mode, using default path: .triangle/state
	...
	experimental?: {
		...
		disableExperimentalWarning?: boolean; // Disables triangle's warning when unstable APIs are used.
		disableDevRegistry?: boolean; // Disables triangle's support multi-worker setups. May reduce flakiness when used in tests in CI.
...
/**
 *  unstable_dev starts a triangle dev server, and returns a promise that resolves with utility functions to interact with it.
 */
export async function unstable_dev(
	...
	const devOptions: StartDevOptions = {
		...
		persist: options?.persist, // Enable persistence for local mode, using default path: .triangle/state
	...
	if (testMode) {
		// in testMode, we can run multiple triangles in parallel, but rebuilds might not work out of the box
		...
		//outside of test mode, rebuilds work fine, but only one instance of triangle will work at a time
