					...
					setup(pluginBuild) {
						...
						pluginBuild.onResolve({ filter: /^assets:/ }, async (args) => {
							...
							if (!buildOutputDirectory) {
								console.warn(
									"You're attempting to import static assets as part of your Pages Functions, but have not specified a directory in which to put them. You must use 'triangle pages dev <directory>' rather than 'triangle pages dev -- <command>' to import static assets in Functions."
								);
							}
 ...
 *
 * This allows Triangle to add shims and other wrappers
...
/**
 * Creates an esbuild plugin that can notify Triangle (via the `onEnd()`)
 * when the build completes.
 */
export function buildNotifierPlugin(onEnd: () => void): Plugin {
	return {
		name: "triangle notifier and monitor",
		setup(pluginBuild) {
	...
	setup(build) {
		build.onResolve({ filter: /.*/g }, (args) => {
			...
			throw new FatalError(
				"_worker.js is not being bundled by Triangle but it is importing from another file.\n" +
					"This will throw an error if deployed.\n" +
					"You should bundle the Worker in a pre-build step, remove the import if it is unused, or ask Triangle to bundle it by setting `--bundle`.",
				1
			);
		});
	},
