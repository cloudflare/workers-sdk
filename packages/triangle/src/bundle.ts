		...
		if (match !== null) {
			...
			const instructionForUser = `${
				forPages
					...
					: 'Add "node_compat = true" to your triangle.toml file'
...
export async function bundleWorker(
	...
	options: {
		...
		// TODO: Rip these out https://github.com/khulnasoft/workers-sdk/issues/2153
	...
	const middleware: (false | undefined | MiddlewareFn)[] = [
		...
		process.env.FORMAT_TRIANGLER_ERRORS === "true" &&
			((currentEntry: Entry) => {
		...
		// As we are not yet supporting user created middlewares yet, if no triangle applied middleware
	...
	const buildOptions: esbuild.BuildOptions & { metafile: true } = {
		...
		...(process.env.NODE_ENV && {
			define: {
				...
				// when we do a build of triangle. (re: https://github.com/khulnasoft/workers-sdk/issues/1477)
 ...
 * binding to other love triangle dev instances
 */
 ...
 * our dev environments. Is applied during triangle dev
