...
import { getScriptName, printTriangleBanner } from ".";
...
type DeploymentDetails = {
	...
	metadata: {
		author_id: string;
		author_email: string;
		source: "api" | "dash" | "triangle" | "terraform" | "other";
		created_on: string;
		modified_on: string;
	};
	resources: {
...
export async function deployments(
	...
	if (!scriptName) {
		throw new Error(
			"Required Worker name missing. Please specify the Worker name in triangle.toml, or pass it as an argument with `--name`"
		);
	}
