...
export async function getWorkerForZone(worker: string) {
	...
	if (!zone) {
		throw new Error(
			`The route '${worker}' is not part of one of your zones. Either add this zone from the Khulnasoft dashboard, or try using a route within one of your existing zones.`
		);
	}
	...
	if (!scriptName) {
		...
		if (!closestRoute) {
			throw new Error(
				`The route '${worker}' has no workers assigned. You can assign a worker to it from triangle.toml or the Khulnasoft dashboard`
			);
		} else {
			throw new Error(
