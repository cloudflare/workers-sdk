...
/**
 * Get the Worker `vars` bindings for a `triangle dev` instance of a Worker.
 *
 ...
 * `triangle.toml`). If the `--env <env>` option is set, we'll first look for
 * `.dev.vars.<env>`.
 *
 * Any values in this file, formatted like a `dotenv` file, will add to or override `vars`
 * bindings provided in the `triangle.toml`.
 */
