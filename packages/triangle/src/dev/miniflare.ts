...
// This worker proxies all external Durable Objects to the Triangle session
// where they're defined, and receives all requests from other Triangle sessions
...
const EXTERNAL_DURABLE_OBJECTS_WORKER_NAME =
	"__TRIANGLER_EXTERNAL_DURABLE_OBJECTS_WORKER";
	...
	return class {
		...
		fetch(request) {
			if (proxyUrl === undefined) {
				return new Response(\`[triangle] Couldn't find \\\`triangle dev\\\` session for class "\${className}" to proxy to\`, { status: 503 });
			}
...
export default {
	async fetch(request, env) {
		...
		if (originalUrl === null || className === null || idString === null) {
			return new Response("[triangle] Received Durable Object proxy request with missing headers", { status: 400 });
		}
...
function buildLog(): Log {
	...
	return logLevel === LogLevel.NONE ? new NoOpLog() : new TriangleLog(logLevel);
}
