...
function useDevRegistry(
	...
	useEffect(() => {
		...
		const interval =
			...
			// https://github.com/khulnasoft/workers-sdk/issues/1182
			mode === "local"
				? setInterval(() => {
						getBoundRegisteredWorkers({
							services,
...
function DevSession(props: DevSessionProps) {
	...
	// TODO(queues) support remote triangle dev
	if (
		!props.local &&
		(props.bindings.queues?.length || props.queueConsumers?.length)
	) {
		logger.warn(
			"Queues are currently in Beta and are not supported in triangle dev remote mode."
		);
	}
