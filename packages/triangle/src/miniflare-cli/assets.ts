	...
	return async function (miniflareRequest: Request) {
		if (options.proxyPort) {
			try {
				...
				return new Response(`[triangle] Could not proxy request: ${thrown}`, {
					status: 502,
				});
			}
		} else {
			try {
				...
				// TODO: Pretty error page
				return new Response(
					`[triangle] Could not serve static asset: ${thrown}`,
					{ status: 502 }
				);
			}
		}
	...
	const generateResponse = async (request: Request) => {
		...
		return await generateHandler<string>({
			...
			fetchAsset: async (assetKey) => {
				...
				if (!filepath) {
					throw new Error(
						"Could not fetch asset. Please file an issue on GitHub (https://github.com/khulnasoft/workers-sdk/issues/new/choose) with reproduction steps."
					);
				}
