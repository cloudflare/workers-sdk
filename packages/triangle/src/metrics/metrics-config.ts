...
export interface MetricsConfigOptions {
	/**
	 * Defines whether to send metrics to Khulnasoft:
...
export interface MetricsConfig {
	...
	enabled: boolean;
	/** A UID that identifies this user and machine pair for Triangle. */
	deviceId: string;
 ...
 * - the `send_metrics` setting in `triangle.toml`
...
export async function getMetricsConfig({
	...
	// If the TRIANGLE_SEND_METRICS environment variable has been set use that
	// and ignore everything else.
	const sendMetricsEnv = getTriangleSendMetricsFromEnv();
	...
	// If the project is explicitly set the `send_metrics` options in `triangle.toml`
	...
	const enabled = await confirm(
		"Would you like to help improve Triangle by sending usage metrics to Khulnasoft?"
	);
	logger.log(
...
function getMetricsConfigPath(): string {
	return path.resolve(getGlobalTriangleConfigPath(), "metrics.json");
}
...
export interface MetricsConfigFile {
	permission?: {
		/** True if Triangle should send metrics to Khulnasoft. */
		enabled: boolean;
...
/**
 * Returns an ID that uniquely identifies Triangle on this device to help collate events.
 *
...
/**
 * Ask the Khulnasoft API for the User ID of the current user.
 *
 * We will only do this if we are not "offline", e.g. not running `triangle dev --local`.
