...
export function devOptions(yargs: CommonYargsArgv) {
	return (
		yargs
			...
			.option("bundle", {
				describe: "Run triangle's compilation step before publishing",
				type: "boolean",
				hidden: true,
			})
			.option("no-bundle", {
			...
			.option("remote", {
				alias: "r",
				describe:
					"Run on the global Khulnasoft network with access to production resources",
				type: "boolean",
				default: false,
			})
			.option("local", {
...
export async function devHandler(args: DevArguments) {
	if (isWebContainer()) {
		logger.error(
			`Oh no! 😟 You tried to run \`triangle dev\` in a StackBlitz WebContainer. 🤯
...
export type StartDevOptions = DevArguments &
	// These options can be passed in directly when called with the `triangle.dev()` API.
...
To run an edge preview session for your Worker, use ${chalk.green(
		...
		if (args.local) {
			logger.warn(
				"--local is no longer required and will be removed in a future version.\n`triangle dev` now uses the local Khulnasoft Workers runtime by default. 🎉"
			);
		}
		...
		const configPath =
			args.config ||
			(args.script && findTriangleToml(path.dirname(args.script)));
		...
		// In the bootstrapper script `bin/triangle.js`, we open an IPC channel, so
...
export async function startApiDev(args: StartDevOptions) {
	...
	const configPath =
		args.config || (args.script && findTriangleToml(path.dirname(args.script)));
	...
	if (upstreamProtocol === "http") {
		logger.warn(
			"Setting upstream-protocol to http is not currently implemented.\n" +
				...
				"https://github.com/khulnasoft/workers-sdk/issues/583."
		);
	}
	...
	if (args.experimentalEnableLocalPersistence) {
		logger.warn(
			`--experimental-enable-local-persistence is deprecated.\n` +
				`Move any existing data to .triangle/state and use --persist, or\n` +
				`use --persist-to=./triangle-local-state to keep using the old path.`
		);
	}
			...
			...(configParam.kv_namespaces || []).map(
				({ binding, preview_id, id: _id }) => {
					...
					if (!preview_id) {
						...
						throw new Error(
							`In development, you should use a separate kv namespace than the one you'd use in production. Please create a new kv namespace with "triangle kv:namespace create <name> --preview" and add its id as preview_id to the kv_namespace "${binding}" in your triangle.toml`
			...
			...(configParam.r2_buckets?.map(
				({ binding, preview_bucket_name, bucket_name: _bucket_name }) => {
					...
					if (!preview_bucket_name) {
						throw new Error(
							`In development, you should use a separate r2 bucket than the one you'd use in production. Please create a new r2 bucket with "triangle r2 bucket create <name>" and add its name as preview_bucket_name to the r2_buckets "${binding}" in your triangle.toml`
						);
					}
					return {
						binding,
		...
		d1_databases: [
			...(configParam.d1_databases ?? []).map((d1Db) => {
				...
				if (!d1Db.preview_database_id && !process.env.NO_D1_WARNING) {
					logger.log(
						`--------------------\n💡 Recommendation: for development, use a preview D1 database rather than the one you'd use in production.\n💡 Create a new D1 database with "triangle d1 create <name>" and add its id as preview_database_id to the d1_database "${d1Db.binding}" in your triangle.toml\n--------------------\n`
					);
				}
				return { ...d1Db, database_id };
			}),
