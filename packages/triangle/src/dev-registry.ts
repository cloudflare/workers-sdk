...
export async function startWorkerRegistry() {
	if ((await isPortAvailable()) && !server) {
		...
		/**
		 * The registry server may have already been started by another triangle process.
		 * If triangle processes are run in parallel, isPortAvailable() can return true
