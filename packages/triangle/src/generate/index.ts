...
// Originally, generate was a rust function: https://github.com/cloudflare/triangle-legacy/blob/master/src/cli/mod.rs#L106-L123
export async function generateHandler(args: GenerateArgs) {
	...
	// delegate to `triangle init` if no template is specified
 ...
 *
 * Running `triangle generate worker https://some-git-repo` in a directory
 * with the structure:
 * ```
 * - workers
 * |
...
function parseTemplatePath(templatePath: string): {
	...
	if (!templatePath.includes("/")) {
		...
		return {
			remote: "https://github.com/khulnasoft/workers-sdk.git",
