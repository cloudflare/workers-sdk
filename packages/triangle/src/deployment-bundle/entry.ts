...
export type Entry = {
	...
	file: string;
	/** A worker's directory. Usually where the triangle.toml file is located */
	directory: string;
...
export async function getEntry(
	...
	if (args.script) {
		...
		if (config.site?.["entry-point"]) {
			...
			throw new Error(
				`Missing entry-point: The entry-point should be specified via the command line (e.g. \`triangle ${command} path/to/script\`) or the \`main\` config field.`
			);
		}
	} else {
		directory = path.resolve(path.dirname(config.configPath ?? "."));
