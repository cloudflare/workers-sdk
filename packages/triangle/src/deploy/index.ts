...
export function deployOptions(yargs: CommonYargsArgv) {
	return (
		yargs
			...
			.option("keep-vars", {
				describe:
					"Stop triangle from deleting vars that are not present in the triangle.toml\nBy default Triangle will remove all vars and replace them with those found in the triangle.toml configuration.\nIf your development approach is to modify vars after deployment via the dashboard you may wish to set this flag.",
				default: false,
				type: "boolean",
			})
			.option("legacy-env", {
...
export async function deployHandler(
	...
	// Check for deprecated `triangle publish` command
	if (args._[0] === "publish") {
		logger.warn(
			"`triangle publish` is deprecated and will be removed in the next major version.\nPlease use `triangle deploy` instead, which accepts exactly the same arguments."
		);
	}
	...
	if (args.latest) {
		logger.warn(
			"Using the latest version of the Workers runtime. To silence this warning, please choose a specific version of the runtime with --compatibility-date, or add a compatibility_date to your triangle.toml.\n"
		);
	}
