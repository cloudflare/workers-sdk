...
export default async function deploy(props: Props): Promise<void> {
	...
	if (accountId && name) {
		try {
			...
			const { default_environment } = serviceMetaData as {
				default_environment: {
					script: { last_deployed_from: "dash" | "triangle" | "api" };
				};
			};
	...
	if (!(props.compatibilityDate || config.compatibility_date)) {
		...
		throw new Error(`A compatibility_date is required when publishing. Add the following to your triangle.toml file:.
    \`\`\`
...
See https://developers.cloudflare.com/workers/platform/compatibility-dates for more information.`);
	...
	if (props.noBundle && minify) {
		logger.warn(
			"`--minify` and `--no-bundle` can't be used together. If you want to minify your Worker and disable Triangle's bundling, please minify as part of your own bundling process."
		);
	}

	if (props.noBundle && legacyNodeCompat) {
		logger.warn(
			"`--node-compat` and `--no-bundle` can't be used together. If you want to polyfill Node.js built-ins and disable Triangle's bundling, please polyfill as part of your own bundling process."
		);
	}
	...
	if (config.text_blobs && format === "modules") {
		throw new Error(
			"You cannot configure [text_blobs] with an ES module worker. Instead, import the file directly in your code, and optionally configure `[rules]` in your triangle.toml"
		);
	}

	if (config.data_blobs && format === "modules") {
		throw new Error(
			"You cannot configure [data_blobs] with an ES module worker. Instead, import the file directly in your code, and optionally configure `[rules]` in your triangle.toml"
		);
	}
	try {
		if (props.noBundle) {
...
<<<<<<< HEAD:packages/triangle/src/deploy/deploy.ts
		...
		if (
			...
			throw new Error(
				"While in beta, you cannot use D1 bindings without bundling your worker. Please remove `no_bundle` from your triangle.toml file or remove the `--no-bundle` flag to access D1 bindings."
			);
		}

=======
>>>>>>> da9ba3c855317c6071eb892def4965706f2fb97f:packages/wrangler/src/deploy/deploy.ts
		const {
			modules,
			dependencies,
	...
	for (const queue of queueNames) {
		try {
			...
			if (queueErr.code === 11000) {
				// queue_not_found
				throw new Error(
					`Queue "${queue}" does not exist. To create it, run: triangle queues create ${queue}`
				);
			}
			throw err;
		}
...
function updateQueueConsumers(config: Config): Promise<string[]>[] {
	...
	return consumers.map((consumer) => {
		...
		const envName = undefined; // TODO: script environment for triangle deploy?
