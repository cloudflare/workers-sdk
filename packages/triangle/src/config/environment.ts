...
interface EnvironmentInheritable {
	 ...
	 *
	 * @default `true` (This is a breaking change from Triangle v1)
	 * @breaking
	 * @inheritable
	 */
	...
	/**
	 * Configures a custom build step to be run by Triangle when building your Worker.
	 *
	 * Refer to the [custom builds documentation](https://developers.cloudflare.com/workers/cli-triangle/configuration#build)
	 * for more details.
	 *
	 * @default {}
	 */
	build: {
		...
		cwd?: string;
		/** The directory to watch for changes while using triangle dev, defaults to the current working directory */
...
interface EnvironmentNonInheritable {
	...
	kv_namespaces: {
		...
		id: string;
		/** The ID of the KV namespace used during `triangle dev` */
		preview_id?: string;
	}[];
	...
	d1_databases: {
		...
		database_id: string;
		/** The UUID of this D1 database for Triangle Dev (if specified). */
	...
	/**
	 * "Unsafe" tables for features that aren't directly supported by triangle.
	 *
	 * NOTE: This field is not automatically inherited from the top level environment,
	 * and so must be specified in every named environment.
	 *
	...
	unsafe: {
		 ...
		 * directly by triangle or miniflare.
		 */
		bindings?: {
			name: string;
			type: string;
...
interface EnvironmentDeprecated {
	 ...
	 *
	 * @deprecated DO NOT USE. This was a legacy bug from Triangle v1, that we do not want to support.
	 */
...
export interface DeprecatedUpload {
	 ...
	 * relative to the triangle.toml file.
	 *
	 * Defaults to the directory containing the triangle.toml file.
	 *
	 * @deprecated
	 */
	dir?: string;
