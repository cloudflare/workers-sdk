...
export function normalizeAndValidateConfig(
	...
	const diagnostics = new Diagnostics(
		`Processing ${
			configPath ? path.relative(process.cwd(), configPath) : "triangle"
		} configuration:`
	);
	...
	deprecated(
		...
		rawConfig,
		"miniflare",
		"Triangle does not use configuration in the `miniflare` section. Unless you are using Miniflare directly you can remove this section.",
		true,
		"😶 Ignored"
	);
	...
	deprecated(
		...
		rawConfig,
		"type",
		"Most common features now work out of the box with triangle, including modules, jsx, typescript, etc. If you need anything more, use a custom build.",
		true,
		"😶 Ignored"
	);
	...
	if (envName !== undefined) {
		...
		if (rawEnv !== undefined) {
			...
			const message =
				...
				`${envNames}\n` +
				`Consider adding an environment configuration section to the triangle.toml file:\n` +
				"```\n[env." +
				envName +
				"]\n```\n";
			...
			if (envNames.length > 0) {
				...
				// Only warn (rather than error) if there are not actually any environments configured in triangle.toml.
...
function normalizeAndValidateBuild(
	...
	return {
		...
		watch_dir:
			...
			// be configured in `triangle.toml`, but who knows, that may
...
function normalizeAndValidateMainField(
	...
	const configDir = path.dirname(configPath ?? "triangle.toml");
		...
		if (
			...
			if (exportedDurableObjects.length > 0 && rawMigrations.length === 0) {
				if (
					...
					diagnostics.warnings.push(
						`In triangle.toml, you have configured [durable_objects] exported by this Worker (${durableObjectClassnames.join(
							", "
						)}), but no [migrations] for them. This may not work as expected until you add a [migrations] section to your triangle.toml. Add this configuration to your triangle.toml:
...
Refer to https://developers.cloudflare.com/workers/learning/using-durable-objects/#durable-object-migrations-in-triangletoml for more details.`
					);
				}
			}
		}
 ...
 * This is to workaround older Triangle v1-era templates that have account_id = '',
 ...
 * As part of backward compatibility with Triangle v1 converting empty string to `undefined`
 */
...
const validateAndNormalizeRules = (
	...
	if (topLevelEnv === undefined) {
		...
		if (rawEnv.rules && deprecatedRules) {
			diagnostics.errors.push(
				`You cannot configure both [rules] and [build.upload.rules] in your triangle.toml. Delete the \`build.upload\` section.`
			);
...
const validateUnsafeBinding: ValidatorFn = (diagnostics, field, value) => {
	...
	if (isRequiredProperty(value, "type", "string")) {
		...
		if (safeBindings.includes(value.type)) {
			diagnostics.warnings.push(
				`The binding type "${value.type}" is directly supported by triangle.\n` +
					`Consider migrating this unsafe binding to a format for '${value.type}' bindings that is supported by triangle for optimal support.\n` +
					"For more details, see https://developers.cloudflare.com/workers/cli-triangle/configuration"
			);
		}
	} else {
