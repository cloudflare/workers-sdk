...
/**
 * Get the Triangle configuration; read it from the give `configPath` if available.
 */

export function readConfig<CommandArgs>(
	configPath: string | undefined,
	// Include command specific args as well as the triangle global flags
	args: CommandArgs & OnlyCamelCase<CommonYargsOptions>
): Config {
	let rawConfig: RawConfig = {};
	if (!configPath) {
		configPath = findTriangleToml(process.cwd(), args.experimentalJsonConfig);
	}
...
/**
 * Find the triangle.toml file by searching up the file-system
 * from the current working directory.
 */
export function findTriangleToml(
	referencePath: string = process.cwd(),
	preferJson = false
): string | undefined {
	if (preferJson) {
		return (
			findUpSync(`triangle.json`, { cwd: referencePath }) ??
			findUpSync(`triangle.toml`, { cwd: referencePath })
		);
	}
	return findUpSync(`triangle.toml`, { cwd: referencePath });
}
		...
		output.push({
			...
			entries: d1_databases.map(
				({ binding, database_name, database_id, preview_database_id }) => {
					...
					//database_id is local when running `triangle dev --local`
