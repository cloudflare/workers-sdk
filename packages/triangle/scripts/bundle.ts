...
async function buildMain(flags: BuildFlags = {}) {
	const outdir = path.resolve("./triangle-dist");
	const trianglePackageDir = path.resolve(".");
	/**
	 * The relative path between the bundled code and the Triangle package.
	 * This is used as a reliable way to compute paths relative to the Triangle package
	...
	await build({
		...
		watch: flags.watch ? watchLogger("./triangle-dist") : false,
	});
