 ...
 * any user-provided `name` field, because preview container application names
 * are generated at deploy time by `previewContainerAppName` from the resolved
 * worker name, preview slug, and class name, so they can be reliably created
 * and cleaned up alongside the preview itself.
 *
 * Default-name generation is therefore switched off. The generated name would
 * be overwritten before it is ever read, and requiring one would reject a
 * config that omits the top-level `name` and supplies it with `--worker-name`
 * instead.
 */
