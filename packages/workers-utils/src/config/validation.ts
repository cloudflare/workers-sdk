		...
		if (isRedirectedConfig) {
			...
			if (rawEnv !== undefined) {
				...
				const message =
					...
					`${envNames}\n` +
					`Consider adding an environment configuration section to the ${configFileName(
						configPath
					)} file:\n` +
					"```\n[env." +
					envName +
	...
	if (rawEnv.placement) {
		...
		// Validate new format (with region/host/hostname)
			...
			if (fieldsPresent.length > 1) {
				...
				diagnostics.errors.push(
					`"placement" fields ${presentFields
						.map((f) => `"${f}"`)
						.join(", ")} are mutually exclusive. Only one can be specified.`
				);
			}
		}
...
const validateTriggers: ValidatorFn = (
	...
	if ("crons" in triggersValue && !Array.isArray(triggersValue.crons)) {
		diagnostics.errors.push(
			`Expected "${triggersFieldName}.crons" to be of type array, but got ${JSON.stringify(
				triggersValue
			)}.`
		);
		isValid = false;
...
function validateContainerApp(
	...
	return (diagnostics, field, value, config) => {
		...
		if (!Array.isArray(value)) {
			diagnostics.errors.push(
				`"containers" field should be an array, but got ${JSON.stringify(
					value
				)}`
			);
			return false;
		...
		for (const containerAppOptional of value) {
			...
			if (!isOptionalProperty(value, "name", "string")) {
				diagnostics.errors.push(
					`Field "name", when present, should be a string, but got ${JSON.stringify(
						value
					)}`
				);
			}
...
const validateR2Binding: ValidatorFn = (diagnostics, field, value) => {
	...
	if (
		...
		diagnostics.errors.push(
			`${field}.bucket_name=${JSON.stringify(
				value.bucket_name
			)} is invalid. ${bucketFormatMessage}`
		);
		isValid = false;
	}
	...
	if (
		...
		diagnostics.errors.push(
			`${field}.preview_bucket_name= ${JSON.stringify(
				value.preview_bucket_name
			)} is invalid. ${bucketFormatMessage}`
		);
		isValid = false;
	}
...
const validateAISearchNamespaceBinding: ValidatorFn = (
	...
	if (typeof value !== "object" || value === null) {
		diagnostics.errors.push(
			`"ai_search_namespaces" bindings should be objects, but got ${JSON.stringify(
				value
			)}`
		);
		return false;
	...
	if (!isRequiredProperty(value, "namespace", "string")) {
		diagnostics.errors.push(
			`"${field}" bindings must have a "namespace" field but got ${JSON.stringify(
				value
			)}.`
		);
		isValid = false;
...
const validateAISearchBinding: ValidatorFn = (diagnostics, field, value) => {
	...
	if (!isRequiredProperty(value, "instance_name", "string")) {
		diagnostics.errors.push(
			`"${field}" bindings must have an "instance_name" field but got ${JSON.stringify(
				value
			)}.`
		);
		isValid = false;
...
const validateSecretsStoreSecretBinding: ValidatorFn = (
	...
	if (typeof value !== "object" || value === null) {
		diagnostics.errors.push(
			`"secrets_store_secrets" bindings should be objects, but got ${JSON.stringify(
				value
			)}`
		);
		return false;
...
const validateHelloWorldBinding: ValidatorFn = (diagnostics, field, value) => {
	if (typeof value !== "object" || value === null) {
		diagnostics.errors.push(
			`"unsafe_hello_world" bindings should be objects, but got ${JSON.stringify(
				value
			)}`
		);
		return false;
...
const validateWorkerLoaderBinding: ValidatorFn = (
	...
	if (typeof value !== "object" || value === null) {
		diagnostics.errors.push(
			`"worker_loader" bindings should be objects, but got ${JSON.stringify(
				value
			)}`
		);
		return false;
...
const validateRateLimitBinding: ValidatorFn = (diagnostics, field, value) => {
	if (typeof value !== "object" || value === null) {
		diagnostics.errors.push(
			`"ratelimits" bindings should be objects, but got ${JSON.stringify(
				value
			)}`
		);
		return false;
	...
	if (
		...
		if (exportedDurableObjects.length > 0 && migrations.length === 0) {
			if (
				...
				In your ${configFileName(
					configPath
				)} file, you have configured \`durable_objects\` exported by this Worker (${durableObjectClassnames.join(
					", "
				)}), but no \`migrations\` for them. This may not work as expected until you add a \`migrations\` section to your ${configFileName(
					configPath
