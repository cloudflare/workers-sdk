...
function normalizeAndValidatePlacement(
	...
	if (rawEnv.placement) {
		...
		const hasNewFormatFields = hasRegion || hasHost || hasHostname;

		// Validate that formats aren't mixed
		if (hasHint && hasNewFormatFields) {
			diagnostics.errors.push(
				`"placement" cannot have both "hint" (old format) and "region"/"host"/"hostname" (new format) fields`
			);
			return inheritable(
				diagnostics,
				topLevelEnv,
		...
		if (hasHint) {
			...
			if (mode === "targeted") {
				// For targeted mode with hint, hint must be an object with scheme and target
				if (!hint || typeof hint !== "object" || Array.isArray(hint)) {
					diagnostics.errors.push(
						`"placement.hint" must be an object when "placement.mode" is "targeted"`
					);
				} else {
					validateRequiredProperty(
						diagnostics,
