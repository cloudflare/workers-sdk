...
- `lowerDecorators(code, id)` (`@cloudflare/workers-utils/decorators`) — lowers standard decorators for the Vite and Vitest plugins, since workerd can't parse them and Vite doesn't lower them. Imports `esbuild`, an optional peer dependency, so keep it out of the main barrel.
