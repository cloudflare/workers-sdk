...
describe("search methods", async () => {
	describe("binary search", () => {
		...
		it("works for a 20,000 entry manifest", async () => {
			...
			const assetManifest = new AssetsManifest(manifest);
			for (const searchEntry of entries) {
				const path = await hashPath(searchEntry.path);
				const foundEntry = binarySearch(new Uint8Array(manifest), path);
				expect(foundEntry).not.toBe(false);
				const foundEntryWithAssetManifest = await assetManifest.get(
					searchEntry.path
				);
				expect(foundEntryWithAssetManifest).not.toBe(null);
				expect(foundEntry).toEqual(
					hexToBytes(foundEntryWithAssetManifest as string)
				);
				expect(foundEntry).toEqual(hexToBytes(searchEntry.contentHash));
			}
		});
		...
		it("throws an error if the search value is longer than the current value", async () => {
			...
			expect(() => {
				...
				`[TypeError: Search value should have a length of 16]`
			);
		});
	});
});
