...
/**
 * Convert route configs into glob patterns suitable for _routes.json `include` rules.
 *
 * @param routes - Route configs containing `routePath` and optional `middleware`
 * @returns Deduplicated array of glob patterns
 */
...
/**
 * Convert Functions routes like `/foo/:bar` to a `RoutesJSONSpec` object
 * used to determine if a request should run in the Functions user-worker.
 * Also consolidates redundant routes such as `[/foo/bar, /foo/:bar]` -> `/foo/*`.
 *
 * @param routes - Route configs from filepath discovery
 * @param description - Optional description string for the spec
 * @returns A `RoutesJSONSpec` to be written to `_routes.json`
 */
...
/**
 * Optimize and return a new Routes JSON Spec instance, performing
 * de-duping, consolidation, truncation, and sorting.
 *
 * @param spec - The routes spec to optimize
 * @returns The optimized routes spec
 */
