...
/**
 * Consolidate redundant routes — e.g. `["/api/*", "/api/foo"]` becomes `["/api/*"]`.
 *
 * @param routes - Array of route patterns. If this is in the same order as Functions
 *   routes (most-specific first), it will be more efficient to reverse it first.
 *   Routes should be in the format: `/api/foo`, `/api/*`.
...
/**
 * Shorten a route until it is within the rule length limit defined by
 * {@link MAX_FUNCTIONS_ROUTES_RULE_LENGTH}.
 *
 * For example `/aaa/bbb` may become `/aaa/*`.
 *
 * @param routeToShorten - Route to shorten if needed
 * @param maxLength - Max length of route to try to shorten to
 * @returns The shortened route
 */
