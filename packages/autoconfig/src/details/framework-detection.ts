...
/**
 * Determines whether the target project is a Cloudflare Pages application by
 * checking for a `pages_build_output_dir` config key, a cached Pages detection
 * result, or a `functions/` directory containing Pages Functions routes.
 *
 * @param projectPath Absolute path to the project root.
 * @param context Autoconfig context providing dialogs, logger, and cache access.
 * @param wranglerConfig The parsed Wrangler configuration, if one exists.
 * @returns `true` when the project is identified as a Pages project.
 */
