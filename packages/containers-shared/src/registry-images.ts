...
const REPOSITORY_COMPONENT_REGEXP = /^[a-z0-9]+(?:(?:[._]|__|-+)[a-z0-9]+)*$/;
const TAG_REGEXP = /^[A-Za-z0-9_][A-Za-z0-9_.-]{0,127}$/;
const IMAGE_TAG_REGEXP = /^[a-zA-Z0-9_][a-zA-Z0-9._-]{0,127}$/;
...
export async function listContainerImages({
	...
	return Object.entries(repositories).flatMap(([repo, tags]) => {
		...
		const visibleTags = tags.filter((tag) => isValidImageTag(tag));
...
export function parseContainerImageTag(image: string): {
	...
	if (
		...
		repository
			.split("/")
			.some((part) => !REPOSITORY_COMPONENT_REGEXP.test(part)) ||
		!TAG_REGEXP.test(tag)
	) {
...
export function isValidImageTag(value: string): boolean {
	return IMAGE_TAG_REGEXP.test(value);
}
