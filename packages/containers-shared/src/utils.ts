...
/**
 * Removes unused tags of the same image from earlier dev sessions, keeping
 * every tag that is active in the current development session.
 *
 * @param activeImageTags - Tags that must remain available to the current
 * development session.
 */
export async function cleanupDuplicateImageTags(
	dockerPath: string,
	imageTag: string,
	activeImageTags: ReadonlySet<string> = new Set()
): Promise<void> {
	try {
		const repoTags = await getImageRepoTags(dockerPath, imageTag);
		const currentRepository = getImageRepository(imageTag);
		// Build IDs can differ between Workers in the same dev session. Restrict
		// cleanup to this image repository so identical sibling images keep their tags.
		const tagsToRemove = repoTags.filter(
			(tag) =>
				getImageRepository(tag) === currentRepository &&
				tag !== imageTag &&
				!activeImageTags.has(tag)
		);
...
function getImageRepository(imageTag: string): string | undefined {
	const tagSeparatorIndex = imageTag.lastIndexOf(":");
	return tagSeparatorIndex === -1
		? undefined
		: imageTag.slice(0, tagSeparatorIndex);
}
