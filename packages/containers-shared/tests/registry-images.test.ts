...
describe("managed registry image operations", () => {
	it("lists user tags, removes account prefixes, and filters full repository names", async ({
		...
		fetchMock.mockResolvedValueOnce(
			Response.json({
				repositories: {
					"/account/team/app": ["latest", "sha256:deadbeef", "sha256-release"],
					"account/empty": [],
		...
		).toEqual([{ name: "team/app", tags: ["latest", "sha256-release"] }]);
		expect(
	...
	it("accepts valid repository separators and the maximum length", ({
		expect,
	}) => {
		for (const repository of [
			...
			expect(parseContainerImageTag(`${repository}:v1`)).toEqual({
				repository,
				tag: "v1",
			});
		}
