...
const config = {
	...
	extensions: {
		...
		paths: {
			...
			"/workflows/{workflow_name}/instances/{instance_id}/step": {
				get: {
					description:
						"Returns the full output (or error) of a single step. Unlike the instance details endpoint, streamed step outputs are returned in full rather than truncated to a preview.",
					operationId: "workflows-get-step-output",
					parameters: [
						{
							in: "path",
							name: "workflow_name",
							...
							content: {
									...
									schema: {
										allOf: [
											...
											},
											{
												properties: {
													result: {
														$ref: "#/components/schemas/workflows_step-output",
													},
												},
												type: "object",
											},
								...
								"application/octet-stream": {
									schema: {
										...
										format: "binary",
										description:
											"Raw bytes of the ReadableStream returned from step.do.",
									},
								},
							},
							description: "Get Workflow Step Output response.",
						},
						"4XX": {
							content: {
								"application/json": {
							...
							description: "Get Workflow Step Output response failure.",
						},
					},
					summary: "Get Workflow Step Output",
					tags: ["Workflows"],
				},
			},
		...
		schemas: {
			...
			"workflows_step-output": {
				...
				properties: {
					status: {
						type: "string",
						enum: ["running", "waiting", "complete", "errored"],
						description:
					...
					error: {
						...
						description: "Error details when the step errored, otherwise null.",
					},
					output: {
						nullable: true,
						description:
