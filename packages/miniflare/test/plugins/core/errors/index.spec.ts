...
test("source maps workers", async ({ expect }) => {
	...
	await esbuild.build({
		entryPoints: [MODULES_ENTRY_PATH, DEP_ENTRY_PATH],
		format: "esm",
		bundle: true,
		sourcemap: true,
		outdir: tmp,
	...
	const depMapContent = await fs.readFile(depPath + ".map", "utf8");
	...
	// The new config format provides module (and source-map) contents inline via
	// the worker `manifest`. Source maps are carried as `sourcemap`-type modules
	// (matching how Wrangler feeds them). Module names are relative identifiers
	// (workerd rejects absolute names); their `//# sourceMappingURL=` comments
	// resolve to sibling `sourcemap` modules, whose `sources` are absolute so
	// stack frames map back to the original fixtures.
	const mf = new Miniflare({
		inspectorPort: 0,
		workers: [
			// Default worker: single module with a co-located source map.
			{
				config: {
					type: "worker",
					name: "",
				...
				config: {
					...
					manifest: {
						...
						modules: {
							...
							"nested/dep.js.map": {
								type: "sourcemap",
								contents: withAbsoluteSources(depMapContent, depPath + ".map"),
							},
						},
					},
				},
