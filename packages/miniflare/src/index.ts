...
const WOBS_MCP_SERVER_NAME = "wobs-local";
const OPENCODE_SCHEMA_URL = "https://opencode.ai/config.json";

type JsonObject = Record<string, unknown>;
type McpInstallAgent = "opencode" | "claude" | "cursor";

const McpInstallRequestSchema = z.object({
	agent: z.enum(["opencode", "claude", "cursor"]),
	explorer_url: z.string().url(),
	server_path: z.string().min(1),
});
...
function getMcpInstallProjectRoot(): string {
	const override = process.env.MINIFLARE_MCP_INSTALL_ROOT;
	if (!override) {
		return process.cwd();
	}
...
function getAgentMcpConfig(explorerUrl: string, serverPath: string): JsonObject {
	return {
		command: "node",
		args: [serverPath],
		env: {
			WOBS_EXPLORER_URL: explorerUrl,
		},
	};
}
...
async function installClaudeConfig(
	...
	config.mcpServers = {
		...mcpServers,
		[WOBS_MCP_SERVER_NAME]: getAgentMcpConfig(explorerUrl, serverPath),
	};
...
export class Miniflare {
	...
	async #handleLoopbackMcpInstallRequest(request: Request): Promise<Response> {
		if (request.method !== "POST") {
			return new Response("Method Not Allowed", { status: 405 });
		}

		const parsed = McpInstallRequestSchema.safeParse(await request.json());
		if (!parsed.success) {
			return Response.json(
				{ ok: false, message: "Invalid MCP install request." },
				{ status: 400 }
			);
		}

		const data = parsed.data;
		const projectRoot = getMcpInstallProjectRoot();
		const explorerUrl = new URL(data.explorer_url).origin;

		if (!path.isAbsolute(data.server_path)) {
			return Response.json(
				{ ok: false, message: "Expected an absolute MCP server path." },
				{ status: 400 }
			);
		}

		const serverPath = path.normalize(data.server_path);

		try {
			let installedPath: string;
			switch (data.agent as McpInstallAgent) {
				case "opencode":
					installedPath = await installOpencodeConfig(
						projectRoot,
						explorerUrl,
						serverPath
					);
					break;
				case "claude":
					installedPath = await installClaudeConfig(
						projectRoot,
						explorerUrl,
						serverPath
					);
					break;
				case "cursor":
					installedPath = await installCursorConfig(
						projectRoot,
						explorerUrl,
						serverPath
					);
			...
			return Response.json({
				ok: true,
				path: installedPath,
				message: `Installed ${WOBS_MCP_SERVER_NAME} MCP config for ${data.agent}.`,
			});
		} catch (error) {
			return Response.json(
	...
	#handleLoopback = async (
		...
		try {
			...
			} else if (url.pathname === "/core/mcp/install") {
				response = await this.#handleLoopbackMcpInstallRequest(request);
