...
export class ImagesService extends WorkerEntrypoint<Env> {
	...
	async list(options?: ImageListOptions): Promise<ImageList> {
		...
		if (options?.cursor) {
			const cursorIndex = images.findIndex((i) => i.id === options.cursor);
			if (cursorIndex >= 0) {
		...
		return {
			...
			cursor: hasMore ? paginatedImages[paginatedImages.length - 1]?.id : undefined,
