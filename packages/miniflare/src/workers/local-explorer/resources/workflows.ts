...
/**
 * Get the DAG (Directed Acyclic Graph) for a workflow.
 *
 * The DAG is generated by static analysis of the workflow source code
 * during bundling and stored in the explorer binding map.
 */
export async function getWorkflowDag(
	...
	if (!info) {
		...
		if (ownerMiniflare) {
			const response = await fetchFromPeer(
				ownerMiniflare,
				`/workflows/${encodeURIComponent(workflowName)}/graph`
			);
			if (response) {
				return response;
			}
	...
	if (!info.dag) {
		return errorResponse(
			404,
			WORKFLOW_ERROR_NOT_FOUND,
			`DAG not available for workflow '${workflowName}'. The workflow source could not be analyzed.`
		);
	}

	return c.json(wrapResponse(info.dag));
}
