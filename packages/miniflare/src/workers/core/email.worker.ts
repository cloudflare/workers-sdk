...
export function isEmailRepliable(
	...
	// not repliable.
	const autoResponseSupress = incomingEmailHeaders.get("x-auto-response-supress")?.toLowerCase()
	if (
		autoResponseSupress !== undefined && autoResponseSupress !==
		"none"
	) {
		return false;
	}
	...
	const autoSubmitedValue = incomingEmailHeaders.get("auto-submitted")?.toLowerCase()
	...
	if (email.inReplyTo !== undefined && email.references !== undefined) {
		...
		if (parsedReferences === undefined) {
			console.log("Failed to parse incoming email's References header. Assuming email as non-repliable");
				return false
			}
...
*/
export async function validateReply(incomingMessage: Email, replyMessage: EmailMessage): Promise<Uint8Array> {
	...
	const expectedReferences = incomingMessage.references !== undefined ? parseMessageIdStyleValue(incomingMessage.references) : []
	const parsedIncomingMessageId = parseMessageIdStyleValue(incomingMessage.messageId)?.[0]
	// I expect the incoming email validator to catch this
	if (expectedReferences === undefined || parsedIncomingMessageId === undefined) {
		console.error("Failed to get the References or Message-Id header from the incoming email. This is unexpected as it passed through validation first.")
	...
	if (parsedReply.references !== undefined) {
		...
		if (parsedReferences === undefined || parsedReferences !== expectedReferences) {
		...
		const encodedReferences = new TextEncoder().encode(`References: ${expectedReferences.reduce((val) => `\r\n <${val}>`)}`)

		const finalReplyEmail = new Uint8Array(encodedReferences.byteLength + rawEmailBuffer.byteLength)
		// prepend References to be in the headers instead of the end of the body
		finalReplyEmail.set(encodedReferences, 0)
		finalReplyEmail.set(encodedReferences, encodedReferences.byteLength)
