...
type Env = {
	...
	[CoreBindings.SERVICE_DEFAULT_ENTRYPOINT_FALLBACK]: Fetcher;
	...
	[K in `${typeof CoreBindings.SERVICE_DEFAULT_ENTRYPOINT_PREFIX}${string}`]:
		| Fetcher
...
function getTargetService(request: Request, url: URL, env: Env) {
	let service: Fetcher | undefined =
		env[CoreBindings.SERVICE_DEFAULT_ENTRYPOINT_FALLBACK];
	...
	if (route !== null) {
		service = env[`${CoreBindings.SERVICE_DEFAULT_ENTRYPOINT_PREFIX}${route}`];
	}
	return service;
}
