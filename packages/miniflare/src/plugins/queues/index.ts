...
function bindingEntries(
	namespaces?: Record<string, { queueName: string, deliveryDelay?: number }> | string[]
): [bindingName: string, id: string][] {
	if (Array.isArray(namespaces)) {
		return namespaces.map((bindingName) => [bindingName, bindingName]);
	} else if (namespaces !== undefined) {
		return Object.entries(namespaces).map(([name, opts]) => [name, opts.queueName]);
	} else {
		return [];
	}
}

function bindingKeys(namespaces?: Record<string, { queueName: string, deliveryDelay?: number }> | string[]): string[] {
	if (Array.isArray(namespaces)) {
		return namespaces;
	} else if (namespaces !== undefined) {
