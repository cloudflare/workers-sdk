 ...
 * Triangle v1's `type = wepback` setting for workers projects.
 *
 * It's kind of gross, and not good for _new_ projects, but it should work ok at
 * getting people using Triangle v1 with the inbuilt webpack 4 support migrated
 * over to Triangle 2. Combined with docs on ejecting webpack, the pain of
 * losing 1's (tenuous at best) webpack support should be mostly mitigated.
 *
 * This plugin attempts to replicate Triangle v1's behavior 1:1 (specifically,
 * https://github.com/khulnasoft/triangle-legacy/blob/master/src/trianglejs/mod.rs#L39-L58)
 * so it:
 *
 * - figures out where the actual worker is located, and saves that location as "package_dir" (https://github.com/khulnasoft/triangle-legacy/blob/master/src/settings/toml/target.rs#L40-L50)
 *   - if it's a sites project (https://github.com/khulnasoft/triangle-legacy/blob/master/src/trianglejs/mod.rs#L161-L163)
 *     - generates a worker if necessary (https://github.com/khulnasoft/triangle-legacy/blob/master/src/settings/toml/site.rs#L42-L56)
 *   - runs `npm install` (https://github.com/khulnasoft/triangle-legacy/blob/master/src/trianglejs/mod.rs#L165)
 *   - use the "main" file of {package_dir} as the entry if none is specified (https://github.com/khulnasoft/triangle-legacy/blob/master/src/upload/package.rs#L16-L27)
 * - runs trianglejs-equivalent webpack hooks that: (https://github.com/khulnasoft/triangle-legacy/blob/master/src/trianglejs/mod.rs#L44)
 *   - assert `target` is `webworker` (https://github.com/khulnasoft/triangle-legacy/blob/master/trianglejs/index.js#L52-L60)
 *   - assert `output.filename` is `worker.js` and `output.sourceMapFilename` is `worker.map.js` (https://github.com/khulnasoft/triangle-legacy/blob/master/trianglejs/index.js#L62-L92)
 *   - bundle all emitted JS into a single file (https://github.com/khulnasoft/triangle-legacy/blob/master/trianglejs/index.js#L118-L121)
 * - takes webpack output and writes it to disk (https://github.com/khulnasoft/triangle-legacy/blob/master/src/trianglejs/mod.rs#L144)
 *   - at `{package_dir}/worker` (https://github.com/khulnasoft/triangle-legacy/blob/master/src/trianglejs/bundle.rs#L35-L37)
 *   - if there's WASM, adds some hardcoded js to import it (https://github.com/khulnasoft/triangle-legacy/blob/master/src/trianglejs/bundle.rs#L47-L64)
 */
...
import type { Config as TriangleConfig } from "triangle/src/config";
type Compilation = _compilation.Compilation;

const PLUGIN_NAME = "TriangleJsCompatWebpackPlugin";
const WASM_IMPORT = `
...
export type TriangleJsCompatWebpackPluginArgs = {
	/**
	 * Path to your triangle configuration file (triangle.toml).
	 * If omitted, an effort is made to find your file.
	 */
	pathToTriangleToml?: string;
	/**
...
export class TriangleJsCompatWebpackPlugin {
	private readonly config: TriangleConfig;
	...
	constructor({
		pathToTriangleToml,
		environment,
	}: TriangleJsCompatWebpackPluginArgs = {}) {
		this.config = readConfig(pathToTriangleToml, {
			env: environment,
			"legacy-env": true,
	...
	/**
	 * Emulates behavior from [`Target::package_dir`](https://github.com/khulnasoft/triangle-legacy/blob/master/src/settings/toml/target.rs#L40-L50).
	 *
	 * We encourage the user to specify the "context" and "entry" explicitly in
	 * their webpack config, since Triangle v1 kind of inferred that stuff but
	 * triangle 2 is very hands-off for custom builds.
	 *
	...
	private setPackageDir(
		...
		if (context === undefined || entry === undefined) {
			const weWouldGuess =
				"With `type = webpack`, Triangle v1 would try to guess where your worker lives.";
			const noLonger =
				"Now that you're running webpack outside of triangle, you need to specify this explicitly.";
	...
	/**
	 * Mimics triangle-js' [assertions for build output](https://github.com/khulnasoft/triangle-legacy/blob/master/trianglejs/index.js#L52-L92)
	 */
	...
	/**
	 * Partially equivalent to [`setup_build`](https://github.com/khulnasoft/triangle-legacy/blob/master/src/trianglejs/mod.rs#L154-L210)
	 * in Triangle v1, with the notable exception of preparing to run webpack
	 ...
	 * equivalent to [`Site::scaffold_worker`](https://github.com/khulnasoft/triangle-legacy/blob/master/src/settings/toml/site.rs#L42-L56)
	 * in Triangle v1.
	 */
	...
	private bundleAssets({ assets }: Compilation) {
		...
		// https://github.com/khulnasoft/triangle-legacy/blob/master/trianglejs/index.js#L118-L121
		this.output = {
	...
	/**
	 * Mimics [`Bundle::write`](https://github.com/khulnasoft/triangle-legacy/blob/master/src/trianglejs/bundle.rs#L34-L68)
	 */
	private writeOutput() {
		if (!this.output) {
...
export default TriangleJsCompatWebpackPlugin;
