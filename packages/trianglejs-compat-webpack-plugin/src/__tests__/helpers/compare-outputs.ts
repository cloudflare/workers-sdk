...
import { mockConsoleMethods } from "triangle/src/__tests__/helpers/mock-console";
import { runTriangle as runTriangle2 } from "triangle/src/__tests__/helpers/run-triangle";
import { writeWorkerSource } from "triangle/src/__tests__/helpers/write-worker-source";
import writeTriangleToml from "triangle/src/__tests__/helpers/write-triangle-toml";
...
import { runTriangle1 } from "./run-triangle-1";
...
import type { RawConfig } from "triangle/src/config";
// import process from "node:process";

type PartialTriangleConfig = Omit<RawConfig, "type" | "webpack_config">;
...
export type ProjectOptions = {
	triangleConfig?: PartialTriangleConfig;
...
export async function compareOutputs({
	...
	const triangle1Dir = path.join(parentDir, "triangle-1");
	const triangle2Dir = path.join(parentDir, "triangle-2");

	// Triangle v1
	fs.mkdirSync(triangle1Dir);
	process.chdir(triangle1Dir);
	...
	let triangle1result: ExecaReturnValue<string> | ExecaError<string>;
	try {
		triangle1result = await runTriangle1("build");
	} catch (e) {
		const error = e as ExecaError<string>;
		if (isAssertionError(error)) {
			throw error;
		} else {
			triangle1result = error;
		}
	...
	const triangle1 = {
		result: triangle1result,
		std: {
			out: std.out,
			err: std.err,
			warn: std.warn,
		},
		output: walk(path.join(triangle1Dir, "worker")),
	};
	...
	// triangle 2
	fs.mkdirSync(triangle2Dir);
	process.chdir(triangle2Dir);

	writeTriangleToml({
		...triangleConfig,
		build: {
			...triangleConfig?.build,
	...
	writePackageJson({
		...
		dependencies: {
			...
			webpack: "^4.46.0",
			"webpack-cli": "^4.9.2",
			"trianglejs-compat-webpack-plugin": PATH_TO_PLUGIN,
		},
	});
	...
	let triangle2result: Error | undefined = undefined;
	...
	try {
		await withCapturedChildProcessOutput(
			async () => await runTriangle2("deploy"),
			{
				stdout,
				stderr,
			}
	...
	if (triangle2result !== undefined && isAssertionError(triangle2result)) {
		throw triangle2result;
	}

	const triangle2 = {
		result: triangle2result,
		std: {
			out: std.out,
			err: std.err,
			warn: std.warn,
		},
		output: walk(path.join(triangle2Dir, "worker")),
	};
 ...
 *
 * Useful for capturing output from webpack when called by triangle 2.
