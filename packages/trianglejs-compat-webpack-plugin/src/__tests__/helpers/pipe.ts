...
export const cleanMessage = (message: string): string =>
	message
		...
		.replaceAll(PATH_TO_TRIANGLER, "[triangle v1]") // standardize calls to triangle v1
		.replaceAll(/found .+ vulnerabilities/gm, "found [some] vulnerabilities") // vuln counts
		.replaceAll(
			// remove specific paths to node, trianglejs, and output file
			/(Error: failed to execute `)(\S*node\S*) (\S*trianglejs\S*) \S*(--output-file=)(\S+)(.+)/gm,
			'$1"node" "trianglejs" "$4[file]"$6'
		)
		.replaceAll("✨  ", "") // triangle only does emojis on unix
